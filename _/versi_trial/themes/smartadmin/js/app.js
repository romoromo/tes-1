...
function reloadDT2(idelement, isSort) {
		isSort = ( (typeof isSort!=undefined && isSort) || isSort==null) ? true : false;
		loadScript("themes/smartadmin/ajs/datatables/jquery.dataTables.min.js", function(){
			loadScript("themes/smartadmin/ajs/datatables/dataTables.colVis.min.js", function(){
				loadScript("themes/smartadmin/ajs/datatables/dataTables.tableTools.min.js", function(){
...
function panggil_ulang(){
	$.ajax({
		type : "POST",
		reloadDTurl : "antrian/antrian/PanggilUlang",
		success : function(data) {
	...
	function getBulanDenda(tglbatas, tglterima)
	{
		//tglterima = moment('2017-10-11').toDate();
		//tglbatas = moment('2017-09-10').toDate();

		tglterima = moment(tglterima).toDate();
		tglbatas = moment(tglbatas).toDate();
		...
		if(tglterima>tglbatas) {
			...
			while(tglbatas_inc<tglterima) {
				tglbatas_inc = moment(tglbatas_inc).add(1, 'M').toDate();
	...
	function convDate(date) {
		strconvDate = date.toString().split('/').join('-').split('-').reverse().join('-');
		return strconvDate;
	}
