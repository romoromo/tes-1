...
$(document).ready(function() {
	...
	if(window.isLogin) {
		do_idle(900000);
		setInterval (cek, 30000);
		setInterval (cekproses, 30000);
	}
	/*setTimeout(function () {         
			 cek();
			 alert("dasd");                       
...
function notifikasi(title, msg, type, istimeout, isrefresh) {
	if (type=='success') {
		var warna = '#296191';
		var icon = 'fa-save';
		if(isrefresh==null || isrefresh==true) 
	...
	$.smallBox({
		title : title,
			...
			icon : "fa "+icon+" swing animated",
			//iconSmall : "fa "+ icon +" bounce animated", // dengan animasi bounce
			timeout : 3000 // lama alert ditampilkan
		});
	wait(3000);
	if(isrefresh==null || isrefresh==true) 
		refresh();
	...
	function cek () {
		$.get('berkas/berkasmasuk/cek', function(respon) {
			$("#notif_masuk").html(respon);
			if (respon>0) {
				$("#tampil-berkas").effect("shake", 
          			{times:8, distance:8}, 1000 );
					$('<audio id="notifberkas"><source src="themes/smartadmin/sound/notify.mp3" type="audio/mpeg"></audio>').appendTo('body');
					$('#notifberkas')[0].play();
			} else if(respon=='NotAuthorized') {
				AppLock();
			}
		});
	}
