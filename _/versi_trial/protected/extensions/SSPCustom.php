 ...
 *
 * Source : https://github.com/emran/ssp/
...
class SSPCustom extends CApplicationComponent {
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
        ...
        if ( isset($request['order']) && count($request['order']) ) {
            $orderBy = array();
            $dtColumns = SSPCustom::pluck( $columns, 'dt' );
        ...
        if ( isset($request['search']) && $request['search']['value'] != '' ) {
            ...
            for ( $i=0, $ien=count($request['columns']) ; $i<$ien ; $i++ ) {
                ...
                if ( $requestColumn['searchable'] == 'true' ) {
                    $binding = SSPCustom::bind( $bindings, '%'.$str.'%', PDO::PARAM_STR );
    ...
    {
        $bindings = array();
        $sql_details = SSPCustom::getConnection();
        $db = SSPCustom::sql_connect( $sql_details );

        // Build the SQL query string from the request
        $limit = SSPCustom::limit( $request, $columns );
        $order = SSPCustom::order( $request, $columns, $joinQuery );
        $where = SSPCustom::filter( $request, $columns, $bindings, $joinQuery );
        ...
        if($joinQuery){
            $col = SSPCustom::pluck($columns, 'db', $joinQuery);
            ...
            $query =  "SELECT SQL_CALC_FOUND_ROWS `".implode("`, `", SSPCustom::pluck($columns, 'db'))."`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = SSPCustom::sql_exec( $db, $bindings,$query);

        // Data set length after filtering
        $resFilterLength = SSPCustom::sql_exec( $db,
            "SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];

        // Total data set length
        $resTotalLength = SSPCustom::sql_exec( $db,
        ...
        return array(
            ...
            "data"            => SSPCustom::data_output( $columns, $data, $joinQuery )
        );
    }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        try {
            ...
            SSPCustom::fatal( "An SQL error occurred: ".$e->getMessage() );
        }
    ...
    static function getConnection()
    {
        ...
        // var_dump($config['db']);Yii::app()->end();
        $confDB = @is_object($config['db']->connectionString) ? $config['db']['connectionString'] : $config['db']['connectionString'];

        $db = explode('dbname=',$confDB);
        $dbname = $db[1];
        $exph = explode('mysql:host=',$confDB);
        $exph2 = explode(';dbname=',$exph[1]);
        $host = $exph2[0];
