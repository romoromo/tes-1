...
<section id="widget-grid" class="">
	...
	<script type="text/javascript">
		...
		function cari() {
			...
			var TBLPENDATAAN_TGLPAJAK = $('#TBLPENDATAAN_TGLPAJAK').val().split('-')[0];
			if (TBLDAFTAR_NOPOK=='') {
				notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
...
function getDataReklame(CALLBACK_FUNCTION) {
	...
	$.ajax({
		...
		success : function (respon) {
			...
			$('#muka_sisi').text(respon.muka_sisi);
			...
			$('#RREKJNSREKTARIF_ID').select2('val',respon.RREKJENISTARIF_ID);
			$('#RREKJNSPOSISI_ID').select2('val',respon.RREKJNSPOSISI_ID);
			$('#REFSUDUTPANDANG_SKOR').select2('val',respon.REFSUDUTPANDANG_SKOR);
