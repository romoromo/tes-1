						...
						<fieldset>
									...
									<label class="select">
										<select class="select2" id="TBLREKENING_KODE" name="TBLREKENING_KODE">
											...
											<?php foreach ($data['rek'] as $list): ?>
												<option selected="" value="<?php echo $list['TBLREKENING_KODE'] ?>">[<?php echo $list['TBLREKENING_KODE_90'] ?>] <?php echo $list['TBLREKENING_NAMAREKENING_90'] ?></option>
											<?php endforeach ?>
										</select>
									</label>
								</section>
							...
							<div class="row">
								...
								<div class="row">
									...
									<section class="col col-md-4">
										<label class="select">
											<select class="select2" id="ISBERIZIN" name="ISBERIZIN">
												<option value="">-- Semua --</option>
													<option value="T">Sudah Berizin</option>
													<option value="F">Belum Berizin</option>
											</select>
										</label>
									</section>
								</div>
								<div class="row">
									...
									<section class="col col-md-4">
										<label class="select">
											<select class="select2" id="ISBAYAR" name="ISBAYAR">
												<option value="">-- Semua --</option>
								...
								<footer>
									...
									<button type="button" onclick="cetakWord2021('normal')" class="btn btn-sm btn-success">
										<i class="fa fa-print"></i>&nbsp;Cetak Kartu Data 2021
									</button>
									<!-- <?php //if (Tblrole::model()->isRole('SUPERADMIN')): ?> -->
									<button type="button" onclick="cetakWord2021('bpk')" class="btn btn-sm btn-success">
										<i class="fa fa-print"></i>&nbsp;Cetak BPK 2022
									</button>
									<!-- <?php //endif ?> -->
...
<script type="text/javascript">
	...
	function getRekeningSub(rekeningkode) {
		...
		.done(function(respon) {
			setComboListCustom('TBLREKENING_KODESUB', 'Sub Rekening', respon);
		})
		.fail(function() {
			console.log("error");
		})
	...
	function cetakWord(jenis) {
		...
		window.TBLDAFTREKLAME_ISJNSPENETAPAN= $("#penetapan").val();
		window.TBLDAFTREKLAME_JNSREKLAME= $("#TBLDAFTREKLAME_JNSREKLAME").val();
		window.ISBERIZIN= $("#ISBERIZIN").val();
		window.ISBAYAR= $("#ISBAYAR").val();
		var URL_APLIKASI = "<?php echo Yii::app()->getBaseUrl(1); ?>";
		window.open(URL_APLIKASI+"/<?= $this->MODULE_URL ?>/cetakWord?jenis="+jenis+"&TBLPENDATAAN_THNPAJAK="+window.TBLPENDATAAN_THNPAJAK+"&TBLPENDATAAN_BLNPAJAK="+window.TBLPENDATAAN_BLNPAJAK+"&TBLPENDATAAN_TGLPAJAK="+window.TBLPENDATAAN_TGLPAJAK+"&TBLREKENING_KODESUB="+window.TBLREKENING_KODESUB+"&TBLDAFTREKLAME_THNSPTPD="+window.TBLDAFTREKLAME_THNSPTPD+"&TBLDAFTREKLAME_BLNSPTPD="+window.TBLDAFTREKLAME_BLNSPTPD+"&TBLDAFTREKLAME_TGLSPTPD="+window.TBLDAFTREKLAME_TGLSPTPD+"&TBLKECAMATAN_ID="+window.TBLKECAMATAN_ID+"&REFJALAN_ID="+window.REFJALAN_ID+"&TBLDAFTREKLAME_ISJNSPENETAPAN="+window.TBLDAFTREKLAME_ISJNSPENETAPAN+"&TBLDAFTREKLAME_JNSREKLAME="+window.TBLDAFTREKLAME_JNSREKLAME
			+"&ISBAYAR="+window.ISBAYAR
			+"&ISBERIZIN="+window.ISBERIZIN);

	}

	function cetakWord2021(jenis) {
		//window.rekening = $("#TBLREKENING_KODE").val
		window.TBLPENDATAAN_THNPAJAK = $('#TBLPENDATAAN_THNPAJAK').val();
		window.TBLPENDATAAN_BLNPAJAK = $('#TBLPENDATAAN_BLNPAJAK').val();
		window.TBLPENDATAAN_TGLPAJAK= $("#TBLPENDATAAN_TGLPAJAK").val();
		...
		window.ISBAYAR= $("#ISBAYAR").val();
		var URL_APLIKASI = "<?php echo Yii::app()->getBaseUrl(1); ?>";
		window.open(URL_APLIKASI+"/<?= $this->MODULE_URL ?>/cetakWord2021?jenis="+jenis+"&TBLPENDATAAN_THNPAJAK="+window.TBLPENDATAAN_THNPAJAK+"&TBLPENDATAAN_BLNPAJAK="+window.TBLPENDATAAN_BLNPAJAK+"&TBLPENDATAAN_TGLPAJAK="+window.TBLPENDATAAN_TGLPAJAK+"&TBLREKENING_KODESUB="+window.TBLREKENING_KODESUB+"&TBLDAFTREKLAME_THNSPTPD="+window.TBLDAFTREKLAME_THNSPTPD+"&TBLDAFTREKLAME_BLNSPTPD="+window.TBLDAFTREKLAME_BLNSPTPD+"&TBLDAFTREKLAME_TGLSPTPD="+window.TBLDAFTREKLAME_TGLSPTPD+"&TBLKECAMATAN_ID="+window.TBLKECAMATAN_ID+"&REFJALAN_ID="+window.REFJALAN_ID+"&TBLDAFTREKLAME_ISJNSPENETAPAN="+window.TBLDAFTREKLAME_ISJNSPENETAPAN+"&TBLDAFTREKLAME_JNSREKLAME="+window.TBLDAFTREKLAME_JNSREKLAME
			+"&ISBAYAR="+window.ISBAYAR
			+"&ISBERIZIN="+window.ISBERIZIN);
	...
	function setComboListCustom(elemen, kata, respon) {
		...
		if (data.length>0) {
			...
			$.each(data, function(i, item) {
				...
				exp = tulisan.split('] ');
				tulisan = exp.hasOwnProperty(1) ? exp[1] : tulisan;
