										...
										<label class="select">
											<select class="select2" id="TREKENINGSUB_KODE" name="TREKENINGSUB_KODE">
												...
												<?php foreach ($data['rek'] as $list): ?>
													<option rekjenis="<?php echo $list['TREKENING_KODE'] ?>" value="<?php echo $list['TREKENINGSUB_KODE'] ?>" tarif="<?php echo $list['TARIF'] ?>" tarif1="<?php echo $list['TARIF1'] ?>" satuan="<?php echo $list['SATUAN'] ?>">[<?php echo $list['TREKENINGSUB_KODE'] ?>] <?php echo $list['TREKENING_NAMA'] ?></option>
												<?php endforeach ?>
											</select>
										</label>
									</section>
				...
				$.ajax({
					...
					success: function(respon) {
						if (respon.data=='proses selanjutnya' || respon.data=='ada') {
							...
							$('#TREKENINGSUB_KODE').select2('val', `4.1.1.<?= $this->PAJAK_AYAT ?>.${respon.TBLDAFTREKLAME_REKJENIS}.${respon.TBLDAFTREKLAME_REKSUBJENIS}`);
...
function cetakPengambilan() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/<?= $this->MODULE_URL ?>/cetakPengambilan?');
}

function cetakJabong() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/<?= $this->MODULE_URL ?>/cetakJabong?');
}

function cetakIzinReklame() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/<?= $this->MODULE_URL ?>/cetakIzinReklame?');
}
...
function simpanPendataan() {
	...
	$.ajax({
		url: '<?= $this->MODULE_URL ?>/simpanPendataan',
		type: 'POST',
		dataType: 'json',
...
function getDataReklame(CALLBACK_FUNCTION) {
	...
	$.ajax({
		url: '<?= $this->MODULE_URL ?>/GetDataReklame',
		type: 'POST',
		dataType: 'json',
		data: {
			 nodaftar: nodaftar
