...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Cetak Rekap WP</h4>
		</div>
	</div>
</div>
...
<section id="widget-grid" class="">
	...
	<div class="row">
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget jarviswidget-color-teal" id="wid-id-cetak-rekap-wp" 
...
<div id="widget-grid-cetak-rekapwp" class="row" style="display: none;">

	<!-- NEW WIDGET START -->
	<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">

		<!-- Widget ID (each widget will need unique ID)-->
		<div class="jarviswidget jarviswidget-color-orange" id="wid-id-cetak-rekap-wp" data-widget-sortable="false" data-widget-deletebutton="false" data-widget-editbutton="false" data-widget-fullscreenbutton="false">
			<!-- widget options:
			usage: <div class="jarviswidget" id="wid-id-0" data-widget-editbutton="false">
			...
			<div>
				...
				<div class="widget-body no-padding">
					<p class="alert alert-info"> 
						<button class="btn btn-default" onclick="cetak()">
							<i class="fa fa-print"></i> Cetak Daftar
						</button>	
						<button class="btn btn-default" onclick="cetak_status()">
							<i class="fa fa-file-text-o"></i> Cetak Daftar Status
						</button>					
					</p>

					<div class="">
					
						<table width="100%" class="table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" id="kontrol_table">
							<thead>
								<tr>
								    <th>
							...
							<tbody>
								<tr>
									...
									<td>1.</td>
								    <td>34.02.120.0000622</td>
								    <td>1</td>
								    <td>410</td>
								    <td>Tegalrejo</td>
								    <td>Tegalrejo</td>
								    <td>PT DJARUM</td>
								    <td>Jl A Yani 28 Pajunan Kudus</td>
								</tr>
							</tbody>
						</table>
...
<script type="text/javascript">
	...
	function generateAutocompleteWP() {
		$('#TBLDAFTAR_NOPOK').autocomplete({
			serviceUrl: 'pendataan/cetak_rekapwp/autocompletewp',
	...
	function cari() {
			...
			$("#widget-grid-cetak-rekapwp").slideDown(600);
			$("#kontrol_table").html(LOADER).fadeIn(400);
			$.ajax({
				url: 'pendataan/cetak_rekapwp/GetData',
				type: 'POST',
				data:  {
			...
			$('#widget-grid-cetak-rekapwp').show('fast');
		// }
	}
	...
	function cetak() {
		...
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/cetak_rekapwp/cetak?' + param);
	}
	...
	function cetak_status() {
		...
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/cetak_rekapwp/cetak_status?' + param);
	}
	...
	function getKelurahan(val) {
		$.ajax({
			url: 'pendataan/cetak_rekapwp/getKelurahan',
			type: 'POST',
			dataType: 'html',
			data: {value: val},
			success: function(respon) {
	...
	function getKelurahanedit(val) {
		$.ajax({
			url: 'pendataan/cetak_rekapwp/getKelurahanedit',
			type: 'POST',
			dataType: 'html',
			data: {value: val},
			success: function(respon) {
	...
	function getKelurahanedit2(val) {
		$.ajax({
			url: 'pendataan/cetak_rekapwp/getKelurahanedit2',
			type: 'POST',
			dataType: 'html',
			data: {value: val},
			success: function(respon) {
	...
	function simpan () {
		$.ajax({
			url: 'pendataan/cetak_rekapwp/Simpan',
			type: 'post',
			dataType: "json",
