...
class Cetak_suratteguran_multibulanController extends Controller
{
	...
	public function actionCetakSemua()
	{
		...
		$select = ''.$namaTBL.'.*, TBLDAFTAR.*,LISTAGG (BULAN_NAMA, ", " ) WITHIN GROUP (ORDER BY TBLPENDATAAN_BLNPAJAK) AS BULAN_AGG';
		$from = ''.$namaTBL.'';
		...
		$otherquery['andwhere'] = '
	        NVL('.$namaTBL . '.TBLPENDATAAN_TGLPAJAK, 0)= 0
		...
		$otherquery = array(
			...
			,'join'=> array('BULAN', ''.$namaTBL.'.TBLPENDATAAN_BLNPAJAK = BULAN.BULAN_ID')
			,'group'=> ''.$namaTBL.'.TBLDAFTAR_NOPOK'
