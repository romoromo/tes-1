	...
	<script type="text/javascript">
		...
		function generateAutocompleteWP() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'pendataan/reklamelama_tetap/autocompletewp',
		...
		function cekPernahDaftar() {
			...
			if (TBLDAFTAR_NOPOK=='') {
				...
				$.ajax({
					url: 'pendataan/reklamelama_tetap/CekPernahDaftar',
					type: 'POST',
					dataType: 'json',
					data: {
		...
		function cari() {
			...
			if (TBLDAFTAR_NOPOK=='') {
				...
				$.ajax({
					url: 'pendataan/reklamelama_tetap/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
...
function cetakPengambilan() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklamelama_tetap/cetakPengambilan?');
}

function cetakJabong() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklamelama_tetap/cetakJabong?');
}

function cetakIzinReklame() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklamelama_tetap/cetakIzinReklame?');
}
...
function simpanPendataan() {
	...
	$.ajax({
		url: 'pendataan/reklamelama_tetap/simpanPendataan',
		type: 'POST',
		dataType: 'json',
