...
class Entryjabong_tetapController extends Controller
{
	var $KODE_GOL = 2;
	var $PAJAK_AYAT = 4;
	...
	public function actionIndex()
	{
		...
		$data['rek'] = Yii::app()->db->createCommand("
			...
			, URS || '.' || PEM || '.' || ORG || '.' || PRG || '.' || KGT || '.' || DIN || '.' || BID || '.' || PEND || '.' || PAD || '.' || PJK || '.' || AYT || '.' || JEN || '.' || SUB AS TREKENING_KODEFULLto
	...
	public function actionCetakJabongWord()
	{
		...
		$namatpl = $path_tpl . 'jabong_tetap.docx';
		...
		$select = "TBLDAFTAR.*, TBLDAFTREKLAME.*,RREKRNDBARU.NMREK";
		...
		$dt['wpnama'] = $data['cetak']['TBLDAFTAR_BADANUSAHANAMA'];
		...
		$dt['masaawal'] = $data['cetak']['TBLDAFTREKLAME_TGLMULAIREKLAME'] .'/'. sprintf("%02d", $data['cetak']['TBLDAFTREKLAME_BLNMULAIREKLAME']) .'/20'. $data['cetak']['TBLDAFTREKLAME_THNMULAIREKLAME'];
		$dt['masaakhir'] = $data['cetak']['TBLDAFTREKLAME_TGLAKHIRREKLAME'] .'/'. sprintf("%02d", $data['cetak']['TBLDAFTREKLAME_BLNAKHIRREKLAME']) .'/20'. $data['cetak']['TBLDAFTREKLAME_THNAKHIRREKLAME'];
		...
		$dt['pjg'] = $data['cetak']['TBLDAFTREKLAME_PANJANG'];
		$dt['lbr'] = $data['cetak']['TBLDAFTREKLAME_LEBAR'];
		...
		$dt['jabatan'] = $datax['jab1']['TBLPEJABAT_URAIAN'];
		$dt['nama'] = $datax['jab1']['TBLPEJABAT_NAMA'];
		$dt['nip'] = $datax['jab1']['TBLPEJABAT_NIP'];
		...
		$namafileDL = "JABONG-TETAP-".$npwpd.".docx";
	...
	public function actionGetNoJabong()
	{
		...
		AND TBLDAFTREKLAME_JNSREKLAME = 'T'
		AND NVL (TBLDAFTREKLAME_NOJABONG, 0) <> 0
		AND NVL (TBLDAFTREKLAME_NOJABONG, 0) <> 150000
		AND NVL (TBLDAFTREKLAME_NOJABONG, 0) <> 12345
		AND NVL (TBLDAFTREKLAME_NOJABONG, 0) <> 4041
		AND NVL (TBLDAFTREKLAME_NOJABONG, 0) <> 3347
		ORDER BY
	...
	public function actiongetdata()
	{
		...
		$data = array();
		$cek = $this->cekPernahDaftar(substr($tahun, -2), $tblpendataan_pajakke, $NOPOK);
		$proses = $this->cekProses(substr($tahun, -2), $tblpendataan_pajakke, $NOPOK);
			...
			TBLDAFTREKLAME_THNJABONG,
			TBLDAFTREKLAME_BLNJABONG,
			TBLDAFTREKLAME_TGLJABONG,
			TBLDAFTREKLAME_NOJABONG,
			REFJABONGREKLAME_IDXKESULITAN,
			TBLDAFTREKLAME_JUMLAHJABONG,
			...
			$data['TBLDAFTREKLAME_LUAS'] =$model['TBLDAFTREKLAME_LUAS'];
			...
			$data['TBLDAFTREKLAME_THNJABONG'] =$model['TBLDAFTREKLAME_THNJABONG'];
			$data['TBLDAFTREKLAME_BLNJABONG'] =$model['TBLDAFTREKLAME_BLNJABONG'];
			$data['TBLDAFTREKLAME_TGLJABONG'] =$model['TBLDAFTREKLAME_TGLJABONG'];
			$data['TBLDAFTREKLAME_NOJABONG'] =$model['TBLDAFTREKLAME_NOJABONG'];
			$data['REFJABONGREKLAME_IDXKESULITAN'] =$model['REFJABONGREKLAME_IDXKESULITAN'];
	...
	public function cekProses($tahun, $pajakke, $TBLDAFTAR_NOPOK)
	{
		$model = Yii::app()->db->createCommand('SELECT TBLPENDATAAN_THNPAJAK FROM TBLDAFTREKLAME WHERE TBLDAFTAR_NOPOK ='.$TBLDAFTAR_NOPOK.' AND TBLPENDATAAN_PAJAKKE = '.$pajakke.' AND TBLPENDATAAN_THNPAJAK = '.$tahun)->queryScalar();
		return $model;
	}

	public function cekPernahDaftar($tahun, $pajakke, $TBLDAFTAR_NOPOK)
	{
		$model = Yii::app()->db->createCommand('SELECT TBLDAFTREKLAME_NOJABONG FROM TBLDAFTREKLAME WHERE TBLDAFTAR_NOPOK ='.$TBLDAFTAR_NOPOK.' AND TBLPENDATAAN_PAJAKKE = '.$pajakke.' AND TBLPENDATAAN_THNPAJAK = '.$tahun)->queryScalar();
		return $model;
	}
	...
	public function actionSimpanJabong()
	{
		...
		$column = array(
			...
			'REFJABONGREKLAME_IDXKESULITAN' => $PARAM['KESULITAN'],
			'TBLDAFTREKLAME_JUMLAHJABONG' => LokalIndonesia::toAngka($PARAM['TBLDAFTREKLAME_JUMLAHJABONG']),
			'TBLDAFTREKLAME_LISTRIKJABONG' => LokalIndonesia::toAngka($PARAM['TBLDAFTREKLAME_LISTRIKJABONG']),
			'TBLDAFTREKLAME_RUPIAHJABONG' => LokalIndonesia::toAngka($PARAM['TBLDAFTREKLAME_RPJABONG']),
			'TBLDAFTREKLAME_HRGDASARJABONG' => LokalIndonesia::toAngka($PARAM['HARGA']),
		);

		$simpan = $command->update('TBLDAFTREKLAME', $column ,'TBLDAFTAR_NOPOK =:NOPOK AND TBLPENDATAAN_THNPAJAK =:THNPJK AND TBLPENDATAAN_PAJAKKE =:TBLPENDATAAN_PAJAKKE',array(':NOPOK' => $NOPOK,':THNPJK' => $THNPJK,':TBLPENDATAAN_PAJAKKE' => $TBLPENDATAAN_PAJAKKE));
