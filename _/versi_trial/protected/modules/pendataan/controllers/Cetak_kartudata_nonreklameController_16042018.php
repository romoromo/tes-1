...
class Cetak_kartudata_nonreklameController extends Controller
{
	...
	public function actionCetakWord()
	{
		...
		switch ( $rek ) {
			...
			$namaTBL = 'TBLDAFTREKLAME';
			$AYAT = '4';
			$AYAT_NAMA = 'PAJAK PENERANGAN JALAN';
			break;
		...
		if ($AYAT=='8') {
			...
			$namatpl = $path_tpl . 'kartudata-hotel.docx';
		} elseif($AYAT=='2'){
			$namatpl = $path_tpl . 'kartudata-restoran.docx';
		} elseif($AYAT=='3'){
			$namatpl = $path_tpl . 'kartudata-hiburan.docx';
		} elseif($AYAT=='7'){
			$namatpl = $path_tpl . 'kartudata-parkir.docx';
		} else {
