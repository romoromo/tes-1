...
class Reklame2016_semipermanenController extends Controller
{
	var $KODE_GOL = 2;
	var $PAJAK_AYAT = 4;
	var $PAJAK_REK = '4.1.1.4.0';
	var $MODULE_URL = 'pendataan/reklame2016_semipermanen';
	public function actionIndex()
	{
		...
		$data['rek'] = Yii::app()->db->createCommand("
			...
			AND JEN IN ('100', '101', '102', '106', '107', '108', '94', '95', '96')
			ORDER BY JEN
		")->queryAll();
	...
	public function cekPernahDaftar($thn, $bulan, $tanggal, $lokasi, $nopok)
	{
		$model = Yii::app()->db->createCommand('SELECT COUNT(TBLDAFTAR_NOPOK) AS JML FROM TBLDAFTREKLAME WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND NVL(TBLPENDATAAN_BLNPAJAK,0)='.$bulan.' AND NVL(TBLPENDATAAN_TGLPAJAK,0)='.$tanggal.' AND TBLPENDATAAN_PAJAKKE='.$lokasi.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	}
	...
	public function actiongetdata()
	{
		...
		$tanggal = intval(base64_decode($_POST['tanggal']));
		
		$data = array();
		$cek = $this->cekPernahDaftar(substr($tahun, -2), $bulan, $tanggal, $tblpendataan_pajakke, $NOPOK);
		if ($cek>0) {
			$data['data'] = 'ada';
			...
			$filter = array(
				...
				,'EQ__TBLDAFTREKLAME.TBLPENDATAAN_TGLPAJAK' => $tanggal
