...
class Reklame2016_tetap_integrasi_90Controller extends Controller
{
	var $KODE_GOL = 2;
	var $PAJAK_AYAT = 9;
	var $PAJAK_REK = '4.1.1.9.0';
	var $MODULE_URL = 'pendataan/reklame2016_tetap_integrasi_90';
	public function actionIndex()
	{
		...
		$data['list_jalan'] = Yii::app()->db->createCommand("SELECT RREKJALAN.*,REPLACE(NVL(RREKJALAN.RREKJALAN_NAMAJALAN, '-'), '  ', '') AS RREKJALAN_NAMAJALAN FROM RREKJALAN")->queryAll();
	...
	public function actiongetdata()
	{
		...
		$proses = $this->cekProses(substr($tahun, -2), $tblpendataan_pajakke, $NOPOK);
			...
			NVL(TBLDAFTREKLAME.TBLDAFTREKLAME_TGLIZIN, '') AS TBLDAFTREKLAME_TGLIZIN,
			NVL(TBLDAFTREKLAME.TBLDAFTREKLAME_BLNIZIN, '') AS TBLDAFTREKLAME_BLNIZIN,
			NVL(TBLDAFTREKLAME.TBLDAFTREKLAME_THNIZIN, '') AS TBLDAFTREKLAME_THNIZIN,
			...
			$otherquery = array(
				...
				,'order'=> 'TBLDAFTAR_NOPOK ASC'
				,'andwhere' => 'NVL(TBLDAFTREKLAME.TBLPENDATAAN_BLNPAJAK, 0)=0 AND NVL(TBLDAFTREKLAME.TBLPENDATAAN_TGLPAJAK, 0)=0'
			);
	...
	public function cekProses($thn, $lokasi, $nopok)
	{
		$model = Yii::app()->db->createCommand('SELECT TBLDAFTREKLAME_PAJAKSKPD AS JML FROM TBLDAFTREKLAME WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND NVL(TBLPENDATAAN_BLNPAJAK, 0)=0 AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLPENDATAAN_PAJAKKE='.$lokasi.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	...
	{
		$sql = 'SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTREKLAME WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND NVL(TBLPENDATAAN_BLNPAJAK, 0)=0 AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLPENDATAAN_PAJAKKE='.$lokasi.' AND TBLDAFTAR_NOPOK='.$nopok;
	...
	public function actionsimpanPendataan()
	{
		...
		$TBLDAFTREKLAME_PENEMPATAN = substr($TBLDAFTREKLAME_PENEMPATAN, 0, 20);
		...
		$LOKASI = DMOrcl::getRequest('LOKASI');
		// $NAMALOKASI = ( $jl = Yii::app()->db->createCommand('SELECT RREKJALAN_NAMAJALAN FROM RREKJALAN WHERE RREKJALAN_KODE='.$REFJALAN_ID)->queryScalar() ) ? $jl : '';
		...
		$arrayData = array(
			...
			'REFPANGGUNG_ID' => '',
			'TBLDAFTREKLAME_NAMAJALAN' => substr($LOKASI,0,20),
		...
		if ($this->cekPernahDaftar($TBLPENDATAAN_THNPAJAK, $TBLPENDATAAN_PAJAKKE, $TBLDAFTAR_NOPOK)>0) {
			# update
			# 2020-11-19 14:55 jgn update no daftar izin agar tidak hilang
			unset($arrayData['TBLDAFTREKLAME_NODAFTARIZIN']);
			$where = 'TBLPENDATAAN_THNPAJAK ='.$TBLPENDATAAN_THNPAJAK.' AND NVL(TBLPENDATAAN_BLNPAJAK, 0)=0 AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLPENDATAAN_PAJAKKE='.$TBLPENDATAAN_PAJAKKE.' AND TBLDAFTAR_NOPOK='.$TBLDAFTAR_NOPOK;
	...
	public function actionCekPernahDaftar()
	{
		...
		$sql = 'SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTREKLAME WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND NVL(TBLPENDATAAN_BLNPAJAK, 0)=0 AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLPENDATAAN_PAJAKKE='.$PAJAKKE.' AND TBLDAFTAR_NOPOK='.$NOPOKOK;
	...
	public function actionGetDataReklame()
	{
		...
		$RREKJNSREKTARIF_ID = Yii::app()->db->createCommand("
		SELECT 
		RREKJNSREKTARIF_ID 
		FROM 
		RREKJNSREKTARIF 
		...
		RREKJNSPOSISI 
		WHERE 
		RREKJNSPOSISI_NAMA LIKE '".$data->data->posisi_pjg_lbr."'")->queryScalar();

		$REFSUDUTPANDANG_SKOR = Yii::app()->db->createCommand("
		SELECT 
		RREKSUDUTPANDANG_SKOR 
		FROM 
		RREKSUDUTPANDANG 
		WHERE 
		...
		$dataarray = array(
			'status_bloksistem'=>$data->data->bloksistem_terakhir=='Pengambilan' ? 1 : 0,
