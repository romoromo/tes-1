...
class Cetak_suratteguran_multibulanController extends Controller
{
	...
	public function actionCetak() // function tidak dipakai
	{
	...
	public function actionCetakSemua()
	{
		...
		$TBLPENDATAAN_BLNPAJAK2 = isset($_REQUEST['TBLPENDATAAN_BLNPAJAK2']) && !empty($_REQUEST['TBLPENDATAAN_BLNPAJAK2']) ? $_REQUEST['TBLPENDATAAN_BLNPAJAK2'] : '';
		...
		LISTAGG (BULAN_NAMA, ', ') WITHIN GROUP (ORDER BY TBLPENDATAAN_BLNPAJAK) AS BULAN_AGG";
		...
		$otherquery = array(
			...
			,'join_bulan'=> array('BULAN', ''.$namaTBL.'.TBLPENDATAAN_BLNPAJAK = BULAN.BULAN_ID')
			,'andwhere'=> "NVL(".$namaTBL . ".TBLPENDATAAN_TGLPAJAK, 0)= 0
				AND NVL (".$namaTBL . "_REGSETOR, '0') = '0'
	        	AND TBLPENDATAAN_BLNPAJAK BETWEEN ".$TBLPENDATAAN_BLNPAJAK."
	        	AND ".$TBLPENDATAAN_BLNPAJAK2.""
			,'group'=> '
				TBLDAFTAR.TBLDAFTAR_NOPOK,
				TBLDAFTAR_BADANUSAHANAMA,
				TBLDAFTAR_BADANUSAHAALAMAT,
				...
				TBLKECAMATAN_IDBADANUSAHA,
				TBLKELURAHAN_IDBADANUSAHA'
			,'order'=> 'TBLDAFTAR.TBLDAFTAR_NOPOK'
		);
        ...
        if ($rek == '1' || $rek == '3') {
        	$namatpl = $path_tpl . 'surat_teguran2 hotel hiburan.docx';
        } else {
        	$namatpl = $path_tpl . 'surat_teguran2 resto parkir.docx';
        }
        ...
        foreach ($data['list_teguran'] as $kolom) :
            ...
            $dt['bulan_agg'] = $kolom['BULAN_AGG'];
            $dt['thnpajak'] = $TBLPENDATAAN_THNPAJAK;
            ...
            if ($rek=='3') {
            	$dt['keterlambatanstimulus'] = '15';
            } else if ($rek=='1') {
            	$dt['keterlambatanstimulus'] = '10';
            }
        ...
        $header['jabatan'] = $data['jab1']['TBLPEJABAT_URAIAN'];
        $header['nama'] = $data['jab1']['TBLPEJABAT_NAMA'];
        $header['nip'] = $data['jab1']['TBLPEJABAT_NIP'];
        ...
        $namafileDL = "Surat-Teguran-Stimulus-No:".$nomor_surat.".docx";
	...
	public function actioncari() {
		...
		NVL(".$namaTBL."_PAJAK,0) AS ".$namaTBL."_PAJAK,
		".$namaTBL."_REGSETOR,
		".$namaTBL."_RUPIAHSETOR,
		LISTAGG (BULAN_NAMA, ', ') WITHIN GROUP (ORDER BY TBLPENDATAAN_BLNPAJAK) AS BULAN_AGG
		";
		$from = ''.$namaTBL.'';
		...
		$otherquery = array(
			...
			,'group'=> '
				...
				TBLDAFTAR_BADANUSAHANAMA,
				TBLDAFTAR_PEMILIKNAMA,
				NVL('.$namaTBL.'_PAJAK,0),
				'.$namaTBL.'_REGSETOR,
				'.$namaTBL.'_RUPIAHSETOR'
			,'order'=> ''.$namaTBL.'.TBLDAFTAR_NOPOK'
		);
	...
	public function actionCetakSuratTeguran()
    {
        ...
        foreach ($data['filter'] as $kodefikasi) {
            ...
            if (is_array($kodefikasi)) {
                ...
                LISTAGG (BULAN_NAMA, ', ') WITHIN GROUP (
				ORDER BY
				TBLPENDATAAN_BLNPAJAK
				) AS BULAN_AGG

                FROM ".$namaTBL."
                JOIN TBLDAFTAR ON ".$namaTBL.".TBLDAFTAR_NOPOK = TBLDAFTAR.TBLDAFTAR_NOPOK
                JOIN BULAN ON ".$namaTBL.".TBLPENDATAAN_BLNPAJAK = BULAN.BULAN_ID
                WHERE
                TBLDAFTAR.TBLDAFTAR_NOPOK = ".$nopok."
                AND TBLPENDATAAN_THNPAJAK = ".substr($TBLPENDATAAN_THNPAJAK, -2)."
                AND TBLPENDATAAN_BLNPAJAK BETWEEN ".$TBLPENDATAAN_BLNPAJAK." AND ".$TBLPENDATAAN_BLNPAJAK2."
