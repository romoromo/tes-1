...
class PajakparkirController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		$tglentry ='20'.$model['' . $this->NAMATABEL . '_THNENTRI'].'-'.$model['' . $this->NAMATABEL . '_BLNENTRI'].'-'.$model['' . $this->NAMATABEL . '_TGLENTRI'];
		...
		if ($cek>0) {
			$data['BULAN_DENDA'] = $bulan_denda = LokalIndonesia::getBulanDenda(date('Y-m-d', strtotime($tglbatas)), date('Y-m-d', strtotime($tglentry)));
			$data['PERSEN_DENDA'] = $bulan_denda*2;
		} else {
