...
class Cetak_daftar_izireklameController extends Controller
{
	...
	public function actionCetakword()
	{
		...
		$namatpl = $path_tpl . 'surat_pemberitahuan_izin_reklame-hanacaraka.docx';
		...
		$otbs->NoErr = 0;
			...
			ORDER BY
			    ...
			    ';
			   if ($_COOKIE['PHPSESSID'] == 'x8vpevv02qreqlpq4ak3je31ot3') :
			...
			$GLOBALS['no_surat'] = sprintf('%06d',$_REQUEST['nomor_surat']) ;
			...
			foreach ($data['cetak'] as $kolom) :
				...
				$MASA = $kolom['TBLPENDATAAN_THNPAJAK']+2000 . '  Bulan ' . LokalIndonesia::getBulan($kolom['TBLPENDATAAN_BLNPAJAK']) . '  Tanggal ' . ($kolom['TBLPENDATAAN_TGLPAJAK']);
			...
			$namafileDL = "Surat Pemberitahuan Izin Reklame Tgl Batas ".$BATAS.".docx";
