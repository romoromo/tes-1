...
class Salin_airtanahController extends Controller
{
	...
	public function actionSalinData()
    {
        ...
        $plus_tahun = 0;
        $plus_bulan = 1;
        $bulan = $TBLPENDATAAN_BLNPAJAK_SALIN + $plus_bulan;
        if ($TBLPENDATAAN_BLNPAJAK_SALIN>=12) {
        	$plus_tahun = 1;
        	$bulan = 1;
        }
        $TGL_AKHIR_MASA = date("t", strtotime("{$TBLPENDATAAN_THNPAJAK_SALIN}-{$TBLPENDATAAN_BLNPAJAK_SALIN}-01"));
        ...
        foreach ($PajakList as $list) {
			...
			$TOTALVOLUME = isset($pch[20]) ? $pch[20] : 0;
			$NILAIAIR1 = isset($pch[21]) ? $pch[21] : 0;
			$PAJAK = isset($pch[22]) ? $pch[22] : 0;
            ...
            $column = array(
				...
				// ,''.$namaTBL.'_TGLAKHIRJUAL' => '0' // tgl akhir sesuai bulan pilih
				,''.$namaTBL.'_TGLAKHIRJUAL' => $TGL_AKHIR_MASA // tgl akhir sesuai bulan pilih
				,''.$namaTBL.'_OMSETPAJAK' => '0'
				,''.$namaTBL.'_PERSENTARIF' => $PERSENTARIF
				,''.$namaTBL.'_THNSPTPD' => $TBLPENDATAAN_THNPAJAK_SALIN + $plus_tahun
				,''.$namaTBL.'_BLNSPTPD' => $bulan
				,''.$namaTBL.'_TGLSPTPD' => $tglspt
				,''.$namaTBL.'_THNBATASSPTPD' => $TBLPENDATAAN_THNPAJAK_SALIN + $plus_tahun
				,''.$namaTBL.'_BLNBATASSPTPD' => $bulan
				,''.$namaTBL.'_TGLBATASSPTPD' => $TGLBATASSPTPD
				,''.$namaTBL.'_THNTERIMA' => $TBLPENDATAAN_THNPAJAK_SALIN + $plus_tahun
				,''.$namaTBL.'_BLNTERIMA' => $bulan
				,''.$namaTBL.'_TGLTERIMA' => $tglspt
				,''.$namaTBL.'_THNENTRI' => $TBLPENDATAAN_THNPAJAK_SALIN + $plus_tahun
				,''.$namaTBL.'_BLNENTRI' => $bulan
				,''.$namaTBL.'_TGLENTRI' => $tglspt
				,''.$namaTBL.'_TOTALVOLUME' => $TOTALVOLUME
				,''.$namaTBL.'_NILAIAIR1' => $NILAIAIR1
				,''.$namaTBL.'_PAJAK' => $PAJAK
				,''.$namaTBL.'_STATUSDATA' => '1'
			);
