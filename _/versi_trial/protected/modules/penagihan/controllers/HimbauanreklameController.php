...
class HimbauanreklameController extends Controller
{
	var $KODE_GOL = 2;
	var $PAJAK_AYAT = 4;
	var $PAJAK_REK = '4.1.1.4.0';
	var $MODULE_URL = 'penagihan/Himbauanreklame';
	...
	public function actionCari()
	{
		...
		SELECT
			...
			NVL(TBLDAFTREKLAME.TBLPENDATAAN_BLNPAJAK,0) TBLPENDATAAN_BLNPAJAK,
...
public function actionCetakWordReklame()
{
	...
	$namatpl = $path_tpl . 'Himbauanreklame.docx';
	...
	$otbs->NoErr = 0;
		...
		foreach ($data['filter'] as $kodefikasi) {
			...
			if (is_array($kodefikasi)) {
				...
				SELECT
					...
					FROM TBLDAFTREKLAME
					JOIN TBLDAFTAR ON TBLDAFTREKLAME.TBLDAFTAR_NOPOK = TBLDAFTAR.TBLDAFTAR_NOPOK
					WHERE
					TBLDAFTREKLAME.TBLDAFTAR_NOPOK = ".$nopok."
					AND TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK = ". $thpajak. "
					AND TBLDAFTREKLAME.TBLPENDATAAN_BLNPAJAK = ". $blpajak. "
					AND TBLDAFTREKLAME.TBLPENDATAAN_TGLPAJAK = " . $tglpajak . "
					AND TBLDAFTREKLAME.TBLPENDATAAN_PAJAKKE = " . $ke . "
					ORDER BY
					TBLDAFTAR_NOPOK,
					TBLDAFTREKLAME_THNSPTPD,
					TBLDAFTREKLAME_BLNSPTPD,
		...
		// debug
		// echo CJSON::encode($header);Yii::app()->end();
		...
		$namafileDL = "Surat Imbauan Reklame {$tanggalsurat}.docx";
	...
	{	
		...
		$namatpl = $path_tpl . 'DAFTAR SURAT IMBAUAN SKPD REKLAME (A4).docx';
		...
		$otbs->NoErr = 0;
			...
			$otbs->MergeField('header', $header);
			// $otbs->PlugIn(OPENTBS_DEBUG_XML_CURRENT); // debug the template
			$namafileDL = "Daftar Imbauan Reklame {$tanggalsurat}.docx";
	...
	public function actionSimpan()
	{
		...
		if (!empty($data['filter'])) {
			...
			echo "<h3>Data yg dikirim tidak benar, ulangi proses cetak Himbauan Pajak Reklame</h3>";
			Yii::app()->end();
		}
		...
		foreach ($data['filter'] as $kodefikasi) {
			...
			if (is_array($kodefikasi)) {
				if (!isset($kodefikasi[0])) {
					echo "<h3>Data yg dikirim tidak benar, ulangi proses cetak himbauan reklame</h3>";
					Yii::app()->end();
				}
				$nopok = $kodefikasi[0];
				$thpajak = $kodefikasi[1];
				...
				SELECT
					...
					TBLDAFTREKLAME.TBLDAFTAR_NOPOK = " . $nopok . "
					AND TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK = " . $thpajak . "
					AND NVL(TBLDAFTREKLAME.TBLPENDATAAN_BLNPAJAK,0) = " . $blpajak . "
					AND NVL(TBLDAFTREKLAME.TBLPENDATAAN_TGLPAJAK,0) = " . $tglpajak . "
					AND TBLDAFTREKLAME.TBLPENDATAAN_PAJAKKE = " . $ke . "
		...
		$data['simpan'] = Yii::app()->db->createCommand($query)->queryAll();
		...
		foreach ($data['simpan'] as $kolom) :
			...
			try {
				$himbauanid = $this->Cekexisthimbauan($kolom['TBLPENDATAAN_THNPAJAK'], $kolom['TBLPENDATAAN_BLNPAJAK'], $kolom['TBLPENDATAAN_TGLPAJAK'], $kolom['TBLDAFTAR_NOPOK'], $kolom['TBLPENDATAAN_PAJAKKE'], $kolom['TBLDAFTREKLAME_REKAYAT'],$tglteguran);

				// echo $himbauanid;Yii::app()->end();
				if (empty($himbauanid)) {

					$inserthistori = "INSERT INTO TBLHIMBAUAN (THP, BLP, HRP, KE, NOPOK, NOHIMBAUAN, TGLHIMBAUAN, NAMA_WP, ALAMAT, NPWPD, JENIS, STATUS_DATA, TBLPENGGUNA_ID) VALUES (" . $kolom['TBLPENDATAAN_THNPAJAK'] . "," . $kolom['TBLPENDATAAN_BLNPAJAK'] . "," . $kolom['TBLPENDATAAN_TGLPAJAK'] . "," . $kolom['TBLPENDATAAN_PAJAKKE'] . "," . $kolom['TBLDAFTAR_NOPOK'] . ",'" . $no_surat . "',TO_DATE('" . $tglteguran . "', 'YYYY-MM-DD HH24:MI:SS'),TRIM('" . $kolom['TBLDAFTAR_BADANUSAHANAMA'] . "'),TRIM('" . $kolom['TBLDAFTAR_BADANUSAHAALAMAT'] . "'),'" . $kolom['TBLDAFTAR_GOLONGAN'] . "." . (sprintf('%07d', $kolom['TBLDAFTAR_NOPOK'])) . "." . (sprintf('%02d', $kolom['TBLKECAMATAN_ID'])) . "." . (sprintf('%02d', $kolom['TBLKELURAHAN_ID'])) . "'," . $kolom['TBLDAFTREKLAME_REKAYAT'] . ",'SKPD',$tblpenggunaid)";
	...
	public function Cekexisthimbauan($thp,$blp,$hrp,$nopok,$ke,$jenis,$tgl)
	{
		$now = date('Y-m-d');
		$himbauanid = Yii::app()->db->createCommand("SELECT TBLHIMBAUAN_ID FROM TBLHIMBAUAN WHERE THP = {$thp} AND BLP = {$blp} AND HRP = {$hrp} AND NOPOK = {$nopok} AND KE = {$ke} AND TGLHIMBAUAN = TO_DATE('{$tgl}', 'YYYY-MM-DD HH24:MI:SS') AND STATUS_DATA = 'SKPD' AND JENIS = '{$jenis}' ")->queryScalar();
		return $himbauanid;
	}
