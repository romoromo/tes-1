...
class Teguran_skpda_2021Controller extends Controller
{
	...
	public function actionCari()
	{
		...
		(
				 ...
				 TO_DATE (
					CONCAT (
						...
						CONCAT (
							'/',
							CONCAT (
								TBLANGSURAN_TGLBATASKETETAPAN,
								CONCAT (
									'/',
									TBLANGSURAN_THNBATASKETETAPAN
								)
							)
						)
					), 'MM/DD/YY'
			...
			SUM (CASE
				WHEN TBLANGSURAN.TBLANGSURAN_SETORAN IS NOT NULL AND
				TO_DATE (
					CONCAT (
						TBLANGSURAN_BLNBATASKETETAPAN,
						CONCAT (
...
public function actioncetakword()
{
	...
	foreach ($datax['savehistori'] as $kolom) {
		$tglteguran = date('Y-m-d H:i:s', strtotime($tanggalsurat));
				...
				$inserthistori = "INSERT INTO TBLTEGURANTAPLAIN (THP, BLP, NOPOK, NOTEGURAN, TGLTEGURAN, NAMA_WP, ALAMAT, NPWPD, JENIS, STATUS_DATA) 
				VALUES (
