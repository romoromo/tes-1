...
<section id="widget-grid" class="">
		...
		<script type="text/javascript">
			...
			function generateAutocompleteWP() {
				$('#TBLDAFTAR_NOPOK').autocomplete({
					serviceUrl: 'penagihan/Teguranreklame_2021/autocompletewp',
			...
			function cari() {
				...
				$.ajax({
					url: 'penagihan/Teguranreklame_2021/cari',
					type: 'POST',
					data: {
    ...
    function cetakDaftar () {
    	...
    	var q = daftaridPajak;
    	window.open('<?= Yii::app()->getBaseUrl(1); ?>/penagihan/Teguranreklame_2021/cetakdaftar/?data=' + q +"&TBLPENDATAAN_THNPAJAK="+$('#TBLPENDATAAN_THNPAJAK').val()+"&TBLDAFTREKLAME_JNSREKLAME="+$('#TBLDAFTREKLAME_JNSREKLAME').select2('val')+"&TBLDAFTAR_NOPOK="+$('#TBLDAFTAR_NOPOK').val()+"&no_surat="+$('#no_surat').val()+"&tglawal="+$('#tglawal').val()+"&tglakhir="+$('#tglakhir').val() );
