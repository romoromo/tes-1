...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entry Keringanan Pajak Reklame Tetap 2016</h4>
		</div>
	</div>
</div>
	...
	<script type="text/javascript">
		...
		function generateAutocompleteWP() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'penetapan/keringanan_reklame2016_tetap/autocompletewp',
		...
		function cekPernahDaftar() {
			...
			var TBLPENDATAAN_THNPAJAK = $('#TBLPENDATAAN_THNPAJAK').val();
				...
				$.ajax({
					url: 'penetapan/keringanan_reklame2016_tetap/CekPernahDaftar',
					type: 'POST',
					dataType: 'json',
					data: {
					...
					success : function (respon) {
						if (respon.status=='belum') {
							notifikasi('Maaf', 'Data SPT belum terdaftar', 'failed', 1,0);
						}
						else{
							cari();
						}
		...
		function cari() {
			...
			if (TBLDAFTAR_NOPOK=='') {
				...
				$.ajax({
					url: 'penetapan/keringanan_reklame2016_tetap/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
...
function cetakPengambilan() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/keringanan_reklame2016_tetap/cetakPengambilan?');
}

function cetakJabong() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/keringanan_reklame2016_tetap/cetakJabong?');
}

function cetakIzinReklame() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/keringanan_reklame2016_tetap/cetakIzinReklame?');
}
...
function simpanPendataan() {
	...
	$.ajax({
		url: 'penetapan/keringanan_reklame2016_tetap/simpanPendataan',
		type: 'POST',
		dataType: 'json',
...
function updatenodaftizin() {
	...
	$.ajax({
		url: 'penetapan/keringanan_reklame2016_tetap/updatenomordaftizin',
		type: 'POST',
		dataType: 'json',
...
/*//form validation*/
	...
	$("#TBLDAFTREKLAME_NODAFTARIZIN").change(function(event) {
		...
		// if (n>1) {
		if (!empty(str) && !str.includes('REK-P')) {
...
function getDataReklame(CALLBACK_FUNCTION) {
	...
	$.ajax({
		url: 'penetapan/keringanan_reklame2016_tetap/GetDataReklame',
		type: 'POST',
		dataType: 'json',
		data: {
			 nodaftar: nodaftar
