<table id="dt_pipeline" class="table table-striped table-bordered table-hover smart-form" width="100%">
	...
	<tbody>
	<?php $no=$data['NOURUT']; foreach ($data['air'] as $row): ?>   
		<tr>
			<td>
				<?php if ($row['TBLDAFTTANAH_TGLSKPD']!=''): ?>
					<label class="label label-success" style="color: white;padding: 2px;" ><strong>Sudah di Tetapkan</strong></label>
				<?php else: ?>
					<?php if ($row['STATUS_DATA'] == '2'): ?> <!-- ubah alur rahmat 14-06-21 -->
					<div align="center">
						<label class="checkbox">
							<input checked="" class="cbx_tetapkan" value="<?php echo $row['TBLDAFTAR_NOPOK'] ?>-<?php echo (int)$row['TBLPENDATAAN_THNPAJAK'] ?>-<?php echo (int)$row['TBLPENDATAAN_BLNPAJAK'] ?>-<?php echo $no; ?>-<?php echo $row['PAJAK'] ?>" name="nopokPajakAir" type="checkbox">
							<i></i>
						</label>
					</div>
					<?php else: ?>
						<label class="label label-danger" style="color: white;padding: 2px;" ><strong>Belum diverifikasi Analis Penetapan</strong></label><!-- ubah redaksional rahmat 14-06-21 -->
					<?php endif ?>
				<?php endif; ?>
			</td>
			<td><?php if ($row['TBLDAFTTANAH_TGLSKPD']!=''): ?>
					<?php echo $row['TBLDAFTTANAH_NOMORSKPD'] ?>
				<?php else: ?>
					<?php echo $no; ?>
				<?php endif; ?>
				</td>
