...
<script type="text/javascript">
	...
	function generateAutocompleteWPHiburan() {
		$('#TBLDAFTAR_NOPOK').autocomplete({
			serviceUrl: 'pendataan/hiburantetap/autocompletewphiburan',
	...
	function cekPernahDaftar() {
		...
		if (TBLDAFTAR_NOPOK=='') {
			...
			$.ajax({
				url: 'pendataan/hiburantetap/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function getTglBatas(THNPAJAK,BLNPAJAK,KODEREK) {
		$.ajax({
			url: 'pendataan/hiburantetap/GetTGLBatas',
			type: 'POST',
			dataType: 'json',
			data: {
				 THNPAJAK : THNPAJAK
	...
	function cari() {
		...
		if (TBLDAFTAR_NOPOK=='') {
			...
			$.ajax({
				url: 'pendataan/hiburantetap/GetWPHiburan',
				type: 'POST',
				dataType: 'json',
				data: {
					 nopokok: window.nopokok
					,THNPAJAK: $('#TBLPENDATAAN_THNPAJAK').val()
					,BLNPAJAK: $('#TBLPENDATAAN_BLNPAJAK').val()
					,TGLPAJAK: $('#TBLPENDATAAN_TGLPAJAK').val() ? $('#TBLPENDATAAN_TGLPAJAK').val() : '0'  
				},
				success: function(respon) {
	...
	function getTarifRekening(kodesubrek) {
		$.ajax({
			url: 'pendataan/hiburantetap/GetKodeRek',
			type: 'POST',
			dataType: 'json',
			data: {
	...
	// 	$('#TBLDAFTHIBURAN_PAJAK').val(ceiling(pajak, 1));
	// 	$('#TBLDAFTHIBURAN_RUPIAHSETOR').val(ceiling(setor, 1));
	...
	function simpanhiburan () {
		$.ajax({
			url: 'pendataan/hiburantetap/SimpanHiburan',
			type: 'post',
			dataType: "json",
