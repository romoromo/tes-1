	...
	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="well well-sm well-light bg-color-greenLight txt-color-white">
				<h4><i class="fa fa-file-text-o"></i> SKPDKB - Pajak Restoran </h4>
			</div>
		</div>
	</div>
				...
				<div class="jarviswidget jarviswidget-color-teal" id="wid-id-kiosss" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
					...
					<div>
						...
						<div class="widget-body no-padding">
							<form id="form-skpdkb-restoran" class="smart-form">
								<fieldset>
									...
									<hr><br>
									<div class="row">
										<section class="col col-md-12"><h4 align="center">Entry SKPDKB Pajak Restoran</h4></section>
									</div>
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" value="<?= date('d-m-Y') ?>" name="TBLDAFTRMAKAN_TGLPERIKSA" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTRMAKAN_TGLPERIKSA">
											</label>
										</section>
									</div>
									<div class="row">
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input">
												<input class="form-control" type="text" id="TBLDAFTRMAKAN_REGKURANGBAYAR" name="TBLDAFTRMAKAN_REGKURANGBAYAR">
											</label>
										</section>
										<section class="col col-md-2">Tanggal SKPDKB</section>
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" value="<?= date('d-m-Y') ?>" name="TBLDAFTRMAKAN_TGLKURANGBAYAR" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTRMAKAN_TGLKURANGBAYAR">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-contro format-rupiah" type="text" id="TBLDAFTRMAKAN_BUNGAKRGBAYAR" name="TBLDAFTRMAKAN_BUNGAKRGBAYAR">
											</label>
										</section>
									...
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTRMAKAN_NAKB" name="TBLDAFTRMAKAN_NAKB">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTRMAKAN_DENDAKRGBAYAR" name="TBLDAFTRMAKAN_DENDAKRGBAYAR">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTRMAKAN_RUPIAHKRGBAYAR" name="TBLDAFTRMAKAN_RUPIAHKRGBAYAR">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" name="TBLDAFTRMAKAN_TGLBTSKRGBAYAR" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTRMAKAN_TGLBTSKRGBAYAR">
											</label>
										</section>
									</div>
								</fieldset>
...
<br>
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function($) {
			...
			$('#TBLDAFTRMAKAN_TGLKURANGBAYAR').val(today);

			setMoment();
		});

		loadScript("<?php echo Yii::app()->getBaseUrl(1) ?>/themes/smartadmin/js/plugin/devbridgeAutocomplete/jquery.autocomplete.min.js", function(){
			generateAutocompleteWPRestoran();
		});

		function getNoSKPDKB(tahun) {
			$.ajax({
				url: 'penetapan/skpdkb_restoran/GetNoSKPDKB',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
				...
				success: function(respon) {
					if (respon=='') {
						$('#TBLDAFTRMAKAN_REGKURANGBAYAR').val(1);
						
					}
					else{
						$('#TBLDAFTRMAKAN_REGKURANGBAYAR').val(respon.NOKB);
		...
		$("#TBLDAFTRMAKAN_TGLKURANGBAYAR").change(function(event) {
			var tglbatas = moment( $("#TBLDAFTRMAKAN_TGLKURANGBAYAR").val().split("-").reverse().join("-") ).add(1, "months");
			$("#TBLDAFTRMAKAN_TGLBTSKRGBAYAR").val( tglbatas.format('L') );
		});

		function generateAutocompleteWPRestoran() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'penetapan/skpdkb_restoran/autocompletewprestoran',
		...
		function reset_form(){
			$("#form-skpdkb-restoran")[0].reset();
			$("#form-skpdkb-restoran .select2").select2('val', '');
			setMoment();
		...
		function runFormValidation() {
			var $FormData = $("#form-skpdkb-restoran").validate({
		...
		function cetak () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/skpdkb_restoran/cetak?'+param);
		}
		...
		function cetakWord () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/skpdkb_restoran/cetakword?'+param);
		}

		$("#TBLDAFTRMAKAN_PAJAKPERIKSA, #TBLDAFTRMAKAN_BUNGAKRGBAYAR, #TBLDAFTRMAKAN_NAKB").change(function(event) {
			HitungSKPDKB();
			setPriceFormat();
		});

		function HitungSKPDKB() {
			PJKPERIKSA = toAngka($('#TBLDAFTRMAKAN_PAJAKPERIKSA').val());
			BUNGAKB = toAngka($('#TBLDAFTRMAKAN_BUNGAKRGBAYAR').val());
			KENAIKAN = toAngka($('#TBLDAFTRMAKAN_NAKB').val());
			...
			$(".TBLDAFTRMAKAN_RUPIAHKRGBAYAR").html(toRp(RUPIAHKB));
			$("#TBLDAFTRMAKAN_RUPIAHKRGBAYAR").val(RUPIAHKB);
			// $("#TBLDAFTRMAKAN_RUPIAHKRGBAYAR").val( RUPIAHKB.toFixed(2) );
		}
		...
		if (TBLDAFTAR_NOPOK=='') {
			notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
				...
				$.ajax({
					url: 'penetapan/skpdkb_restoran/CekPernahDaftar',
					type: 'POST',
					dataType: 'json',
					data: {
		...
		function cari () {
			...
			if (CARI_NOPOK=='') {
				...
				$.ajax({
					url: 'penetapan/skpdkb_restoran/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
					...
					success: function(respon) {
						if (respon.data=='sudah entri') {
							...
							$.SmartMessageBox({
								title : "Informasi", // judul Smart Alert
								content : "Data KB dengan Nomor Pokok "+CARI_NOPOK+", Masa Pajak "+ $('#TBLPENDATAAN_BLNPAJAK').select2('val')+" "+$('#TBLPENDATAAN_THNPAJAK').val()+" sudah pernah mendaftar. <br> Apakah anda ingin mengubah data ini?", // konten dari smart alert
								buttons : '[Perubahan][Cetak ulang]' // pengaturan tombol
									...
									if (ButtonPressed === "Perubahan") {
										...
										$('#TBLDAFTRMAKAN_REGKURANGBAYAR').val(respon.TBLDAFTRMAKAN_REGKURANGBAYAR);
										$('#TBLDAFTRMAKAN_PAJAKPERIKSA').val(respon.TBLDAFTRMAKAN_PAJAKPERIKSA);
										$('#TBLDAFTRMAKAN_BUNGAKRGBAYAR').val(respon.TBLDAFTRMAKAN_BUNGAKRGBAYAR);
										$('#TBLDAFTRMAKAN_NAKB').val(respon.TBLDAFTRMAKAN_NAKB);
										$('#TBLDAFTRMAKAN_DENDAKRGBAYAR').val(respon.TBLDAFTRMAKAN_DENDAKRGBAYAR);
										$('#TBLDAFTRMAKAN_RUPIAHKRGBAYAR').val(respon.TBLDAFTRMAKAN_RUPIAHKRGBAYAR);
										...
										$('#TBLDAFTRMAKAN_TGLKURANGBAYAR').val(respon.TBLDAFTRMAKAN_TGLKURANGBAYAR +'-'+ respon.TBLDAFTRMAKAN_BLNKURANGBAYAR +'-20'+respon.TBLDAFTRMAKAN_THNKURANGBAYAR);
										$('#TBLDAFTRMAKAN_TGLBTSKRGBAYAR').val(respon.TBLDAFTRMAKAN_TGLBTSKRGBAYAR +'-'+ respon.TBLDAFTRMAKAN_BLNBTSKRGBAYAR +'-20'+ respon.TBLDAFTRMAKAN_THNBTSKRGBAYAR);
							...
							$('#footer-restoran').show('fast');
							$('#btncetak').show('fast');
							$('#TBLDAFTRMAKAN_PAJAKPERIKSA').val('');
							$('#TBLDAFTRMAKAN_BUNGAKRGBAYAR').val('');
							$('#TBLDAFTRMAKAN_NAKB').val('');
							$('#TBLDAFTRMAKAN_DENDAKRGBAYAR').val('');
							$('#TBLDAFTRMAKAN_RUPIAHKRGBAYAR').val('');
							$('#TBLDAFTRMAKAN_NOMORPERIKSA').val('');
		...
		function simpanskpdkb () {
			$.ajax({
				url: 'penetapan/skpdkb_restoran/SimpanSKPDKBRestoran',
				type: 'post',
				dataType: "json",
				data:  $("#form-skpdkb-restoran").serialize()+'&idkecamatan='+window.idkecamatan+'&idkelurahan='+window.idkelurahan,
		...
		function hapuskb() {
			...
			$.SmartMessageBox({
				...
				buttons : '[Tidak][Ya]' // pengaturan tombol
					...
					if (ButtonPressed === "Ya") {
						$.ajax({
							url: 'penetapan/skpdkb_restoran/HapusSKPDKB',
							type: 'post',
							dataType: "json",
							data:  $("#form-skpdkb-restoran").serialize(),
							success: function(data) {
								if (data.status=="success") {
									notifikasi('Sukses','Data Berhasil Disimpan', 'success',1,0);
									$('#btnsimpan').attr('disabled','');
