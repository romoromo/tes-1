...
<div class="well well-sm well-light text-center">
	<h4><i class="fa fa-tasks"></i> Cetak Ulang Ketetapan By Tanggal</h4>
</div>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="widget-body slideInRight fast animated" id="cari" >
				  ...
				  <section class="col col-md-3">
					<label class="input"> <i class="icon-append fa fa-calendar"></i>
					  <input type="text" value="<?= date('d-m-Y') ?>" name="TGLSKPD2" class="datepicker" data-dateformat="dd-mm-yy" id="TGLSKPD2">
					</label>
				  </section>
								</div>
								<div class="row" style="display: none">
						...
						<div class="widget-body no-padding">
							<div class="widget-body-toolbar">
								...
								<div class="group_button_proses_reklame pull-right" style="display: none;">
									...
									<button class="btn btn-sm btn-default hidden" id="btnskpdreklame" onclick="cetakSKPDWord()" type="button">
								...
								<div class="group_button_proses_airtanah pull-right" style="display: none;">
									...
									<button class="btn btn-sm btn-default hidden" id="btnskpdairtanah" onclick="cetakSKPDWordAirtanah()" type="button">
										<i class="fa fa-print"></i> Cetak SKPD
									</button>
...
<form target="_blank" id="form-skpd-reklame" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakskpdword/">
	<input type="hidden" name="data" id="skpd-reklame">
</form>

<form target="_blank" id="form-skpd-airtanah" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakskpdwordairtanah/">
	<input type="hidden" name="data" id="skpd-airtanah">
</form>

<form target="_blank" id="form-cetak-nota-reklame" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetaknotaword/">
	<input type="hidden" name="data" id="nota-reklame">
</form>

<form target="_blank" id="form-cetak-nota-pat" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetaknotawordairtanah/">
	<input type="hidden" name="data" id="nota-pat">
</form>

<form target="_blank" id="form-cetak-daftar-reklame" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakdaftarword/">
</form>

<form target="_blank" id="form-cetak-daftar-pat" style="display: none;" method="post" action="<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakdaftarwordairtanah/">
</form>
...
<script type="text/javascript">
	...
	function loadDataTable() {
		...
		$.ajax({
			url: 'penetapan/cetak_ulang_ketetapan_bytanggal/cari',
			type: 'POST',
			// data: $("#form-cari").serialize(),
			data: (window.parametercari = $("#form-cari").serialize()+'&'+jQuery.param(param)),
		})
	...
	function cetakNota() {
		...
		// $('#skpd').val(q);
		// $('#form-skpd').submit();
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakskpd/?data=' + q);
	}
	...
	TGLSKPD2 = $('#TGLSKPD2').val();
		// TGLBATASSKPD = $('#TGLBATASSKPD').val();
		var q = daftaridPajak;
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakskpd/?data=' + q +"&TBLDAFTREKLAME_JNSREKLAME="+window.TBLDAFTREKLAME_JNSREKLAME+"&TBLPENDATAAN_THNPAJAK="+window.TBLPENDATAAN_THNPAJAK+"&TBLPENDATAAN_BLNPAJAK="+TBLPENDATAAN_BLNPAJAK+"&TBLDAFTREKLAME_TGLSKPD="+TBLDAFTREKLAME_TGLSKPD+"&TGLSKPD="+TGLSKPD+"&TGLSKPD2="+TGLSKPD2);
	}
	...
	function cetakDaftarWordAirtanah() {
		...
		var q = btoa(daftaridPajak);
		// window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakdaftarwordairtanah/?data=' + q +"&TBLPENDATAAN_THNPAJAK="+window.TBLPENDATAAN_THNPAJAK+"&TBLPENDATAAN_BLNPAJAK="+TBLPENDATAAN_BLNPAJAK+"&TBLDAFTREKLAME_TGLSKPD="+TBLDAFTREKLAME_TGLSKPD+"&TGLENTRI="+TGLENTRI+"&TGLSKPD="+TGLSKPD+"&TGLBATASSKPD="+TGLBATASSKPD);
		$('#form-cetak-daftar-pat').submit();
	}

	$('#form-cetak-daftar-pat').submit(function () {
		var params = $.unserialize(window.parametercari);
		$(this).append($.map(params, function (value, name) {
			return   $('<input>', {
				type: 'hidden',
				name: name,
				value: value
	...
	function cetakNotaWordAirtanah() {
		...
		var q = btoa(daftaridPajak);
		// window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetaknotawordairtanah/?data=' + q );
		$('#nota-pat').val(q);
		$('#form-cetak-nota-pat').submit();
	}

	$('#form-cetak-nota-pat').submit(function () {
		var params = $.unserialize(window.parametercari);
		$(this).append($.map(params, function (value, name) {
			return   $('<input>', {
				type: 'hidden',
	...
	function cetakNotaWord() {
		...
		var q = btoa(daftaridPajak);
		// window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetaknotaword/?data=' + q +"&TBLDAFTREKLAME_JNSREKLAME="+window.TBLDAFTREKLAME_JNSREKLAME);
		$('#nota-reklame').val(q);
		$('#form-cetak-nota-reklame').submit();
	}

	$('#form-cetak-nota-reklame').submit(function () {
		var params = $.unserialize(window.parametercari);
		$(this).append($.map(params, function (value, name) {
			return   $('<input>', {
				type: 'hidden',
	...
	function cetakDaftarWord() {
		...
		var q = btoa(daftaridPajak);
		// window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakdaftarword/?data=' + q +"&TBLDAFTREKLAME_JNSREKLAME="+window.TBLDAFTREKLAME_JNSREKLAME+"&TBLPENDATAAN_THNPAJAK="+window.TBLPENDATAAN_THNPAJAK+"&TBLPENDATAAN_BLNPAJAK="+TBLPENDATAAN_BLNPAJAK+"&TBLDAFTREKLAME_TGLSKPD="+TBLDAFTREKLAME_TGLSKPD+"&TGLSKPD="+TGLSKPD+"&TGLSKPD2="+TGLSKPD2);
		$('#form-cetak-daftar-reklame').submit();
	}

	$('#form-cetak-daftar-reklame').submit(function () {
		var params = $.unserialize(window.parametercari);
		$(this).append($.map(params, function (value, name) {
			return   $('<input>', {
				type: 'hidden',
	...
	function cetakSKPDWord() {
		...
		var q = btoa(daftaridPajak);
		// window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cetak_ulang_ketetapan_bytanggal/cetakskpdword/?data=' + q);
		$('#skpd-reklame').val(q);
		$('#form-skpd-reklame').submit();
	}
