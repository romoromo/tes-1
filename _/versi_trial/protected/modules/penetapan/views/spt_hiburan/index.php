...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entry Pendataan Pajak Hiburan</h4>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function generateAutocompleteWPHiburan() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'penetapan/spt_hiburan/autocompletewphiburan',
	...
	if (TBLDAFTAR_NOPOK=='') {
		notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
			...
			$.ajax({
				url: 'penetapan/spt_hiburan/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'penetapan/spt_hiburan/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										if (respon.data=='proses selanjutnya') {
											...
											$('#hit_penjualanbulan').val(respon.TBLDAFTHIBURAN_OMSETPAJAK);
											$('#hit_pajak').val(respon.TBLDAFTHIBURAN_PAJAK);
											$('#hit_bungaspt').val(respon.TBLDAFTHIBURAN_BUNGASPTPD);

											$('#hit_totalsetor').val(respon.TBLDAFTHIBURAN_PAJAK);

											$('#wp_input_tgl_spt').val((respon.TBLDAFTHIBURAN_TGLSPTPD)+'-'+(respon.TBLDAFTHIBURAN_BLNSPTPD)+'-'+'20'+(respon.TBLDAFTHIBURAN_THNSPTPD));
											
											$('#hit_tglbatasspt').val((respon.TBLDAFTHIBURAN_TGLTERIMA)+'-'+(respon.TBLDAFTHIBURAN_BLNTERIMA)+'-'+'20'+(respon.TBLDAFTHIBURAN_THNTERIMA));

											$('#hit_tglterimaspt').val((respon.TBLDAFTHIBURAN_TGLBATASSPTPD)+'-'+(respon.TBLDAFTHIBURAN_BLNBATASSPTPD)+'-'+'20'+(respon.TBLDAFTHIBURAN_THNBATASSPTPD));

											//$('#hit_tglentryspt').val((respon.TBLDAFTHIBURAN_TGLENTRI)+'-'+(respon.TBLDAFTHIBURAN_BLNENTRI)+'-'+'20'+(respon.TBLDAFTHIBURAN_THNENTRI));
	...
	$.ajax({
		url: 'penetapan/spt_hiburan/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			 THNPAJAK : THNPAJAK
	...
	function simpan() {
		...
		$.ajax({
			url: 'penetapan/spt_hiburan/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
