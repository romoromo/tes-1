...
class Skpdnihil_hiburanController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		// echo $cek['REGNIHIL']. $proses['TBLPENDATAAN_THNPAJAK']. $proses['CEKPAJAK'];Yii::app()->end();
		if ($proses['TBLPENDATAAN_THNPAJAK'] == '') {
			$data['data'] = 'belum terdaftar';
		} else if ($proses['CEKPAJAK'] != '0') { // lapor pajak tidak 0
		 ...
		 if ($cekbayar == '') {
			$data['data'] = 'belum bayar';
				...
				if ($cek['REGNIHIL'] != '0') {
					...
					$filter_AND = array(
						'EQ__TBLDAFTAR.TBLDAFTAR_GOLONGAN' => $this->KODE_GOL, 'EQ__REFBADANUSAHA.REFBADANUSAHA_REKAYAT' => $this->PAJAK_AYAT, "EQ__TBLDAFTHIBURAN.TBLPENDATAAN_THNPAJAK" => $tahun_substr, "EQ__TBLDAFTHIBURAN.TBLPENDATAAN_BLNPAJAK" => $bulan_akhir
					);

					$otherquery = array(
						'limit' => 30, 'andwhere' => 'NVL(TBLDAFTHIBURAN.TBLDAFTHIBURAN_REGNIHIL, 0) <> 0', 'join' => array('REFBADANUSAHA', 'REFBADANUSAHA.REFBADANUSAHA_ID= TBLDAFTAR.REFBADANUSAHA_IDBADANUSAHA'), 'join_2' => array('TBLDAFTHIBURAN', "TBLDAFTAR.TBLDAFTAR_NOPOK = TBLDAFTHIBURAN.TBLDAFTAR_NOPOK"), 'order' => 'TBLDAFTAR.TBLDAFTAR_NOPOK ASC'
					);
					...
					$otherquery = array(
						'limit' => 30, 'join' => array('REFBADANUSAHA', 'REFBADANUSAHA.REFBADANUSAHA_ID= TBLDAFTAR.REFBADANUSAHA_IDBADANUSAHA'), 'join_2' => array('TBLDAFTHIBURAN', "TBLDAFTAR.TBLDAFTAR_NOPOK = TBLDAFTHIBURAN.TBLDAFTAR_NOPOK"), 'order' => 'TBLDAFTAR.TBLDAFTAR_NOPOK ASC'
					);
					$model = DBFetch::query(array('select' => $select, 'from' => $from, 'filter' => $filter, 'filter_AND' => $filter_AND, 'filterOR' => true, 'otherquery' => $otherquery, 'mode' => 'DETAIL'));
	...
	public function actionCetakWord()
	{
        ...
        $otherquery['andwhere'] = '
	        NVL(TBLDAFTHIBURAN.TBLPENDATAAN_TGLPAJAK, 0)= 0';
	...
	{
		$namatable = $this->namatabel;
		$model = Yii::app()->db->createCommand("SELECT TBLPENDATAAN_THNPAJAK,NVL(" . $namatable . "_PAJAK,0) CEKPAJAK  FROM ".$namatable." WHERE TBLPENDATAAN_THNPAJAK ='$tahun' AND TBLPENDATAAN_BLNPAJAK='$bulan' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLDAFTAR_NOPOK=".$TBLDAFTAR_NOPOK)->queryRow();
		return $model;
	}

	public function cekPernahDaftar($thn, $bulan, $nopok)
	{
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLDAFTHIBURAN_REGNIHIL,0) REGNIHIL, NVL(TBLDAFTHIBURAN_NOMORPERIKSA, 0) REGPERIKSA FROM $this->namatabel WHERE TBLPENDATAAN_THNPAJAK ='$thn' AND TBLPENDATAAN_BLNPAJAK='$bulan' AND TBLDAFTAR_NOPOK='$nopok'")->queryRow();
		return $model;
	}
