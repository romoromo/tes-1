...
class Skpdnihil_restoranController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 2;
	...
	public function actionIndex()
	{
		...
		$data['sub_rek'] = TRekening::model()->getRekeningSubAktif('4110200');
	...
	public function actionGetNoSKPDNihil()
	{
		...
		$data = Yii::app()->db->createCommand("SELECT
			MAX(TO_NUMBER (TO_CHAR(NVL(TBLDAFTRMAKAN_REGNIHIL, 0))))+1 AS nolb
		FROM
			TBLDAFTRMAKAN
		WHERE
			TBLPENDATAAN_THNPAJAK = ".$splthn."
		AND NVL (TBLDAFTRMAKAN_REGNIHIL, 0) <> 0
		ORDER BY
	...
	public function actiongetdata()
	{
		...
		NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_REGNIHIL, 0) AS TBLDAFTRMAKAN_REGNIHIL,
		...
		NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_RUPIAHPERIKSA,0) AS TBLDAFTRMAKAN_RUPIAHPERIKSA,
		NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_TANGGLNIHIL,0) AS TBLDAFTRMAKAN_TANGGLNIHIL,
		NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_BULANNIHIL,0) AS TBLDAFTRMAKAN_BULANNIHIL,
		NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_TAHUNNIHIL,0) AS TBLDAFTRMAKAN_TAHUNNIHIL 
		";
		$from = 'TBLDAFTAR';
		$filter = array(
		...
		$filter_AND = array(
			...
			,"EQ__TBLDAFTRMAKAN.TBLPENDATAAN_BLNPAJAK" => $bulan_akhir
		);

		$otherquery = array(
			'limit'=> 30
			,'andwhere'=> 'NVL(TBLDAFTRMAKAN.TBLDAFTRMAKAN_REGNIHIL, 0) <> 0'
		...
		$data['TBLDAFTRMAKAN_RUPIAHPERIKSA'] = $model['TBLDAFTRMAKAN_RUPIAHPERIKSA'];
		$data['TBLDAFTRMAKAN_REGNIHIL'] = $model['TBLDAFTRMAKAN_REGNIHIL'];
		...
		$data['TBLDAFTRMAKAN_TANGGLNIHIL'] = sprintf('%02d',$model['TBLDAFTRMAKAN_TANGGLNIHIL']); 
		$data['TBLDAFTRMAKAN_BULANNIHIL'] = sprintf('%02d',$model['TBLDAFTRMAKAN_BULANNIHIL']); 
		$data['TBLDAFTRMAKAN_TAHUNNIHIL'] = $model['TBLDAFTRMAKAN_TAHUNNIHIL']; 
		...
		$filter_AND = array(
			...
			,"EQ__TBLDAFTRMAKAN.TBLPENDATAAN_THNPAJAK" => $tahun_substr
			,"EQ__TBLDAFTRMAKAN.TBLPENDATAAN_BLNPAJAK" => $bulan_akhir
			// ,"EQ__TBLDAFTRMAKAN.TBLDAFTRMAKAN_BLNKBAKHIR" => $bulan_akhir
			// ,'EQ__tblsubyek_isaktif' => "T"
		);
	...
	public function actionCetakWord()
	{
        ...
        $otherquery['andwhere'] = '
	        NVL(TBLDAFTRMAKAN.TBLPENDATAAN_TGLPAJAK, 0)= 0';
		...
		$dt['thnkb'] = '20'.$data['cetak']['TBLDAFTRMAKAN_TAHUNNIHIL'];
		$dt['blnawal'] = LokalIndonesia::getbulan($data['cetak']['TBLDAFTRMAKAN_BLNKBAWAL']);
		$dt['blnakhir'] = LokalIndonesia::getbulan($data['cetak']['TBLDAFTRMAKAN_BLNKBAKHIR']);
		$dt['regkb'] = $data['cetak']['TBLDAFTRMAKAN_REGNIHIL'];
		...
		$dt['pajakperiksa'] = LokalIndonesia::rupiah($data['cetak']['TBLDAFTRMAKAN_RUPIAHPERIKSA']);
		...
		$namafileDL = "SKPDNIHIL-RUMAHMAKAN-".$npwpd.".docx";
	...
	{
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLDAFTRMAKAN_REGNIHIL,0) FROM $this->namatabel WHERE TBLPENDATAAN_THNPAJAK ='$thn' AND TBLPENDATAAN_BLNPAJAK='$bulan' AND TBLDAFTAR_NOPOK='$nopok' AND NVL(TBLDAFTRMAKAN_NOMORPERIKSA, 0) <> 0")->queryScalar();
		return $model;
	}
	...
	public function actionSimpanSKPDNRestoran()
	{
		Yii::import('ext.LokalIndonesia');
		...
		$TBLPENDATAAN_BLNPAJAK = (int)!empty($_REQUEST['TBLDAFTRMAKAN_BLNKBAKHIR']) ? $_REQUEST['TBLDAFTRMAKAN_BLNKBAKHIR'] : 0;

		$TBLDAFTRMAKAN_TANGGLNIHIL = !empty($_REQUEST["TBLDAFTRMAKAN_TANGGLNIHIL"]) ? $_REQUEST["TBLDAFTRMAKAN_TANGGLNIHIL"] : '';

		$TBLDAFTRMAKAN_TGLPERIKSA = !empty($_REQUEST["TBLDAFTRMAKAN_TGLPERIKSA"]) ? $_REQUEST["TBLDAFTRMAKAN_TGLPERIKSA"] : '';

		if ($TBLDAFTRMAKAN_TANGGLNIHIL !='') {
			$exp_TGLTERIMADAFTAR = explode('-', $TBLDAFTRMAKAN_TANGGLNIHIL);
		...
		$column = array(
			...
			"TBLDAFTRMAKAN_REGNIHIL" => LokalIndonesia::toAngka($PARAM["TBLDAFTRMAKAN_REGNIHIL"]),

			"TBLDAFTRMAKAN_TANGGLNIHIL" => $pecahtgldaftar,
			"TBLDAFTRMAKAN_BULANNIHIL" => $pecahbulandaftar,
			"TBLDAFTRMAKAN_TAHUNNIHIL" => $pecahthndaftar,
			...
			"TBLDAFTRMAKAN_RUPIAHPERIKSA" => LokalIndonesia::toAngka($PARAM["TBLDAFTRMAKAN_RUPIAHPERIKSA"]),
