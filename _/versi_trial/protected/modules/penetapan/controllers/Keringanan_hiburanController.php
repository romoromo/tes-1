...
class Keringanan_hiburanController extends Controller
{
	...
	public function actionGetWPHiburan()
	{
		...
		$data = Yii::app()->db->createCommand("
			SELECT TBLDAFTHIBURAN.*, NVL(TBLDAFTHIBURAN_REGSETOR, 'XXX') AS TBLDAFTHIBURAN_REGSETOR 
			FROM TBLDAFTHIBURAN 
			WHERE TBLPENDATAAN_THNPAJAK =:THNPAJAK
			AND TBLPENDATAAN_BLNPAJAK=:BLNPAJAK
			AND TBLDAFTAR_NOPOK=:NOPOKOK
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=:TGLPAJAK
		")
		->bindParam(':THNPAJAK', $THNPAJAK)
		->bindParam(':BLNPAJAK', $BLNPAJAK)
		->bindParam(':NOPOKOK', $NOPOKOK)
		->bindParam(':TGLPAJAK', $TGLPAJAK)
		->queryRow();
		echo CJSON::encode($data);
	}
	...
	public function actionSimpanHiburan()
	{
		...
		$TBLPENDATAAN_TGLPAJAK = (int)isset($_REQUEST['TBLPENDATAAN_TGLPAJAK']) && !empty($_REQUEST['TBLPENDATAAN_TGLPAJAK']) ? $_REQUEST['TBLPENDATAAN_TGLPAJAK'] : 0;
		if (Yii::app()->request->getParam('is_tanggal') != 'T') {
		...
		$TBLDAFTHIBURAN_PERSENRINGAN = isset($_REQUEST['TBLDAFTHIBURAN_PERSENRINGAN']) && !empty($_REQUEST['TBLDAFTHIBURAN_PERSENRINGAN']) ? $_REQUEST['TBLDAFTHIBURAN_PERSENRINGAN'] : 0;
		...
		if($ada_data=='belum'){
			...
			$column = array(
				...
				'TBLDAFTHIBURAN_PAJAK' => LokalIndonesia::toAngka($TBLDAFTHIBURAN_PAJAK),
				'TBLDAFTHIBURAN_PAJAKSKPD' => LokalIndonesia::toAngka($TBLDAFTHIBURAN_PAJAK),
				'TBLDAFTHIBURAN_PERSENRINGAN' => LokalIndonesia::toAngka($TBLDAFTHIBURAN_PERSENRINGAN),
				// 'TBLDAFTHIBURAN_BUNGASPTPD' => LokalIndonesia::toAngka($TBLDAFTHIBURAN_BUNGASPTPD),
				// 'TBLDAFTHIBURAN_RUPIAHSETOR' => '',
				// 'TBLDAFTHIBURAN_TGLTERIMA' => $exp_TGLTERIMA[0],
				// 'TBLDAFTHIBURAN_BLNTERIMA' => $exp_TGLTERIMA[1],
