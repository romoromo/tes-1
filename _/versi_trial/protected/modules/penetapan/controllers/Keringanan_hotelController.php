...
class Keringanan_hotelController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 1;
	...
	public function actiongetdata()
	{
		...
		/*} else if ($teguran==0) {
			...
			$filter_AND = array(
				...
				,'EQ__TBLDAFTHOTEL.TBLPENDATAAN_TGLPAJAK' => $tgl
			...
			TBLDAFTHOTEL.TBLPENDATAAN_THNPAJAK,
			TBLDAFTHOTEL.TBLPENDATAAN_BLNPAJAK,
			NVL(TBLDAFTHOTEL.TBLDAFTHOTEL_OMSETPAJAK, 0) AS TBLDAFTHOTEL_OMSETPAJAK,
			NVL(TBLDAFTHOTEL.TBLDAFTHOTEL_PAJAK, 0) AS TBLDAFTHOTEL_PAJAK,
			NVL(TBLDAFTHOTEL.TBLDAFTHOTEL_BUNGASPTPD, 0) AS TBLDAFTHOTEL_BUNGASPTPD,
			TBLDAFTHOTEL.TBLDAFTHOTEL_THNSPTPD,
			TBLDAFTHOTEL.TBLDAFTHOTEL_BLNSPTPD,
	...
	public function actionsimpandata()
	{
		...
		$column = array(
		    ...
		    'TBLDAFTHOTEL_PERSENRINGAN'=>$hit_keringanan,
		...
		// var_dump($column);Yii::app()->end();
		if ($cek>0) {
			$res['data'] = 'ada';
			$command = Yii::app()->db->createCommand();

			$simpan = $command->update('TBLDAFTHOTEL', $column ,'TBLDAFTAR_NOPOK =:NOPOK AND TBLPENDATAAN_THNPAJAK =:THNPAJAK AND TBLPENDATAAN_BLNPAJAK =:BLNPAJAK AND NVL(TBLPENDATAAN_TGLPAJAK, 0) =:TGLPAJAK',array(':NOPOK' => $nopokok, ':THNPAJAK' =>substr($tahun_pajak, -2), ':BLNPAJAK' => $bulan_pajak, ':TGLPAJAK' => (int)$tanggal_pajak));
			if ($simpan) {
				$res['status'] = 'update';
			}else{
				$res['status'] = 'failed';
	...
	{
		$model = Yii::app()->db->createCommand('SELECT TBLDAFTHOTEL_RUPIAHSETOR FROM TBLDAFTHOTEL WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND TBLPENDATAAN_BLNPAJAK='.$bulan.' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)='.$tgl.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	...
	{
		$model = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTHOTEL WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND TBLPENDATAAN_BLNPAJAK='.$bulan.' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)='.$tgl.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	}

	public function cekTeguran($thn, $bulan, $tgl, $nopok)
	{
		$model = Yii::app()->db->createCommand('SELECT TBLDAFTHOTEL_TGLENTRI FROM TBLDAFTHOTEL WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND TBLPENDATAAN_BLNPAJAK='.$bulan.' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)='.$tgl.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	}
	...
	public function actionDelt()
	{
		...
		$dt = Yii::app()->db->createCommand('SELECT NVL(TBLDAFTHOTEL_TGLENTRI, 0) AS TGLENTRI FROM TBLDAFTHOTEL WHERE 
		...
		if ($dt && $dt['TGLENTRI']==0) {
			$del = Yii::app()->db->createCommand()->delete('TBLDAFTHOTEL', '
				TBLPENDATAAN_THNPAJAK =:thn 
				AND TBLPENDATAAN_BLNPAJAK=:bulan 
				AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=:tgl
				AND TBLDAFTAR_NOPOK=:nopok
