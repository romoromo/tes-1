...
class HiburantetapController extends Controller
{
	var $KODE_GOL = 4;
	var $PAJAK_AYAT = 0;
	...
	public function actionGetWPHiburan()
	{
		...
		$data = Yii::app()->db->createCommand('SELECT * FROM TBLDAFTHIBURAN WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND NVL(TBLPENDATAAN_TGLPAJAK , 0)='.$TGLPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryRow();
	...
	public function actionSimpanHiburan()
	{
		...
		if($ada_data=='belum'){
			...
			$simpan = $command->update('TBLDAFTHIBURAN', $column ,'TBLDAFTAR_NOPOK =:NOPOK AND TBLPENDATAAN_THNPAJAK =:THNPAJAK AND TBLPENDATAAN_BLNPAJAK =:BLNPAJAK AND NVL(TBLPENDATAAN_TGLPAJAK , 0)=:TGLPAJAK',array(':NOPOK' => $TBLDAFTAR_NOPOK, ':THNPAJAK' =>substr($TBLPENDATAAN_THNPAJAK, -2), ':BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK , ':TGLPAJAK' => $TBLPENDATAAN_TGLPAJAK));
			if ($simpan>0) {
				echo CJSON::encode(array('status'=>'success'));
			}
			else{
	...
	public function actionCekPernahDaftar()
	{
		...
		$TGLPAJAK = $_REQUEST['TGLPAJAK'] ? $_REQUEST['TGLPAJAK'] : '0';

		$cek = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTHIBURAN WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND NVL(TBLPENDATAAN_TGLPAJAK , 0)='.$TGLPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryScalar();
		if ($cek) {
