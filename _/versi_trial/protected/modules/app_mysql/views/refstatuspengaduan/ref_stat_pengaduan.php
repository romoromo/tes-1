	...
	<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="jarviswidget jarviswidget-color-blueDark" id="wid-id-12" data-widget-editbutton="false">
				...
				<header>
					...
					<h2>Form Input Status Pengaduan</h2>
					...
					<div class="widget-body no-padding">
						<div class="widget-body-toolbar">
							<a  data-toggle="modal" onclick="tambah()" data-target="#tambahData">
                                <button class="btn btn-labeled btn-primary btn-sm" name="modalFormAdd"><span class="btn-label">
						...
						<table id="datatable_fixed_column" class="table table-striped table-bordered smart-form">
							<thead>
								...
								<tr>
								  <th width="20">No</th>
								  <th width="155"><div align="center">Status Pengaduan </div></th>
								  <th width="120"><div align="center">Status Aktif </div></th>
								  <th width="89">&nbsp;</th>
							  </tr>
								<tr class="second">
									<td>
							...
							foreach ($listdata as $datatabel1) {
								if ($datatabel1['refstatuspengaduan_isaktif']=='T') {
									$stts='Aktif';
								}
							...
							echo '
								<tr>
									...
									?>
									<?php echo '
									<td><div align="center">'.$datatabel1['refstatuspengaduan_keterangan'].'</div></td>
									<td><div align="center">'.$stts.'</div></td>
									';?>
							        <!-- <td><div align="center">Aktif</div></td> -->
                                    ...
                                    data-content="
                                    	<div class='row'><div class='col-md-12'><div class='well'>                                        
                                   			<a  data-toggle='modal' data-target='#editData'>
                                   				<button onclick='edit(<?php echo $datatabel1['refstatuspengaduan_id']; ?>)' class='btn btn-labeled btn-success btn-sm' type='submit'><span class='btn-label'>
                                    				<i class='fa fa-edit'></i></span>Edit</button></a>
                                        				
                                        		<button onclick='hapus(<?php echo $datatabel1['refstatuspengaduan_id']; ?>)' class='btn btn-labeled btn-danger btn-sm'><span class='btn-label'>
...
<div class="modal fade" id="modalFormAdd" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form action="" name="formStatusPengaduan" id="formStatusPengaduan" class="smart-form" novalidate>
				<fieldset>
					<div class="row">
						...
						<section class="col col-8">
							<label class="input"> <i class="icon-append fa fa-th-list"></i>
								<input class="form-control" name="refstatuspengaduan_keterangan" id="refstatuspengaduan_keterangan" required rows="1" placeholder="Isikan Status Pengaduan di sini">
									<b class="tooltip tooltip-bottom-right">Mohon Isikan Status Pengaduan di sini</b>
							</label>
						</section>
					</div>
				<hr>
						...
						<section class="col col-8">
							<div>
								<label class="select"> <i class="icon-append fa fa-square"></i>
									<select name="refstatuspengaduan_isaktif" id="refstatuspengaduan_isaktif">
...
<script>
	...
	function runFormValidation() {
		var $orderForm = $("#formStatusPengaduan").validate({
				// Rules for form validation
				rules : {
					refstatuspengaduan_keterangan : {
						required : true
					},
					refstatuspengaduan_isaktif : {
						required : true,
					}
				},
				...
				messages : {
					refstatuspengaduan_keterangan : {
						required : 'Mohon Isikan Status Pengaduan'
					},
					refstatuspengaduan_isaktif : {
						required : 'Mohon Masukan Status'
					}
				},
	...
	function notifikasi(title, msg, type) {1
		if (type=='success') {
			var warna = '#296191';
			var icon = 'fa-save';
		} else {
	...
	function tambah() {
		...
		$("#call").val('tambah');
		$("#refstatuspengaduan_keterangan").val();
		$("#refstatuspengaduan_isaktif").val();

	}

	function edit(id) {
		window.refstatuspengaduan_id = id;
		$("#modalFormAdd").modal('show'); // panggil modal
		$("#tambahData").modal('show'); // panggil modal
		$("#call").val('edit');
		$("#refstatuspengaduan_keterangan").val(getdata(id,'refstatuspengaduan_keterangan'));
		$("#refstatuspengaduan_isaktif").val(getdata(id,'refstatuspengaduan_isaktif'));
	}

	function simpan() {
		$.ajax({
			url: 'app/refstatuspengaduan/simpandata',
			type: 'post',
			data: {
				'call' : $("#call").val(),
				'id' : window.refstatuspengaduan_id,
				'refstatuspengaduan_keterangan' : $("#refstatuspengaduan_keterangan").val(),
				'refstatuspengaduan_isaktif' : $("#refstatuspengaduan_isaktif").val(),
			},
	...
	function getdata(id,field) {
		var get = $.ajax({
			url: 'app/refstatuspengaduan/ajxgetdata',
			type: 'post',
			async: false,
	...
	function hapus(id) {
		...
		$.SmartMessageBox({
			...
			if (ButtonPressed === "Ya") {
				...
				/*jalankan ajax hapus*/
				$.ajax({
					url: 'app/refstatuspengaduan/hapusdata',
					type: 'post',
