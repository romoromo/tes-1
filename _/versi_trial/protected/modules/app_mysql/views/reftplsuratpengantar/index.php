...
<div class="well well-sm well-light">	
	...
	<a href="#app/Reftblsuratpengantar" class="btn btn-labeled btn-primary">
		<span class="btn-label">
			<i class="fa fa-plus "></i>
		</span>Tabel Surat Pengantar
	</a>
</div>
	...
	<div class="row">
		...
		<div class="col-md-2">
			...
			<button onclick="buattemplatesk()" class="btn btn-labeled btn-primary">
				...
				</span>Buat Template Surat Pengantar
			</button>
		</div>
</div>
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-editbutton="false">
				...
				<header>
					...
					<h2>Template Surat Pengantar</h2>
				    ...
				    <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
                      ...
                      <?php $no=1; foreach ($datatemplatesk as $templatesk): ?>
                        <tr>
                          ...
                          <td><a href="file/temp_surat_pengantar/<?php echo $templatesk['tblspizin_sptemplate']; ?>"><?php echo $templatesk['tblspizin_sptemplate']; ?></a></td>
                          <td><?php echo $templatesk['tblspizin_tabelsp']; ?></td>
                          <td>
                           <button onclick="edit(<?php echo $templatesk['tblspizin_id']; ?>)" type="button" class="btn btn-labeled btn-success btn-sm" style="width:65px; height:30px;"> 		
					    		<i class="fa fa-edit"></i>
					    		Edit
					    	</button>

					    	<button  onclick="hapus(<?php echo $templatesk['tblspizin_id']; ?>)" type='button' class='btn btn-labeled btn-danger btn-sm' style="width:65px; height:30px;">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel"><i class="fa fa-check"></i>&nbsp;Form Template Surat Pengantar</h4>
			</div>
				...
				<fieldset>
					<div class="row">
						<section class="col col-3">
							<p>Tabel Variabel Template Surat Pengantar </p>
						</section>
						<section class="col col-1">:
						</section>
						<section class="col col-6">
							<label class="select">
								<select class="input-sm" id="nama_table">
								<option value="0">===Silahkan Pilih Table===</option>
										<?php
											foreach ($comboTable as $table)  {
											echo '<option value="'.$table['tblspizin_tabelvariabel_id'].'">'.$table['tblspizin_tabelsp'].'</option>';
											}
										?>
								</select> <i></i>  
							</label>
			...
			<form method="post" enctype="multipart/form-data" class="smart-form" action="app/Reftplsuratpengantar/SimpanFileTemplateSK" id="form-upload-sk">
			<fieldset>
				<div class="row">
					<section class="col col-6">
						<label class="label">Upload Template Surat Pengantar</label>
						<label for="file" class="input input-file">
								<div class="button">
									<input type="file" id="nama_filesk" name="nama_filesk" onchange="this.parentNode.nextSibling.value = this.value">Browse</div><input type="text" placeholder="Upload File RTF" readonly="">
									<input type="hidden" name="namafilenya_sk" id="namafilenya_sk" value="">
									<b class="tooltip tooltip-top-right">Upload Template Surat Pengantar</b>
						</label>
						<input type="hidden" name="namafilenya_sk" id="namafilenya_sk" value="">
					</section>
				</div>
...
function edit (id) {
	window.cmd = "edit";
	$.ajax({
		url: 'app/Reftplsuratpengantar/Edit',
		type: 'post',
		data: {id: id},
...
function getvariabel (tabel) {
	$.ajax({
		url: 'app/Reftplsuratpengantar/InfoVariabel',
		type: 'post',
		data: {tabel: tabel},
		success:function(respon){
...
function simpan() {
	...
	$.ajax({
		url: 'app/Reftplsuratpengantar/simpan',
		type: 'POST',
		data: {
			cmd: window.cmd,
			id: window.id,
		...
		,success: function  (respon) {
				...
				if (respon=='success'){
					...
					notifikasi("Sukses","Data berhasil disimpan","success",1,1);
				}
				else
...
function hapus(id) {
	$.SmartMessageBox({
				...
				if (ButtonPressed === "Ya") {
					$.ajax({
						url: 'app/Reftplsuratpengantar/Hapus',
						type: 'post',
						data: {id: id},
						success:function (respon) {
							if (respon=='success') {
								...
								notifikasi("Gagal", "Data Gagal dihapus","success");
							}
						}
					});
				}
...
function getJenisPermohonan(idizin) {
		$.ajax({
                type: 'POST',
                url: 'app/Reftplsuratpengantar/GetJenisPermohonan',
                dataType: 'html',
                data: {
                	id: idizin
                },
