  ...
  <label class="select">
    <label class="input">
      <input type="number" value="<?php echo date('Y'); ?>" class="form-control" type="text" id="tahunpjk_akhir" name="param[tahunpjk_akhir]">
             ...
             <div class="row">
               <section class="col col-md-2">
                <p>Tanggal Cut Off Jatuh Tempo</p>
              </section>
              <section class="col col-md-3">
                <label class="input"><i class="icon-append fa fa-calendar"></i>
                  <input type="text" id="tgl_cutoff" name="param[tgl_cutoff]" class="datepicker" data-dateformat="dd-mm-yy" >
                </label>
              </section>
            </div>
          ...
          <button type="button" class="btn btn-sm btn-primary" onclick="cari2()">
            <i class="fa  fa-search"></i>&nbsp;Lihat Perkembangan Piutang
          </button>
          <!-- <?php //endif ?> -->
	...
	<article class="col-sm-12 col-md-12 col-lg-12">
		...
		data-widget-deletebutton="false" data-widget-fullscreenbutton="false" data-widget-sortable="false">
         ...
         <header>  
          ...
          <!-- <?php //if (Tblrole::model()->isRole('SUPERADMIN')): ?> -->
          <button type="button" class="btn btn-success btn-sm" onclick="cetakperkembangan('excel')">
            <i class="fa fa-print"></i>&nbsp;Cetak Perkembangan Piutang
          </button>
          <!-- <?php// endif ?> -->
        </header>
...
function cari () {
			...
			$.ajax({
				url: 'pembukuandanpelaporan/daftartunggakan_skpdkb/Cari',
				type: 'POST',
				data: {
  ...
  function cari2 () {
      ...
      $("#tabel_laporan").html('<div align="center">'+LOADER+'').fadeIn(400);
      $.ajax({
        url: 'pembukuandanpelaporan/daftartunggakan_skpdkb/Cari2',
        type: 'POST',
        data: {
          TBLREKENING_KODE : $('#TBLREKENING_KODE').val(),
          TBLDAFTAR_NOPOK : $('#TBLDAFTAR_NOPOK').val(),
      ...
      .fail(function() {
		...
		function cetakperkembangan(jenis){
			var TBLREKENING_KODE = $('#TBLREKENING_KODE').val();
			var TBLDAFTAR_NOPOK = $('#TBLDAFTAR_NOPOK').val();
      var tahunpjk = ($('#th_pajak').prop('checked') ? $('#tahunpjk').val() : '');
      var tahunpjk_akhir = ($('#th_pajak').prop('checked') ? $('#tahunpjk_akhir').val() : '');
      var tahunkb = ($('#tahun_kb').prop('checked') ? $('#tahunkb').val() : '');
      var tahunkb_akhir = ($('#tahun_kb').prop('checked') ? $('#tahunkb_akhir').val() : '');
      var tgl_cutoff = $('#tgl_cutoff').val();
      window.open('<?php echo Yii::app()->getBaseUrl(1) ?>/pembukuandanpelaporan/daftartunggakan_skpdkb/Cari2?TBLDAFTAR_NOPOK='+TBLDAFTAR_NOPOK+'&TBLREKENING_KODE='+TBLREKENING_KODE+'&tahunkb='+tahunkb+'&jenis='+jenis+'&tahunpjk='+tahunpjk+'&tgl_cutoff='+tgl_cutoff+'&tahunpjk_akhir='+tahunpjk_akhir+'&tahunkb_akhir='+tahunkb_akhir);
    }
    ...
    function cetak(jenis){
      ...
      var tgl_cutoff = $('#tgl_cutoff').val();
      window.open('<?php echo Yii::app()->getBaseUrl(1) ?>/pembukuandanpelaporan/daftartunggakan_skpdkb/Cetak?TBLDAFTAR_NOPOK='+TBLDAFTAR_NOPOK+'&TBLREKENING_KODE='+TBLREKENING_KODE+'&tahunkb='+tahunkb+'&jenis='+jenis+'&tahunpjk='+tahunpjk+'&tgl_cutoff='+tgl_cutoff+'&tahunpjk_akhir='+tahunpjk_akhir+'&tahunkb_akhir='+tahunkb_akhir);
    }
    ...
    function CetakExcel(){
      ...
      window.open('<?= Yii::app()->getBaseUrl(1); ?>/pembukuandanpelaporan/daftartunggakan_skpdkb/Cetak?'+ param);
    }
    ...
    function CetakWord(){
      ...
      window.open('<?= Yii::app()->getBaseUrl(1); ?>/pembukuandanpelaporan/daftartunggakan_skpdkb/CetakWord?'+ param);
    }
