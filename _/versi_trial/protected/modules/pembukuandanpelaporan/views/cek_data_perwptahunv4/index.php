...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> History Data Pembayaran WP</h4>
		</div>
	</div>
</div>
					...
					<form action="" id="form_sumber_pajak" class="smart-form" novalidate>
						<fieldset>
							...
							<section class="col col-md-2">
							  <label class="select">
							    <label class="input">
							      <input type="number" value="<?php echo date('Y'); ?>" class="form-control" type="text" id="masapajak_tahun2" name="masapajak_tahun2">
							    </label>
							  </label>
							</section>
							</div>
							...
							<div class="row" id="div_nopok" style="display:none">
								<section class="col col-md-2">
									<p>No. Pokok</p>
								</section>
								<section class="col col-md-3">
							...
							<footer>
								...
								<div class="row">
								<button type="button" onclick="cetakbulanan()" class="btn btn-sm btn-primary">
									<i class="fa fa-print"></i>&nbsp;Cetak Daftar SPTPD
								</button>
								<button type="button" onclick="caribulanan()" class="btn btn-sm btn-default">
									<i class="fa fa-desktop"></i>&nbsp;View Daftar SPTPD
								</button>
								</div>
								<div class="row">
								<button type="button" onclick="cetak()" class="btn btn-sm btn-primary">
									<i class="fa fa-print"></i>&nbsp;Cetak Ketetapan Lain
								</button>
								<button type="button" onclick="cari()" class="btn btn-sm btn-default">
									<i class="fa fa-desktop"></i>&nbsp;View Ketetapan Lain
								</button>
								</div>
							</footer>					
						</form>
...
<!--Modal Salin-->
<div class="modal fade" id="salin_data" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false" data-keyboard="false" data-backdrop="static" >
...
<script type="text/javascript">
	...
	$("#TREKENINGSUB_KODE").change(function(event) {
		...
		var jns = 'SPTPD'
		if ( inArray($("#TREKENINGSUB_KODE").val(), ['4', '8']) ) {
			jns = 'SKPD'
		}
		var elm = `<i class="fa fa-desktop"></i>&nbsp;View Daftar ${jns}`
		$('button[onclick="caribulanan()"]').html(elm)
		$('button[onclick="cetakbulanan()"]').html(elm)
	});

	// $("#nopok").change(function(event) {
	// 	generateAutocompleteWP();
	// });
	...
	function generateAutocompleteWP() {
		...
		$('#nopok').autocomplete({
			serviceUrl: 'pembukuandanpelaporan/cek_data_perwptahunv4/autocompletewpv2',
	...
	function cari() {
		...
		$.ajax({
			url: 'pembukuandanpelaporan/cek_data_perwptahunv4/cari',
			type: 'POST',
			data: {
				rekening: $("#TREKENINGSUB_KODE").val()
				, TBLPENDATAAN_THNPAJAK: ($('#th_pajak').prop('checked') ? $("#masapajak_tahun").val() :'')
				, TBLPENDATAAN_THNPAJAK2: ($('#th_pajak').prop('checked') ? $("#masapajak_tahun2").val() :'')
				, TBLDAFTAR_NOPOK : $("#nopok ").val()
			},
		})
		.done(function(respon) {
			$("#div_tabel").html(respon);
			$('#TBLDAFTAR_BADANUSAHANAMA').html(window.nama);
			$('#TBLDAFTAR_BADANUSAHAALAMAT').html(window.alamat);
		})
		.fail(function() {
			console.log("error");
		})
	...
	function caribulanan() {
		...
		$.ajax({
			url: 'pembukuandanpelaporan/cek_data_perwptahunv4/cari2',
			type: 'POST',
			data: {
				rekening: $("#TREKENINGSUB_KODE").val()
				, TBLPENDATAAN_THNPAJAK: ($('#th_pajak').prop('checked') ? $("#masapajak_tahun").val() :'')
	...
	function cetak2() {
		...
		$.ajax({
			url: 'pembukuandanpelaporan/cek_data_perwptahunv4/cetak',
			type: 'POST',
			data: {
				rekening: $("#TREKENINGSUB_KODE").val()
				, TBLPENDATAAN_THNPAJAK: ($('#th_pajak').prop('checked') ? $("#masapajak_tahun").val() :'')
	...
	// 	}
	// 	);
	// 	window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/cek_data_perwptahunv4/cetak?' + param);
	// }
	...
	function cetak() {
		...
		var TBLPENDATAAN_THNPAJAK = ($('#th_pajak').prop('checked') ? $("#masapajak_tahun").val() :'');
		var TBLPENDATAAN_THNPAJAK2 = ($('#th_pajak').prop('checked') ? $("#masapajak_tahun2").val() :'');
		var TBLDAFTAR_NOPOK = $("#nopok ").val();
		var URL_APLIKASI = "<?php echo Yii::app()->getBaseUrl(1); ?>";
		window.open(URL_APLIKASI+"/pembukuandanpelaporan/cek_data_perwptahunv4/cetak?TBLPENDATAAN_THNPAJAK2="+TBLPENDATAAN_THNPAJAK2+"&TBLPENDATAAN_THNPAJAK="+TBLPENDATAAN_THNPAJAK+"&TBLDAFTAR_NOPOK="+TBLDAFTAR_NOPOK+"&rekening="+rekening);
	...
	function cetakbulanan() {
		...
		var TBLDAFTAR_NOPOK = $("#nopok ").val();
		var URL_APLIKASI = "<?php echo Yii::app()->getBaseUrl(1); ?>";
		window.open(URL_APLIKASI+"/pembukuandanpelaporan/cek_data_perwptahunv4/cetak2?TBLPENDATAAN_THNPAJAK2="+TBLPENDATAAN_THNPAJAK2+"&TBLPENDATAAN_THNPAJAK="+TBLPENDATAAN_THNPAJAK+"&TBLDAFTAR_NOPOK="+TBLDAFTAR_NOPOK+"&rekening="+rekening);
