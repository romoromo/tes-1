				...
				if (respon.model.PAJAKSETOR<500000) {
					...
					if (respon.data.validate=='kurang') {
						...
						if (respon.data.exist_bpd=='ada') {
							notifikasi('Data pernah disetor!', 'data dengan nomor pokok '+$('#nopok').val()+' disetor melalui menu setor pajak langsung. Silahkan gunakan menu <b>Setoran Pajak (BPD)</b>', 'fail', 1,0);
							return false;
						}
						...
						$('#bunga_setor').val(toRp(respon.model.<?php echo $this->namatabel; ?>_BUNGASPTPD));
						$('#dendasetor').val(toRp(respon.model.DENDASETOR));
						// $('#pajaksetor').val(toRp(respon.model.PAJAKSETOR));
