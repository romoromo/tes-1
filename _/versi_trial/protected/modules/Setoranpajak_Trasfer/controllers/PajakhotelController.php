	...
	public function actiongetdata()
	{
		...
		if ($tahun=="") {
			...
			if ($data['exist']=='ada') {
				...
				$bungasetor = "NVL(TBLSETOR_BUNGAKETETAPAN,0) AS bunga_setor";
				...
				$bungasetor = "NVL(" . $kolombunga . ",0) AS bunga_setor";
		...
		public function cekpernahsetorbpd($dataFilter)
		{
			...
			$model = Yii::app()->db->createCommand("
				...
				WHERE 
				TBLPENDATAAN_THNPAJAK = ".(int) substr($dataFilter[':tahun'], -2)."
				AND TBLSETORANBPD_JNSBAYAR IN ('SPTPD','SKPD')
				AND NVL(TBLPENDATAAN_BLNPAJAK, 0) = ".$dataFilter[':bln']." 
				AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$dataFilter[':tgl']." 
				AND NVL(TBLPENDATAAN_PAJAKKE, 0) = ".$dataFilter[':ke']." 
				AND TBLDAFTAR_NOPOK =".$dataFilter[':nopok']." 
		...
		// var_dump($setbpd->params);
			...
			if ($jenis_setoran=='SPTPD' OR $jenis_setoran=='SKPDKB' OR $jenis_setoran == 'SKPD') {
			...
			if ($jenis_setoran=='SPTPD') {
			$TBLSETOR_RUPIAHPAJAK = $pajaksetor;
			$TBLSETOR_RUPIAHSETOR = $pajaksetor;
			$TBLSETOR_RUPIAHTAGIHAN = 0;
			$TBLSETOR_BUNGAKETETAPAN = $bunga_setor;
			...
			$TBLSETOR_RUPIAHTAGIHAN = 0;
			$TBLSETOR_BUNGAKETETAPAN = $bunga_setor;
			$TBLSETOR_DENDAKURANGBAYAR = isset($setbpd[$NAMATABEL . '_KENAIKANKRGBAYAR']) ? $setbpd[$NAMATABEL . '_KENAIKANKRGBAYAR'] : 0;
			$TBLSETOR_DENDAKETETAPAN = (isset($setbpd[$NAMATABEL . '_KENAIKANKRGBAYAR']) ? $setbpd[$NAMATABEL . '_KENAIKANKRGBAYAR'] : 0)+$dendasetor;

			} elseif ($jenis_setoran=='STPD') {
			$TBLSETOR_RUPIAHPAJAK = 0;
			$TBLSETOR_RUPIAHSETOR = $pajaksetor;
			$TBLSETOR_RUPIAHTAGIHAN = $pajaksetor;
			...
			$arrayOfDataInsert = array(
				...
				,'BAKB' => "0"
				,'TBLSETOR_RUPIAHPAJAK' => $TBLSETOR_RUPIAHPAJAK
...
if ($dendasetor>'0') {
	...
	$arrayOfDataInsert['TBLSETOR_DENDAKURANGBAYAR'] =
			$jenis_setoran == 'SKPDKB' ? $TBLSETOR_DENDAKURANGBAYAR : 0;
...
public function actionCetakSSPD()
{
	...
	$from = 'TBLDAFTAR';
		...
		$otherquery = array(
            	...
            	,'join_seto'=>array("TBLSETOR", "{$NAMATABEL}.TBLDAFTAR_NOPOK=TBLSETOR.TBLDAFTAR_NOPOK AND {$NAMATABEL}.TBLPENDATAAN_THNPAJAK=TBLSETOR.TBLPENDATAAN_THNPAJAK AND {$NAMATABEL}.TBLPENDATAAN_BLNPAJAK=TBLSETOR.TBLPENDATAAN_BLNPAJAK")
        	);
		$add = "";
		if ($data['jenis_setoran']=='STPD') {
			...
			$filter = array(
				'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK, 'EQ__' . $NAMATABEL . '.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK, 'EQ__' . $NAMATABEL . '.TBLPENDATAAN_BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK
			);
			$add = "AND TRIM(TBLSETOR.TBLSETOR_JNSBAYAR)='SKPD'";
        } else {
        	$filter = array(
	            'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK
...
public function CetakWordSSPD($data=array())
{
	...
	$namatpl = $path_tpl . 'SSPD-TRANSFER-NONREK.docx';
	$namafileDL = "SSPD-Transfer-Hotel.docx"; 
	...
	$otbs->NoErr = 0;
		...
		if ($jenis_setoran=='STPD' || $jenis_setoran == 'SKPD') {
			...
			$nominal2 = isset($rowWP['TBLSETOR_BUNGAKETETAPAN']) ? $rowWP['TBLSETOR_BUNGAKETETAPAN'] : 0;
		...
		if ($jenis_setoran=='STPD' || $jenis_setoran == 'SKPD' || $jenis_setoran == 'SKPDKB') {
