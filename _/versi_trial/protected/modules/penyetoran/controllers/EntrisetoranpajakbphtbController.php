...
class EntrisetoranpajakbphtbController extends Controller
{
	...
	public function actionautocomplete()
	{
		...
		// $filter_AND = array(
		// 	'EQ__TBLDAFTAR_GOLONGAN' => 1
		// 	,'EQ__REFBADANUSAHA_REKAYAT' => 11
		// );
	...
	public function cekpernahsetor($tahun,$bln,$tgl,$nopok)
	{
		$model = Yii::app()->db->createCommand("
			...
			AND TBLPENDATAAN_BLNPAJAK = ".$bln." 
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".(int)$tgl." 
		...
		if ($exist=='ada') {
			...
			if ($update) {
				...
				$arrayOfData = array(
					...
					'TBLSETORANBPD_DENDAKURANGBAYAR'=>isset($_REQUEST['dendasetor']) && !empty($_REQUEST['dendasetor']) ? $_REQUEST['dendasetor'] : 0,
					'DDRET'=>"",
					'NAIK'=>"",
					'ADM'=>"",
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$namatpl = $path_tpl . 'SSPD-NONREK-BPHTB.docx';
		$namafileDL = "SSPD-BPHTB.docx"; 
		...
		;
		$utama['wp_nama'] = trim($rowWP['TBLDAFTAR_PEMILIKNAMA']);
		$utama['wp_alamat'] = (trim($rowWP['TBLDAFTAR_PEMILIKALAMAT']));
