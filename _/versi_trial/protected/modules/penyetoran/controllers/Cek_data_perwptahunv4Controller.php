...
class Cek_data_perwptahunv4Controller extends Controller
{
	...
	public function getData(){
		...
		// add faktor penghilang denda angsur 1-4-2021 rahmat
		$sql="
		...
		SELECT
			...
			,0 AS JML_ANGSUR,
			NVL(TBLANGSURAN_BEBASPOK,0) AS FAKTORPENGURANGPOK,
			NVL(TBLANGSURAN_BEBASBNG,0) AS FAKTORPENGURANGABNG,
			NVL(TBLANGSURAN_BEBASDND,0) AS FAKTORPENGURANGDND 
		FROM
			TBLSETOR
		...
		";
			...
			SELECT
				...
				AS JML_ANGSUR,
				0 AS FAKTORPENGURANGPOK,
				0 AS FAKTORPENGURANGABNG,
				0 AS FAKTORPENGURANGDND
			FROM
				{$namaTBL}
	...
	public function getDataBulanan(){
		...
		SELECT
			...
			{$namaTBL}.TBLDAFTAR_NOPOK,
			CASE WHEN TBLSETOR.TBLSETOR_NOMORSSPD is not null 
			THEN TBLSETOR.TBLSETOR_NOMORSSPD 
			ELSE TBLSETORANBPD.TBLSETORANBPD_NOMORSSPD
			END
			 AS NOMORSSPD,
			CASE WHEN TBLSETOR.TBLSETOR_THNSETOR is not null 
			THEN TBLSETOR.TBLSETOR_THNSETOR 
			ELSE TBLSETORANBPD.TBLSETORANBPD_THNSETOR
			END
			 AS TBLSETOR_THNSETOR,

			 CASE WHEN TBLSETOR.TBLSETOR_THNSETOR is not null 
			THEN TBLSETOR.TBLSETOR_BLNSETOR 
			ELSE TBLSETORANBPD.TBLSETORANBPD_BLNSETOR
			END
			 AS TBLSETOR_BLNSETOR,
			 CASE WHEN TBLSETOR.TBLSETOR_THNSETOR is not null 
			THEN TBLSETOR.TBLSETOR_TGLSETOR 
			ELSE TBLSETORANBPD.TBLSETORANBPD_TGLSETOR
			END
			 AS TBLSETOR_TGLSETOR,
			...
			NVL(NULL,0) AS BUNGA,
			NVL(NULL,0) AS KENAIKAN,
			CASE WHEN NVL({$namaTBL}.{$namaTBL}_PAJAKSKPD, 0) <> 0 THEN NVL({$namaTBL}.{$namaTBL}_PAJAKSKPD, 0) ELSE NVL({$namaTBL}.{$namaTBL}_PAJAK, 0) END AS PAJAK,
			NVL(NULL,0) AS RPKB,
			TBLSETOR_JNSKETETAPAN,
		...
		LEFT JOIN TBLSETORANBPD ON TBLSETORANBPD.TBLPENDATAAN_THNPAJAK = {$namaTBL}.TBLPENDATAAN_THNPAJAK
		AND NVL(TBLSETORANBPD.TBLPENDATAAN_BLNPAJAK, 0) = NVL({$namaTBL}.TBLPENDATAAN_BLNPAJAK, 0)
		AND NVL(TBLSETORANBPD.TBLPENDATAAN_TGLPAJAK, 0) = NVL({$namaTBL}.TBLPENDATAAN_TGLPAJAK, 0)
		AND NVL(TBLSETORANBPD.TBLPENDATAAN_PAJAKKE, 0) = NVL({$namaTBL}.TBLPENDATAAN_PAJAKKE, 0)
		AND TBLSETORANBPD.TBLDAFTAR_NOPOK = {$namaTBL}.TBLDAFTAR_NOPOK
		AND TBLSETORANBPD.TBLSETORANBPD_REKPENDAPATAN = {$namaTBL}.{$namaTBL}_REKPENDAPATAN
		AND TBLSETORANBPD.TBLSETORANBPD_REKPAD = {$namaTBL}.{$namaTBL}_REKPAD
		AND TBLSETORANBPD.TBLSETORANBPD_REKPAJAK = {$namaTBL}.{$namaTBL}_REKPAJAK
		AND TBLSETORANBPD.TBLSETORANBPD_REKAYAT = {$namaTBL}.{$namaTBL}_REKAYAT
