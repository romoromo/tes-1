...
class EntrisetoranpajakhotelController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$kolomdenda = "".$this->namatabel."_KENAIKANKRGBAYAR";
		...
		if ($tahun=="") {
			...
			if ($data['exist'] == 'adasetbpd') {
				$joinsetbpd = " LEFT JOIN TBLSETORANBPD ON " . $this->namatabel . ".TBLDAFTAR_NOPOK=TBLSETORANBPD.TBLDAFTAR_NOPOK AND ".$this->namatabel.".TBLPENDATAAN_THNPAJAK=TBLSETORANBPD.TBLPENDATAAN_THNPAJAK AND ".$this->namatabel. ".TBLPENDATAAN_BLNPAJAK=TBLSETORANBPD.TBLPENDATAAN_BLNPAJAK AND NVL(" . $this->namatabel . ".TBLPENDATAAN_TGLPAJAK,0) = NVL(TBLSETORANBPD.TBLPENDATAAN_TGLPAJAK,0) AND TBLSETORANBPD_JNSBAYAR = '". $jenis_setoran."'";

				$bungasetor = "NVL(TBLSETORANBPD_BUNGAKETETAPAN,0) AS bungasetor";
				$dendasetor = "NVL(TBLSETORANBPD_DENDAKETETAPAN,0) AS dendasetor";
				$pajaksetor = "NVL(TBLSETORANBPD_RUPIAHSETOR,0) AS pajaksetor";
				$totalsetor = "NVL(TBLSETORANBPD_RUPIAHSETOR,0)+NVL(TBLSETORANBPD_BUNGAKETETAPAN,0)+NVL(TBLSETORANBPD_DENDAKETETAPAN,0) AS totalsetor";
			} else if ($data['exist'] == 'adaseto') {
				$joinseto = " LEFT JOIN TBLSETOR ON " . $this->namatabel . ".TBLDAFTAR_NOPOK=TBLSETOR.TBLDAFTAR_NOPOK AND " . $this->namatabel . ".TBLPENDATAAN_THNPAJAK=TBLSETOR.TBLPENDATAAN_THNPAJAK AND " . $this->namatabel . ".TBLPENDATAAN_BLNPAJAK=TBLSETOR.TBLPENDATAAN_BLNPAJAK AND NVL(" . $this->namatabel . ".TBLPENDATAAN_TGLPAJAK,0) = NVL(TBLSETOR.TBLPENDATAAN_TGLPAJAK,0) AND TRIM(TBLSETOR_JNSBAYAR) = '" . $jenis_setoran . "'";

				$bungasetor = "NVL(TBLSETOR_BUNGAKETETAPAN,0) AS bungasetor";
				$dendasetor = "NVL(TBLSETOR_DENDAKETETAPAN,0) AS dendasetor";
				$pajaksetor = "NVL(TBLSETOR_RUPIAHSETOR,0) AS pajaksetor";
				$totalsetor = "NVL(TBLSETOR_RUPIAHSETOR,0)+NVL(TBLSETOR_BUNGAKETETAPAN,0)+NVL(TBLSETOR_DENDAKETETAPAN,0) AS totalsetor";
			} else {
				$bungasetor = "NVL(".$kolombunga.",0) AS bungasetor";
				$dendasetor = "NVL(".$kolomdenda.",0) AS dendasetor";
				$pajaksetor = "NVL(".$kolompajak.",0) AS pajaksetor";
				$totalsetor = "NVL(".$kolomtotal.",0) AS totalsetor";			
			}
			$sql = "
	...
	{	
		...
		if ($jenis_setoran=='SKPDKB') {
			...
			WHERE 
			TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)."
			AND TRIM(TBLSETORANBPD_JNSBAYAR) = 'SKPDKB'
			AND TBLPENDATAAN_BLNPAJAK = ".$bln." 
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$tgl." 
			AND TBLDAFTAR_NOPOK =".$nopok."
			")->queryRow();
		...
		if ($jenis_setoran=='SPTPD' || $jenis_setoran == 'SKPD') {
			$modelseto = Yii::app()->db->createCommand("
			SELECT COUNT(TBLPENDATAAN_BLNPAJAK) AS JML 
			FROM TBLSETOR
			WHERE 
			TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)."
			AND TRIM(TBLSETOR_JNSBAYAR) IN ('SPTPD','SKPD')
			AND TBLPENDATAAN_BLNPAJAK = ".$bln." 
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$tgl." 
			AND TBLDAFTAR_NOPOK =".$nopok." 
			")->queryRow();
	...
	public function actionsimpandata()
	{
		...
		$bungasetor = isset($_REQUEST['bungasetor']) && !empty($_REQUEST['bungasetor']) ? $_REQUEST['bungasetor'] : 0;
		...
		if ($exist=='ada') {
		...
		}else if ($jenis_setoran=='SPTPD' || $jenis_setoran=='SKPDKB'|| $jenis_setoran == 'SKPD'){
			...
			if ($update) {
				...
				if ($jenis_setoran == 'SPTPD') {
					$TBLSETORANBPD_RUPIAHPAJAK = $pajaksetor;
					$TBLSETORANBPD_RUPIAHSETOR = $pajaksetor;
					$TBLSETORANBPD_BUNGAKETETAPAN = $bungasetor;
					$TBLSETORANBPD_BUNGAKURANGBAYAR = 0;
					$TBLSETORANBPD_DENDAKETETAPAN = 0;
					$TBLSETORANBPD_DENDAKURANGBAYAR = 0;
				} elseif ($jenis_setoran == 'SKPDKB') {
					$TBLSETORANBPD_RUPIAHPAJAK = 0;
					$TBLSETORANBPD_RUPIAHSETOR = $pajaksetor;
					$TBLSETORANBPD_BUNGAKETETAPAN = $bungasetor;
					$TBLSETORANBPD_BUNGAKURANGBAYAR = $bungasetor;
					$TBLSETORANBPD_DENDAKETETAPAN= (isset($model[$this->namatabel . '_KENAIKANKRGBAYAR']) ? $model[$this->namatabel . '_KENAIKANKRGBAYAR'] : 0) + $dendasetor;
					$TBLSETORANBPD_DENDAKURANGBAYAR = isset($model[$this->namatabel . '_KENAIKANKRGBAYAR']) ? $model[$this->namatabel . '_KENAIKANKRGBAYAR'] : 0;
				} elseif ($jenis_setoran == 'SKPD') {
					$TBLSETORANBPD_RUPIAHPAJAK = $pajaksetor;
					$TBLSETORANBPD_RUPIAHSETOR = $pajaksetor;
					$TBLSETORANBPD_BUNGAKETETAPAN = 0;
					$TBLSETORANBPD_BUNGAKURANGBAYAR = 0;
				...
				$arrayOfData = array(
					...
					'BAKB'=>"",
					'TBLSETORANBPD_RUPIAHPAJAK'=> $TBLSETORANBPD_RUPIAHPAJAK,
					...
					'TBLSETORANBPD_BUNGAKETETAPAN'=> $TBLSETORANBPD_BUNGAKETETAPAN,
					'TBLSETORANBPD_BUNGATAGIHAN'=>"",
					'TBLSETORANBPD_BUNGAKURANGBAYAR'=> $TBLSETORANBPD_BUNGAKURANGBAYAR,
					'TBLSETORANBPD_DENDAKETETAPAN'=> $TBLSETORANBPD_DENDAKETETAPAN,
					'TBLSETORANBPD_DENDATAGIHAN'=>"0",
					'TBLSETORANBPD_DENDAKURANGBAYAR'=> $TBLSETORANBPD_DENDAKURANGBAYAR,
					'DDRET'=>$jenis_setoran=='SKPDKB' ? (isset($model[$this->namatabel.'_KENAIKANKRGBAYAR']) ? $model[$this->namatabel.'_KENAIKANKRGBAYAR'] : 0) : 0,
					'NAIK'=>"0",
					'ADM'=>"0",
	...
	public function actionCetakSSPD()
	{
		...
		$data['jenis_setoran'] = !empty(DMOrcl::getRequest('jenis_setoran')) ? base64_decode(base64_decode(DMOrcl::getRequest('jenis_setoran'))) : '';
		...
		} else if ($data['jenis_setoran']=='SKPD') {
	    	$filter = array(
	            'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK
	            ,'EQ__TBLSETORANBPD.TBLSETORANBPD_JNSBAYAR' => 'SKPD'
	        );
        } else {
        	$filter = array(
	            'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$IS_BUNGA = isset($rowWP['TBLSETORANBPD_BUNGAKETETAPAN']) && $rowWP['TBLSETORANBPD_BUNGAKETETAPAN']>0;
		...
		$utama['rincinominal1'] = LokalIndonesia::ribuan($nominal1);
		 // || $jenis_setoran=='SKPDKB'
		if ($jenis_setoran=='STPD' || $jenis_setoran == 'SKPD') {
			$nominal2 = '0';
		...
		} elseif ($jenis_setoran=='STPD' || $jenis_setoran=='SKPD') {
			$utama['rincitotal'] = LokalIndonesia::ribuan($rincitotal = $nominal1+$nominal3);
		} else {
