...
class EntripajakreklameController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$kolomdenda = "0";
			$kolompajak = "".$this->namatabel."_PAJAKSKPD";
			$kolomtotal = "(".$this->namatabel."_BUNGATAGIHAN+".$this->namatabel."_PAJAKSKPD)";
		...
		if ($tahun=="") {
			...
			$data['exist'] = $this->cekpernahsetor($tahun, $bulan, $tgl, $lokasi, $nopok);
			$model = Yii::app()->db->createCommand("SELECT ".$this->namatabel.".*, TBLDAFTAR.*, ".$this->namatabel."_REKURUSAN || '.' || 
							...
							NVL(".$kolombunga.",0) + NVL(".$kolompajak.",0) AS totalsetor
							FROM ".$this->namatabel." 
	...
	public function cekpernahsetor1($tahun,$bln,$nopok,$lokasi)
	{
		$model = Yii::app()->db->createCommand("
			SELECT COUNT(TBLPENDATAAN_BLNPAJAK) AS JML FROM ".$this->namatabel."
			WHERE TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)." AND TBLPENDATAAN_BLNPAJAK = ".$bln." AND TBLDAFTAR_NOPOK =".$nopok." AND ".$this->namatabel."_REGSETOR IS NOT NULL AND NVL(TBLPENDATAAN_PAJAKKE, 0) = {$lokasi} AND ".$this->namatabel."_RUPIAHSETOR > 0")->queryRow();
		if ($model['JML']>0) {
			return 'ada';
		}else{
			return 'tidak';
	...
	public function cekpernahsetor($tahun,$bulan,$tgl,$lokasi,$nopok)
	{
		$model = Yii::app()->db->createCommand("
			SELECT COUNT(TBLPENDATAAN_BLNPAJAK) AS JML FROM ".$this->namatabel."
	...
	public function actionsimpandata()
	{
		...
		$exist = $this->cekpernahsetor1($tahun,$bln,$nopok,$lokasi);
		if ($exist=='ada') {
			...
			if ($update) {
				...
				$arrayOfData = array(
					...
					'TBLSETORANBPD_NOMORSURAT'=>isset($model[$this->namatabel.'_NOSKPD']) ? $model[$this->namatabel.'_NOSKPD'] : '',
					...
					'TBLSETORANBPD_BUNGAKETETAPAN'=>isset($model[$this->namatabel.'_BUNGARUPIAH']) ? $model[$this->namatabel.'_BUNGARUPIAH'] : 0,
		...
		$NAMATABEL = $this->namatabel;
         ...
         $select = "
			...
			(
				...
				AND NVL(TBLSETORANBPD.TBLPENDATAAN_TGLPAJAK, 0) = NVL({$NAMATABEL}.TBLPENDATAAN_TGLPAJAK, 0)
				AND NVL(TBLSETORANBPD.TBLPENDATAAN_PAJAKKE, 0) = NVL({$NAMATABEL}.TBLPENDATAAN_PAJAKKE, 0)
			) AS TBLSETORANBPD_DENDAKETETAPAN
		";
         $from = 'TBLDAFTAR';
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$utama['jenis_bayar'] = $IS_SKPDKB ? 'SKPDKB' : 'SKPD';
