...
class EntrisetoranpajakairtanahController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$kolomtotal = "(".$this->namatabel."_BUNGASPTPD+".$this->namatabel."_PAJAKSKPD)";
		...
		if ($tahun=="") {
			...
			$model = Yii::app()->db->createCommand("SELECT ".$this->namatabel.".*, TBLDAFTAR.*, ".$this->namatabel."_REKURUSAN || '.' || 
							...
							WHERE (TBLPENDATAAN_THNPAJAK='".substr($tahun,-2)."' AND TBLPENDATAAN_BLNPAJAK='".$bln."' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)='".$tgl."') AND ".$this->namatabel.".TBLDAFTAR_NOPOK=".$nopok."
							".$andstpd."
							")
					->queryRow();
		}
	...
	public function actionsimpandata()
	{
		...
		if ($exist=='ada') {
			...
			$update = $command->update($this->namatabel.'', array(
				...
				), 'TBLPENDATAAN_THNPAJAK=:thn AND TBLPENDATAAN_BLNPAJAK=:bln AND NVL(TBLPENDATAAN_TGLPAJAK,0) =:tgl AND TBLDAFTAR_NOPOK=:nopok', array(':thn'=>substr($tahun,-2), ':bln'=>$bln, ':tgl'=>$tgl,':nopok'=>$nopok));
			if ($update) {
				$model = Yii::app()->db->createCommand('SELECT * FROM '.$this->namatabel.' WHERE TBLPENDATAAN_THNPAJAK='.substr($tahun,-2).' AND TBLPENDATAAN_BLNPAJAK='.$bln.' AND NVL(TBLPENDATAAN_TGLPAJAK,0)= '.$tgl.' AND TBLDAFTAR_NOPOK='.$nopok.'')->queryRow();
	...
	public function actionCetakSSPD()
	{
        ...
        $otherquery = array(
            ...
            ,'join_setbpd'=>array("TBLSETORANBPD", "{$NAMATABEL}.TBLDAFTAR_NOPOK=TBLSETORANBPD.TBLDAFTAR_NOPOK AND {$NAMATABEL}.TBLPENDATAAN_THNPAJAK=TBLSETORANBPD.TBLPENDATAAN_THNPAJAK AND {$NAMATABEL}.TBLPENDATAAN_BLNPAJAK=TBLSETORANBPD.TBLPENDATAAN_BLNPAJAK AND NVL({$NAMATABEL}.TBLPENDATAAN_TGLPAJAK,0)=NVL(TBLSETORANBPD.TBLPENDATAAN_TGLPAJAK,0)")
        );
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$namafileDL = "SSPD-Air-Tanah.docx"; 
		...
		$utama['rincirek2'] = '4.1.4.06.08'; //$IS_BUNGA ? '4.1.4.06.01' : '';
