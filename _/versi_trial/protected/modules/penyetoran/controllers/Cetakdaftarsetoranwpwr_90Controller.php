...
class Cetakdaftarsetoranwpwr_90Controller extends Controller
{
	var $MODULE_URL = 'penyetoran/cetakdaftarsetoranwpwr_90';
	...
	public function cetakWordSub($data)
	{
		$path_tpl =  dirname(Yii::app()->basePath) . DIRECTORY_SEPARATOR . 'file' . DIRECTORY_SEPARATOR . 'tpl_penyetoran' . DIRECTORY_SEPARATOR;
		$namatpl = $path_tpl . 'daftar_setoran_wpwr_sub.docx';
		...
		$GLOBALS['tglcetak'] = LokalIndonesia::TanggalUmum(date('Y-m-d'));
		$GLOBALS['tglsetor'] = $GLOBALS['tglsetor'] = $tglsetor = sprintf('%02d', $data['tgl']).'/'.sprintf('%02d', $data['bulan']).'/'.'20'.sprintf('%02d', $data['tahun']);
		...
		foreach ($data['hasil'] as $rowWP) :
			if ($rowWP['TBLSETOR_REKAYAT'] != $AYAT_b4) :
			$AYAT_b4 = $rowWP['TBLSETOR_REKAYAT'];

			$master['no'] = '';
			$master['AYAT'] = $rowWP['TBLSETOR_REKAYAT'];
			$master['detail'] = array();
			...
			if ($row['TBLSETOR_REKAYAT'] == $AYAT_b4) :
			...
			$master['totalrek'] = LokalIndonesia::ribuan($master_setoran);
		...
		$footer=array();
		$footer['totalsetor'] = LokalIndonesia::ribuan($totalsetor);
