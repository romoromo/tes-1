...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i>  Entri Setoran Pajak Air Tanah</h4>
		</div>
	</div>
</div>
								...
								<div id="id_respencarian">
									...
									<div class="row">
										<div class="col-md-6">
											...
											<div  style="padding: 3%;">
												<div class="row">
													<section class="col col-md-2">Tanggal SKPD</section>
													<section class="col col-md-4">
														<label class="input"> <i class="icon-append fa fa-calendar"></i>
															<input readonly=""  style="background: #eee;" type="text" name="<?php echo $this->namatabel; ?>_TGLSKPD" class="datepicker form-control" data-dateformat="dd-mm-yy" id="<?php echo $this->namatabel; ?>_TGLSKPD">
														</label>
													</section>
													<section class="col col-md-1">Batas SKPD</section>
													<section class="col col-md-4">
														<label class="input"> <i class="icon-append fa fa-calendar"></i>
												...
												<div class="row">
													...
													<section class="col col-md-9">
														<label class="input">
															<input class="form-control" style="background: #eee;" id="<?php echo $this->namatabel; ?>_PAJAKSKPD" readonly=""  type="text">
														</label>
													</section>
												</div>
											</div>
										...
										<div class="col-md-6">
											...
											<div  style="padding: 3%;">
												<div class="row">
													...
													<section class="col col-md-4">
														<label class="input">
															<input type="text" style="background: #eee;" class="form-control" readonly=""  value="B" id="<?php echo $this->namatabel; ?>_TIPESETOR">
														</label>
													</section>
												...
												<div class="row">
													...
													<section class="col col-md-10">
														<label class="input" >
															...
															<input class="form-control format-rupiah" style="background: #eee;"  readonly="" id="dendasetor" type="text">
														</label>
													</section>
												</div>
												<div class="row">
												...
												<div class="row">
													...
													<section class="col col-md-10">
														<label class="input">
															...
															<input style="background: #eee;" readonly="" class="form-control  format-rupiah" id="totalsetor" type="text">
														</label>
													</section>
												</div>
											</div>
										...
										<div class="col col-md-12">
											<div class="row">
												...
												<div class="row">
													...
													<section class="col col-md-4">
														<label class="input"> <i class="icon-append fa fa-calendar"></i>
															<input type="text" class="datepicker" value="<?php date('d-m-Y'); ?>" data-dateformat="dd-mm-yy" id="TANGGAL_ENTRY" name="TANGGAL_ENTRY">
														</label>
													</section>
												</div>
												<div class="row">
...
<script type="text/javascript">
	...
	function generateAutocompleteWPHotel() {
		$('#nopok').autocomplete({
			serviceUrl: 'penyetoran/entrisetoranpajakairtanah/autocomplete',
	...
	function cari() {
		...
		$("#btnCetakSSPD").hide();
		$.ajax({
			url: 'penyetoran/entrisetoranpajakairtanah/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun : $('#tahun').val(),
			...
			success:function (respon) {
				if (respon.data.validate=='kurang') {
					...
					if(respon.model.<?php echo $this->namatabel; ?>_THNSKPD) {
						var tahunthnskpd = respon.model.<?php echo $this->namatabel; ?>_THNSKPD;
						var bulanskpd = respon.model.<?php echo $this->namatabel; ?>_BLNSKPD;
						var TGLBATASSKPD = respon.model.<?php echo $this->namatabel; ?>_TGLBATASSKPD;
						if(tahunthnskpd.length==1) {
							var viewtahunthnskpd = '200'+tahunthnskpd;
						} else {
							var viewtahunthnskpd = '20'+tahunthnskpd;
						}
						...
						if(bulanskpd.length==1) {
							var viewbulanskpd = '0'+bulanskpd;
						} else {
							var viewbulanskpd = bulanskpd;
						}					
						$('#<?php echo $this->namatabel; ?>_TGLSKPD').val(viewtglbatasskpd+'-'+bulanskpd+'-'+viewtahunthnskpd);
					} else {
						$('#<?php echo $this->namatabel; ?>_TGLSKPD').val('');
					}

					if(respon.model.<?php echo $this->namatabel; ?>_THNBATASSKPD) {
						var tahunbatasskpd = respon.model.<?php echo $this->namatabel; ?>_THNBATASSKPD;
						var bulanbatasskpd = respon.model.<?php echo $this->namatabel; ?>_BLNBATASSKPD;
						if(tahunbatasskpd.length==1) {
							var viewtahunbatasskpd = '200'+tahunbatasskpd;
						} else {
							var viewtahunbatasskpd = '20'+tahunbatasskpd;
						}
						if(bulanbatasskpd.length==1) {
							var viewbulanbatasskpd = '0'+bulanbatasskpd;
						} else {
							var viewbulanbatasskpd = bulanbatasskpd;
						}					
						$('#<?php echo $this->namatabel; ?>_TGLBATASSKPD').val(respon.model.<?php echo $this->namatabel; ?>_TGLBATASSKPD+'-'+viewbulanbatasskpd+'-'+viewtahunbatasskpd);
					} else {
					...
					if(respon.model.<?php echo $this->namatabel; ?>_PAJAKSKPD) {
						$('#<?php echo $this->namatabel; ?>_PAJAKSKPD').val(toRp(respon.model.<?php echo $this->namatabel; ?>_PAJAKSKPD));
					}
					...
					// $('#pajaksetor').val((respon.model.PAJAKSETOR));
					...
					if (respon.data.exist=='ada') {
						...
						if (jns_setoran=='SKPD' || jns_setoran=='SKPDKB') {
							kolomtglbatas = jns_setoran=='SKPDKB' ? 'TGLBTSKRGBAYAR' : 'TGLBATASSKPD';
	...
	function simpan() {
		...
		if (NOMOR_SSPD=='') {
			...
			$.ajax({
				url: 'penyetoran/entrisetoranpajakairtanah/simpandata',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun : $('#tahun').val(),
	...
	function getrekening(kode) {
		$.ajax({
			url: 'penyetoran/entrisetoranpajakairtanah/getrekening',
			type: 'POST',
			dataType: 'json',
			data: {
				kode : kode,
	...
	function hitungDenda() {
		...
		if (jns_setoran=='SKPD') {
			...
			if (tglbatas=='') {
				tglbatas = $("#<?= $this->namatabel ?>_TGLBATASSKPD").val();
			}
			window.BULAN_DENDAX = getBulanDenda(convDate(tglbatas), '<?= date('Y-m-d') ?>');
			var totalpajak = Number(toAngka($('#<?php echo $this->namatabel; ?>_PAJAKSKPD').val()));
			// rpdenda = Number(toAngka($("#<?php echo $this->namatabel; ?>_DENDAKRGBAYAR").val()));
			totalbayar = Number(toAngka($("#<?php echo $this->namatabel; ?>_PAJAKSKPD").val()));
			rpdenda_tambahan = ((BULAN_DENDAX * 2) /100) * totalpajak;
			// rpdenda_setor = rpdenda + rpdenda_tambahan;
			rpdenda_setor = rpdenda_tambahan;
			// $("#dendasetor").val(rpdenda_setor.toFixed(2));
			// $("#totalsetor").val((rpdenda_setor + totalbayar).toFixed(2));
			$("#dendasetor").val(rpdenda_setor.toFixed(0));
			//$("#totalsetor").val((rpdenda_setor + totalbayar).toFixed(0));
			setPriceFormat();
	...
	function hitungDenda2() {
		...
		if (jns_setoran=='SKPD') {
			...
			pokok = Number(toAngka($('#<?php echo $this->namatabel; ?>_PAJAKSKPD').val()));

			//tglbts = moment(new Date(tglbts1)).format("DD-MM-YYYY");
	...
	function hitungsetor(){
		...
		if (jns_setoran=='SKPD') {
			...
			totalbayar =  Number(toAngka($("#<?= $this->namatabel ?>_BUNGATAGIHAN").val()));
			// $("#dendasetor").val(0);
			$("#totalsetor").val(toRp(totalbayar));
			// setPriceFormat();
		}
	}
	...
	function cetakSSPD() {
		...
		window.open('penyetoran/entrisetoranpajakairtanah/cetakSSPD/?'+jQuery.param(param));
	}
