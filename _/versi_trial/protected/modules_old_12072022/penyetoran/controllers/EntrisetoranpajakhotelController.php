...
class EntrisetoranpajakhotelController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$andjoinstpd = "JOIN TBLSTPD ON TBLSTPD.TBLPENDATAAN_THNPAJAK = ".$this->namatabel.".TBLPENDATAAN_THNPAJAK AND TBLSTPD.TBLPENDATAAN_BLNPAJAK = ".$this->namatabel.".TBLPENDATAAN_BLNPAJAK AND NVL(TBLSTPD.TBLPENDATAAN_TGLPAJAK,'0') = NVL(".$this->namatabel.".TBLPENDATAAN_TGLPAJAK,'0') AND TBLSTPD.TBLDAFTAR_NOPOK = ".$this->namatabel.".TBLDAFTAR_NOPOK ";
			$andstpd = "AND TBLSTPD_JENIS = 'STPD'";
			$kolombunga = "0";
			$kolomdenda = "0";
			$kolompajak = "TBLSTPD_BUNGATAGIHAN";
			$kolomtotal = "TBLSTPD_BUNGATAGIHAN";
		} else if($jenis_setoran=='STPD-KB') {
			$andjoinstpd = "JOIN TBLSTPD ON TBLSTPD.TBLPENDATAAN_THNPAJAK = ".$this->namatabel.".TBLPENDATAAN_THNPAJAK AND TBLSTPD.TBLPENDATAAN_BLNPAJAK = ".$this->namatabel.".TBLPENDATAAN_BLNPAJAK AND NVL(TBLSTPD.TBLPENDATAAN_TGLPAJAK,'0') = NVL(".$this->namatabel.".TBLPENDATAAN_TGLPAJAK,'0') AND TBLSTPD.TBLDAFTAR_NOPOK = ".$this->namatabel.".TBLDAFTAR_NOPOK ";
			$andstpd = "AND TBLSTPD_JENIS = 'STPD-KB'";
			$kolombunga = "0";
			$kolomdenda = "0";
			$kolompajak = "TBLSTPD_BUNGATAGIHAN";
			$kolomtotal = "TBLSTPD_BUNGATAGIHAN";	
	...
	{	
		...
		if ($jenis_setoran=='STPD-KB') {
			$model = Yii::app()->db->createCommand("
			SELECT COUNT(TBLPENDATAAN_BLNPAJAK) AS JML 
			FROM TBLSETORANBPD
			WHERE 
			TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)."
			AND TBLSETORANBPD_JNSBAYAR = 'STPD-KB'
			AND TBLPENDATAAN_BLNPAJAK = ".$bln." 
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$tgl." 
			AND TBLDAFTAR_NOPOK =".$nopok."
			")->queryRow();
	...
	public function actionsimpandata()
	{
		...
		if ($exist=='ada') {
			...
			if ($update) {
				...
				$arrayOfData = array(
					...
					'BURET'=>"0",
					'TBLSETORANBPD_DENDAKETETAPAN'=>$jenis_setoran=='SKPDKB' ? ((isset($model[$this->namatabel.'_KENAIKANKRGBAYAR']) ? $model[$this->namatabel.'_KENAIKANKRGBAYAR'] : 0)+$dendasetor) : 0,
		...
		}else if ($jenis_setoran=='STPD' || $jenis_setoran=='STPD-KB'){ //TBLSTPD
				$command = Yii::app()->db->createCommand();
				...
				$model = Yii::app()->db->createCommand("SELECT ".$this->namatabel.".*,TBLSTPD_RUPIAHTAGIHAN,TBLSTPD_BUNGATAGIHAN FROM ".$this->namatabel." JOIN TBLSTPD ON TBLSTPD.TBLPENDATAAN_THNPAJAK = ".$this->namatabel.".TBLPENDATAAN_THNPAJAK AND TBLSTPD.TBLPENDATAAN_BLNPAJAK = ".$this->namatabel.".TBLPENDATAAN_BLNPAJAK AND NVL(TBLSTPD.TBLPENDATAAN_TGLPAJAK,0) = NVL(".$this->namatabel.".TBLPENDATAAN_TGLPAJAK,0) AND TBLSTPD.TBLDAFTAR_NOPOK = ".$this->namatabel.".TBLDAFTAR_NOPOK 
					WHERE ".$this->namatabel.".TBLPENDATAAN_THNPAJAK=".substr($tahun,-2)." 
					AND ".$this->namatabel.".TBLPENDATAAN_BLNPAJAK=".$bln." 
					AND NVL(".$this->namatabel.".TBLPENDATAAN_TGLPAJAK, 0)=".$tgl." 
					AND ".$this->namatabel.".TBLDAFTAR_NOPOK=".$nopok."
					AND TBLSTPD_JENIS LIKE '".$jenis_setoran."'")->queryRow();
				print_r($model);die();
				$command_insrt = Yii::app()->db->createCommand();
				$arrayOfData = array(
					'TBLPENDATAAN_THNPAJAK'=>substr($tahun,-2),
					...
					'TBLSETORANBPD_REKPENDAPATAN'=>isset($model[$this->namatabel.'_REKPENDAPATAN']) ? $model[$this->namatabel.'_REKPENDAPATAN'] : '',
					'TBLSETORANBPD_REKPAD'=>isset($model[$this->namatabel.'_REKPAD']) ? $model[$this->namatabel.'_REKPAD'] : '',
					'TBLSETORANBPD_REKPAJAK'=>'4',
					'TBLSETORANBPD_REKAYAT'=>'7',
					'TBLSETORANBPD_REKJENIS'=>isset($model[$this->namatabel.'_REKAYAT']) ? $model[$this->namatabel.'_REKAYAT'] : '',
					'TBLSETORANBPD_REKSUBJENIS'=>'',
					'TBLSETORANBPD_GOLONGAN'=>isset($model['TBLDAFTAR_GOLONGAN']) ? $model['TBLDAFTAR_GOLONGAN'] : '',
					//belum tahu di isi apa
					...
					'BAKB'=>"",
					'TBLSETORANBPD_RUPIAHPAJAK'=>isset($model['TBLSTPD_RUPIAHTAGIHAN']) ? $model['TBLSTPD_RUPIAHTAGIHAN'] : 0,
					'TBLSETORANBPD_RUPIAHSETOR'=>isset($model['TBLSTPD_BUNGATAGIHAN']) ? $model['TBLSTPD_BUNGATAGIHAN'] : 0,
					'TBLSETORANBPD_KETETAPANPAJAK'=>"0",
					// 'TBLSETORANBPD_KETETAPANPAJAK'=>($jenis_setoran=='SKPDKB' &&  ? $pajak : (isset($model[$this->namatabel.'_PAJAK']) ? $model[$this->namatabel.'_PAJAK'] : 0)),
					// 'TBLSETORANBPD_RUPIAHSETOR'=>$pajaksetor,
					'RPRET'=>"",
					// 'TBLSETORANBPD_KETETAPANPAJAK'=>$pajaksetor,
					'TBLSETORANBPD_RUPIAHTAGIHAN'=>isset($model['TBLSTPD_BUNGATAGIHAN']) ? $model['TBLSTPD_BUNGATAGIHAN'] : 0,
					...
					'NAIK'=>"0",
					'ADM'=>"0",
					'RPSSKP'=>isset($model['TBLSTPD_RUPIAHTAGIHAN']) ? $model['TBLSTPD_RUPIAHTAGIHAN'] : 0,
					'RPSSTP'=>"0",
					'RPSKB'=>"0",
					'RPSRET'=>"0",
					'UK1'=>"0",
	...
	public function actionCetakSSPD()
	{
	    ...
	    } elseif ($data['jenis_setoran']=='STPD-KB') {
        	$filter = array(
	            'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK
	            ,'EQ__TBLSETORANBPD.TBLSETORANBPD_JNSBAYAR' => 'STPD-KB'
	        );
	...
	public function CetakWordSSPD($data=array())
	{
		...
		} elseif ($jenis_setoran=='STPD' || $jenis_setoran=='STPD-KB') {
			$nominal1 = '0';
		} else {
		...
		if ($jenis_setoran=='STPD' || $jenis_setoran=='STPD-KB') {
			$nominal2 = '0';
		...
		if ($jenis_setoran=='SKPDKB') {
			...
			$nominal3 = isset($rowWP['TBLSETORANBPD_RUPIAHSETOR']) ? $rowWP['TBLSETORANBPD_RUPIAHSETOR'] :0;
		} else {
			$nominal3 = '0';
