...
class EntrisetoranpajakrestoranController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$andstpd  	= "AND ".$this->namatabel."_NOSURATTAGIHAN > 0";
			$kolombunga = "".$this->namatabel."_BUNGATAGIHAN";
			$kolomdenda = "0";
			$kolompajak = "".$this->namatabel."_PAJAK";
			$kolomtotal = "".$this->namatabel."_RUPIAHTAGIHAN";
		...
		if($jenis_setoran=='SKPDKB'){
			$data['ang'] = $this->cekpernahangsur(substr($tahun,-2),$bln,$tgl,$nopok, $jenis_setoran);	
		}
		if ($tahun=="") {
			...
			$data['exist'] = $this->cekpernahsetor(substr($tahun,-2),$bln,$tgl,$nopok, $jenis_setoran);
			$sql = "
	...
	public function cekpernahsetor($tahun,$bln,$tgl,$nopok, $jenis_setoran='')
	{
		...
		if ($jenis_setoran=='SKPDKB') {
			...
			WHERE 
			TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)."
			AND TBLSETORANBPD_JNSKETETAPAN = 'K'
			AND TBLPENDATAAN_BLNPAJAK = ".$bln." 
			AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$tgl." 
			AND TBLDAFTAR_NOPOK =".$nopok."
			")->queryRow();
		...
		if ($exist=='ada') {
			...
			if ($update) {
				...
				$arrayOfData = array(
					...
					'BAKB'=>"",
					'TBLSETORANBPD_RUPIAHPAJAK'=>$pajaksetor>0 && $jenis_setoran=='SKPDKB' ? $pajaksetor : (isset($model[$this->namatabel.'_PAJAK']) ? $model[$this->namatabel.'_PAJAK'] : 0),
					'TBLSETORANBPD_RUPIAHSETOR'=>$pajaksetor>0 && $jenis_setoran=='SKPDKB' ? $pajaksetor : (isset($model[$this->namatabel.'_PAJAK']) ? $model[$this->namatabel.'_PAJAK'] : 0),
				...
				$arrayOfData = array(
					...
					'RPRET'=>"",
					// 'TBLSETORANBPD_KETETAPANPAJAK'=>$pajaksetor,
					'TBLSETORANBPD_RUPIAHTAGIHAN'=>isset($model[$this->namatabel.'_PAJAK']) ? $model[$this->namatabel.'_PAJAK'] : 0,
					'TBLSETORANBPD_PAJAKKURANGBAYAR'=>"",
					'TBLSETORANBPD_BUNGAKETETAPAN'=>"",
					'TBLSETORANBPD_BUNGATAGIHAN'=>isset($model[$this->namatabel.'_BUNGATAGIHAN']) ? $model[$this->namatabel.'_BUNGATAGIHAN'] : 0,
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$IS_DENDA = isset($rowWP['TBLSETORANBPD_DENDAKURANGBAYAR']) && $rowWP['TBLSETORANBPD_DENDAKURANGBAYAR']>0;
		...
		if ($jenis_setoran=='STPD') {
			$nominal2 = $rowWP['TBLSETORANBPD_BUNGATAGIHAN'];
		} else {
		...
		$nominal3 = $IS_DENDA ? $rowWP['TBLSETORANBPD_DENDAKURANGBAYAR'] : 0;
		$utama['rincino3'] = '3';
