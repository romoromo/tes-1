...
<table width="100%" border="0" style="border-collapse:collapse;" cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed table-hover smart-form has-tickbox" >
	...
	<tbody>
		...
		<tr>
			...
			<td><?php 
			if ($list[$data['NamaTBL'].'_REGSURATANGSUR']=='') {
				echo 'KB';
			} else {
				echo 'A';
			}
			...
			<td><?php if ($list[$data['NamaTBL'].'_REGSURATANGSUR']=='') {
				echo LokalIndonesia::ribuan($totaltunggakan2);
			} else {
				echo LokalIndonesia::ribuan($totaltunggakan);
			}
			?></td> <!-- Tunggakan -->
		</tr>
	<?php endforeach ?>
