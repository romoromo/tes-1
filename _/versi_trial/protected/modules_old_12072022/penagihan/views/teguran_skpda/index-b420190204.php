<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Teguran SKPD A </h4>
		</div>
	</div>
</div>
							...
							<div class="row">
								...
								<section class="col col-md-6">
									...
									<div class="row">
										...
										<section class="col col-md-7">
											<label class="input">
												<input class="input-sm" type="text" id="waktu" name="waktu">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-9">
											<label class="textarea"> 										
												<textarea rows="3" name="tempt_undangan">Ruang Sub Bid Penagihan dan Keberatan Pendapatan Daerah.Gedung Dinas Penanaman Modal dan Perizinan Lt.2 Kota Yogyakarta.</textarea> 
											</label>
										</section>
									</div>
								</section>
...
function cari() {
	...
	$.ajax({
		url: 'penagihan/Teguran_skpda/cari',
		type: 'POST',
		data: {
	...
	// }
    	...
    	if ($('#nomor_surat').val()=='') {
			...
			var q = daftaridPajak;
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/penagihan/Teguran_skpda/cetakword/?data=' + q +"&TBLPENDATAAN_THNPAJAK="+$('#TBLPENDATAAN_THNPAJAK').val()+"&TBLDAFTAR_NOPOK="+$('#TBLDAFTAR_NOPOK').val()+"&TBLREKENING_KODE="+$('#TBLREKENING_KODE').val()+"&nomor_surat="+$('#nomor_surat').val()+"&tglawal="+$('#tglawal').val()+"&tglakhir="+$('#tglakhir').val()+"&waktu="+$('#waktu').val() );
    	}    	
		...
		function cetakdaftar() {
			...
			var q = daftaridPajak;
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/penagihan/Teguran_skpda/cetakdaftar/?data=' + q +"&TBLPENDATAAN_THNPAJAK="+$('#TBLPENDATAAN_THNPAJAK').val()+"&TBLDAFTAR_NOPOK="+$('#TBLDAFTAR_NOPOK').val()+"&TBLREKENING_KODE="+$('#TBLREKENING_KODE').val()+"&nomor_surat="+$('#nomor_surat').val()+"&tglawal="+$('#tglawal').val()+"&tglakhir="+$('#tglakhir').val()+"&waktu="+$('#waktu').val() );
    	}    	
    	// window.open(window.APP_URL+'/pendataan/cetak_suratteguran/cetaksuratteguran');
