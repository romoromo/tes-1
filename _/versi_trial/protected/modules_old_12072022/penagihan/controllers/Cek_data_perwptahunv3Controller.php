...
class Cek_data_perwptahunv3Controller extends Controller
{
	...
	public function getData(){
		...
		SELECT
			'SPT BULANAN' AS TIPE,
			{$namaTBL}.{$namaTBL}_THNENTRI AS {$namaTBL}_THNSKPD,
			{$namaTBL}.{$namaTBL}_BLNENTRI AS {$namaTBL}_BLNSKPD,
			{$namaTBL}.{$namaTBL}_TGLENTRI AS {$namaTBL}_TGLSKPD,
			{$namaTBL}.{$namaTBL}_THNBATASSKPD AS THNBATASSKPD,
			{$namaTBL}.{$namaTBL}_BLNBATASSKPD AS BLNBATASSKPD,
			{$namaTBL}.{$namaTBL}_TGLBATASSKPD AS TGLBATASSKPD,
			...
			NVL({$namaTBL}.TBLPENDATAAN_BLNPAJAK, 0) AS BLP2,
			...
			CONCAT({$jnsbayar}, {$namaTBL}.TBLPENDATAAN_THNPAJAK) AS TBLSETOR_JNS2,
		...
		SELECT
			...
			NVL(TBLANGSURAN.TBLANGSURAN_PAJAKKE, 0) AS BLP2,
			...
			CONCAT('SKPD-A', TBLANGSURAN.TBLPENDATAAN_BLNPAJAK) AS TBLSETOR_JNS2,
		...
		if ($rek=='4') {
			...
			SELECT
				...
				CONCAT('SKPDKB', {$namaTBL}_THNSKPD) AS TBLSETOR_JNS2,
				TBLSETOR.TBLSETOR_RUPIAHSETOR,
			...
			SELECT
				...
				{$namaTBL}.{$namaTBL}_TGLKURANGBAYAR,
				{$namaTBL}.TBLPENDATAAN_THNPAJAK,
				NVL({$namaTBL}.{$namaTBL}_BLNKBAWAL, 0) AS BLP,
				...
				AND TBLANGSURAN.TBLPENDATAAN_THNPAJAK='{$masapajak_tahun}'
				GROUP BY TBLANGSURAN.TBLPENDATAAN_BLNPAJAK)
				WHERE ROWNUM = 1
				) AS JML_ANGSUR
			FROM
				{$namaTBL}
