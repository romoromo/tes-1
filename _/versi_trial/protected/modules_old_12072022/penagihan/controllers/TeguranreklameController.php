...
//}
	...
	public function actionCari()
	{
		...
		$TBLDAFTREKLAME_BLNSKPDA = !empty(DMOrcl::getRequest('TBLDAFTREKLAME_BLNSKPD')) ? DMOrcl::getRequest('TBLDAFTREKLAME_BLNSKPD') : '';
		$TBLDAFTREKLAME_BLNSKPDK = !empty(DMOrcl::getRequest('TBLDAFTREKLAME_BLNSKPD')) ? DMOrcl::getRequest('TBLDAFTREKLAME_BLNSKPD') : '';
		...
		if($TBLDAFTREKLAME_BLNSKPDA==''){
			...
			$BLNSKPDA = "AND TBLDAFTREKLAME_BLNSKPD >= ".$BLNSKPDA."";
		}
		...
		if($TBLDAFTREKLAME_BLNSKPDK==''){
			...
			$BLNSKPDK = "AND TBLDAFTREKLAME_BLNSKPD <= ".$BLNSKPDK."";
		}
...
public function actioncetakWordReklame()
{
	...
	$otbs->NoErr = 0;
		...
		foreach ($data['cetak'] as $kolom) :
				...
				$utama['noo'] = null;
				$utama['ket2'] = trim($kolom['TBLDAFTAR_BADANUSAHANAMA']);
				$utama['no'] = $no++;
