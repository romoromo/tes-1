...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i>  Entri Setoran Transfer Pajak Penerangan Jalan</h4>
		</div>
	</div>
</div>
					...
					<form action="" id="form_sumber_pajak" class="smart-form" novalidate>
						<fieldset>
							...
							<div class="row" style="display: none;">
								...
								<section class="col col-md-2">
									<label class="select">
										<select name="idgolongan_tarif" id="idgolongan_tarif" class="select2">
											<option disabled="">== Silahkan Pilih ==</option>
											<?php foreach ($data['jns_pajak'] as $jenis): ?>
												<option value="<?php echo $jenis['REFGOLTARIFPJU_ID']; ?>" tarif="<?php echo $jenis['REFGOLTARIFPJU_TARIF']; ?>"> <?php echo $jenis['REFGOLTARIFPJU_NAMA']; ?></option>
											<?php endforeach ?>
										</select>
									</label>
								</section>
									...
									<div class="col-md-6">
										...
										<div  style="padding: 3%;">
											...
											<div class="row">
												...
												<section class="col col-md-10">
													<label class="input" >
														<input class="form-control disabled format-rupiah-desimal" style="background: #eee;" name="denda_setor" id="denda_setor" type="text">
													</label>
												</section>
											</div>
											<div class="row">
...
<script type="text/javascript">
	...
	function cari() {
		...
		$("#btnCetakSSPD").hide();
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajakppj/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun : $('#tahun').val(),
				...
				// idgolongan_tarif : $('#idgolongan_tarif').select2('val'),
				nopok : $('#nopok').val(),
			},
			success:function (respon) {
				if (respon.data.validate=='kurang') {
					...
					$('#bunga_setor').val(toRp(respon.model.BUNGASETOR));
...
function simpan() {
	...
	if (NOMOR_SSPD=='') {
		...
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajakppj/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun : $('#tahun').val(),
				...
				pajaksetor : $('#pajaksetor').val(),
				bunga_setor : $('#bunga_setor').val(),
...
function cetakSSPD() {
	var param = {
		...
		//,TBLPENDATAAN_PAJAKKE: btoa(btoa($('#idgolongan_tarif').select2('val')))
	}
	window.open('Setoranpajak_Trasfer/pajakppj/cetakSSPD/?'+jQuery.param(param));
}
