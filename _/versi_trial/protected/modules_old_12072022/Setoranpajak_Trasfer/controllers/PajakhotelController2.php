...
public function actionCetakSSPD()
{
	...
	{$NAMATABEL}_REGSETOR AS TBLSETOR_NOMORSSPD
	";
	$from = 'TBLDAFTAR';
    ...
    if ($data['jenis_setoran']=='STPD' || $data['jenis_setoran']=='SKPDKB') {
		$otherquery['andwhere'] = '
		NVL(TBLSETOR.TBLPENDATAAN_TGLPAJAK, 0)='.$TBLPENDATAAN_TGLPAJAK.'
...
public function CetakWordSSPD($data=array())
{
	...
	$otbs->NoErr = 0;
		...
		// $nominal1 = $rowWP[$NAMATABEL.'_RUPIAHSETOR'];
		$utama['rincino1'] = '1';
		$KODEAYAT = sprintf('%02d', $rowWP['TBLSETOR_REKAYAT']);
		$KODEJENIS = sprintf('%02d', $rowWP['TBLSETOR_REKJENIS']);
