					  ...
					  <table id="dt_basic" class="table table-striped table-bordered table-hover" width="100%">
						  <thead>			                
							  <tr>
								  ...
								  <th data-class="expand"><div align="center"><i class=" text-muted hidden-md hidden-sm hidden-xs"></i>Nama Syarat </div></th>
							      <th >&nbsp;</th>
							  </tr>
						  </thead>						  
						  <tbody>
						  		<?php $no=1; foreach ($datapersyaratan as $persyaratan): ?> 
							  <tr>
								  <td><?php echo $no++; ?></td>
								  <td><?php echo $persyaratan['tblpersyaratan_nama']; ?></td>
							      <td>
							     
								    	<button onclick="edit(<?php echo $persyaratan['tblpersyaratan_id']; ?>)" type="button" class="btn btn-labeled btn-success btn-sm" style="width:65px; height:30px;">
								    		
								    			<i class="fa fa-edit"></i>
								    		Edit
								    	</button>

								    	<button  onclick="hapus(<?php echo $persyaratan['tblpersyaratan_id']; ?>)" type='button' class='btn btn-labeled btn-danger btn-sm' style="width:65px; height:30px;">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel"><i class="fa fa-check"></i>&nbsp;Form Tambah Syarat</h4>
			</div>
			<form action="" id="form_syarat" class="smart-form" novalidate>
					...
					<div class="row">
						...
						<section class="col col-6">
							<label class="textarea textarea-resizable"> 										
								<textarea style="height: 150px;" rows="3" class="custom-scroll" id="nama_syarat" name="nama_syarat"></textarea> 
							</label>
						</section>
					</div>
...
<script type="text/javascript">
	...
	function runFormValidation() {
		var $orderForm = $("#form_syarat").validate({
				...
				messages : {
					nama_syarat : {
						required : 'Mohon isikan Nama Syarat'
					}
				},
...
function simpan () {
		//$("#eg7").click();
		$.ajax({
			url: 'app/refpersyaratan/simpan',
			type: 'POST',
			data: {
				cmd: window.cmd,
				id: window.id,
				nama_syarat: $('#nama_syarat').val(),
			}
...
function edit(id) {
	...
	window.cmd = "edit";
	$.ajax({
		url: 'app/refpersyaratan/GetDataSyarat',
		type: 'POST',
		data: {id: id},
		success : function (respon) {
			$("#nama_syarat").val(respon.split('||')[1]);
		}
	});
...
function hapus(id) {
	...
	$.SmartMessageBox({
		title : "Konfirmasi",
		content : "Apakah anda yakin akan menghapus Persyaratan ini?",
		buttons : '[Tidak][Ya]'
	}, function(ButtonPressed) {
		if (ButtonPressed === "Ya") {
			$.ajax({
				url: 'app/refpersyaratan/HapusDataSyarat',
				type: 'POST',
				data: {id: id},
				success: function  (respon) {
					if (respon=='success') 
