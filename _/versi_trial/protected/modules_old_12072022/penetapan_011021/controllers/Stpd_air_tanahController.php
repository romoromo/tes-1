...
class Stpd_air_tanahController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if ($bayar > 0) {
			...
			$filter_AND = array(
				...
				,'LK__TBLSTPD.TBLSTPD_JENIS' => $jenis
			);
			...
			$data['TBLDAFTTANAH_TGLSPTPD'] = sprintf("%02d",$model['TBLDAFTTANAH_TGLSPTPD']); 
			$data['TBLDAFTTANAH_BLNSPTPD'] = sprintf("%02d",$model['TBLDAFTTANAH_BLNSPTPD']);
			$data['TBLDAFTTANAH_THNSPTPD'] = $model['TBLDAFTTANAH_THNSPTPD'];
	...
	public function cekProses($jenis, $tahun, $bulan, $TBLDAFTAR_NOPOK)
	{ 
		// if ($jenis == 'SPTPD') {
		if (strtoupper($jenis) == 'SKPD') {
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLSTPD_BUNGATAGIHAN,0) FROM TBLSTPD WHERE TBLPENDATAAN_THNPAJAK =".$tahun." AND TBLPENDATAAN_BLNPAJAK=".$bulan." AND TBLDAFTAR_NOPOK=".$TBLDAFTAR_NOPOK." AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = 0 AND TBLSTPD_JENIS LIKE 'STPD'")->queryScalar();

		} elseif ($jenis == 'SKPDKB') {
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLSTPD_BUNGATAGIHAN,0) FROM TBLSTPD WHERE TBLPENDATAAN_THNPAJAK =".$tahun." AND TBLPENDATAAN_BLNPAJAK=".$bulan." AND TBLDAFTAR_NOPOK=".$TBLDAFTAR_NOPOK." AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = 0 AND TBLSTPD_JENIS LIKE 'STPD-KB'")->queryScalar();
		}
		return $model;
	}
	...
	{
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLSTPD_NOMORSSPD,0) AS JML FROM TBLSTPD WHERE TBLPENDATAAN_THNPAJAK =".$THNPAJAK." AND TBLPENDATAAN_BLNPAJAK=".$BLNPAJAK." AND TBLDAFTAR_NOPOK=".$NOPOKOK." AND TBLSTPD_JENIS LIKE '".$jenis."'")->queryScalar();
		return $model;
	}
