...
class Skpdnihil_abtController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 8;
	var $PAJAK_REK = '4.1.1.8.0';

	public $namatabel = 'TBLDAFTTANAH';
	...
	public function actionGetNoSKPDNihil()
	{
		...
		$data = Yii::app()->db->createCommand("SELECT
			MAX(TO_NUMBER (TO_CHAR(NVL(TBLDAFTTANAH_REGNIHIL, 0))))+1 AS nolb
		FROM
			TBLDAFTTANAH
		WHERE
			TBLPENDATAAN_THNPAJAK = ".$splthn."
		AND NVL (TBLDAFTTANAH_REGNIHIL, 0) <> 0
		ORDER BY
	...
	public function actiongetdata()
	{
		...
		NVL(TBLDAFTTANAH.TBLDAFTTANAH_REGNIHIL, 0) AS TBLDAFTTANAH_REGNIHIL,
		...
		NVL(TBLDAFTTANAH.TBLDAFTTANAH_RUPIAHPERIKSA,0) AS TBLDAFTTANAH_RUPIAHPERIKSA,
		NVL(TBLDAFTTANAH.TBLDAFTTANAH_TANGGLNIHIL,0) AS TBLDAFTTANAH_TANGGLNIHIL,
		NVL(TBLDAFTTANAH.TBLDAFTTANAH_BULANNIHIL,0) AS TBLDAFTTANAH_BULANNIHIL,
		NVL(TBLDAFTTANAH.TBLDAFTTANAH_TAHUNNIHIL,0) AS TBLDAFTTANAH_TAHUNNIHIL 
		";
		$from = 'TBLDAFTAR';
		$filter = array(
		...
		$filter_AND = array(
			...
			,"EQ__TBLDAFTTANAH.TBLPENDATAAN_BLNPAJAK" => $bulan_akhir
		);

		$otherquery = array(
			'limit'=> 30
			,'andwhere'=> 'NVL(TBLDAFTTANAH.TBLDAFTTANAH_REGNIHIL, 0) <> 0'
		...
		$data['TBLDAFTTANAH_RUPIAHPERIKSA'] = $model['TBLDAFTTANAH_RUPIAHPERIKSA'];
		$data['TBLDAFTTANAH_REGNIHIL'] = $model['TBLDAFTTANAH_REGNIHIL'];
		...
		$data['TBLDAFTTANAH_TANGGLNIHIL'] = sprintf('%02d',$model['TBLDAFTTANAH_TANGGLNIHIL']); 
		$data['TBLDAFTTANAH_BULANNIHIL'] = sprintf('%02d',$model['TBLDAFTTANAH_BULANNIHIL']); 
		$data['TBLDAFTTANAH_TAHUNNIHIL'] = $model['TBLDAFTTANAH_TAHUNNIHIL']; 
		...
		$filter_AND = array(
			...
			,"EQ__TBLDAFTTANAH.TBLPENDATAAN_THNPAJAK" => $tahun_substr
			,"EQ__TBLDAFTTANAH.TBLPENDATAAN_BLNPAJAK" => $bulan_akhir
			// ,"EQ__TBLDAFTTANAH.TBLDAFTTANAH_BLNKBAKHIR" => $bulan_akhir
			// ,'EQ__tblsubyek_isaktif' => "T"
		);
	...
	public function actionCetakWord()
	{
		...
		$dt['thnkb'] = '20'.$data['cetak']['TBLDAFTTANAH_TAHUNNIHIL'];
		$dt['blnawal'] = LokalIndonesia::getbulan($data['cetak']['TBLDAFTTANAH_BLNKBAWAL']);
		$dt['blnakhir'] = LokalIndonesia::getbulan($data['cetak']['TBLDAFTTANAH_BLNKBAKHIR']);
		$dt['regkb'] = $data['cetak']['TBLDAFTTANAH_REGNIHIL'];
		...
		$dt['pajakperiksa'] = LokalIndonesia::rupiah($data['cetak']['TBLDAFTTANAH_RUPIAHPERIKSA']);
		...
		$namafileDL = "SKPDNIHIL-AIRTANAH-".$npwpd.".docx";
	...
	{
		$model = Yii::app()->db->createCommand("SELECT NVL(TBLDAFTTANAH_REGNIHIL,0) FROM $this->namatabel WHERE TBLPENDATAAN_THNPAJAK ='$thn' AND TBLPENDATAAN_BLNPAJAK='$bulan' AND TBLDAFTAR_NOPOK='$nopok' AND NVL(TBLDAFTTANAH_NOMORPERIKSA, 0) <> 0")->queryScalar();
		return $model;
	}

	public function cekBayar($thn, $bulan, $nopok)
	{
		$model = Yii::app()->db->createCommand("SELECT TBLSETOR_NOMORSSPD FROM TBLSETOR WHERE TBLPENDATAAN_THNPAJAK ='$thn' AND TBLPENDATAAN_BLNPAJAK='$bulan' AND NVL(TBLPENDATAAN_TGLPAJAK, 0)=0 AND TBLDAFTAR_NOPOK='$nopok' AND TRIM(TBLSETOR_JNSBAYAR) = 'SKPD'")->queryScalar();
		return $model;
	}

	public function actionSimpanSKPDNTanah()
	{
		Yii::import('ext.LokalIndonesia');
		...
		$TBLPENDATAAN_BLNPAJAK = (int)!empty($_REQUEST['TBLDAFTTANAH_BLNKBAKHIR']) ? $_REQUEST['TBLDAFTTANAH_BLNKBAKHIR'] : 0;

		$TBLDAFTTANAH_TANGGLNIHIL = !empty($_REQUEST["TBLDAFTTANAH_TANGGLNIHIL"]) ? $_REQUEST["TBLDAFTTANAH_TANGGLNIHIL"] : '';

		$TBLDAFTTANAH_TGLPERIKSA = !empty($_REQUEST["TBLDAFTTANAH_TGLPERIKSA"]) ? $_REQUEST["TBLDAFTTANAH_TGLPERIKSA"] : '';

		if ($TBLDAFTTANAH_TANGGLNIHIL !='') {
			$exp_TGLTERIMADAFTAR = explode('-', $TBLDAFTTANAH_TANGGLNIHIL);
		...
		$column = array(
			...
			"TBLDAFTTANAH_REGNIHIL" => LokalIndonesia::toAngka($PARAM["TBLDAFTTANAH_REGNIHIL"]),

			"TBLDAFTTANAH_TANGGLNIHIL" => $pecahtgldaftar,
			"TBLDAFTTANAH_BULANNIHIL" => $pecahbulandaftar,
			"TBLDAFTTANAH_TAHUNNIHIL" => $pecahthndaftar,
			...
			"TBLDAFTTANAH_RUPIAHPERIKSA" => LokalIndonesia::toAngka($PARAM["TBLDAFTTANAH_RUPIAHPERIKSA"]),
