...
class Angsuran_hotelController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 1;
	var $PAJAK_REK = '4.1.1.1.0';
	var $namatabel = 'TBLDAFTHOTEL';
	var $modul_url = 'penetapan/angsuran_hotel';
	var $jenispajak = 'HOTEL';
	...
	public function cekKB($tahun, $bulan, $TBLDAFTAR_NOPOK)
	{
		$model = Yii::app()->db->createCommand("SELECT {$this->namatabel}_BLNKBAWAL FROM {$this->namatabel} WHERE TBLDAFTAR_NOPOK = ".$TBLDAFTAR_NOPOK." AND TBLPENDATAAN_THNPAJAK = ".$tahun." AND TBLPENDATAAN_BLNPAJAK = ".$bulan."")->queryScalar();
		return $model;
	}
	...
	public function actiongetdata()
	{
		...
		$cekkb = $this->cekKB($tahun_substr, $bulan, $TBLDAFTAR_NOPOK);
		$arrayFilter = array(
		...
		NVL({$this->namatabel}.{$this->namatabel}_KENAIKANKRGBAYAR,0) AS {$this->namatabel}_KENAIKANKRGBAYAR,
		NVL({$this->namatabel}.{$this->namatabel}_DENDAKRGBAYAR,0) AS {$this->namatabel}_DENDAKRGBAYAR,
		NVL({$this->namatabel}.{$this->namatabel}_RUPIAHKRGBAYAR,0) AS {$this->namatabel}_RUPIAHKRGBAYAR,
		...
		NVL({$this->namatabel}.{$this->namatabel}_TGLKURANGBAYAR,0) AS {$this->namatabel}_TGLKURANGBAYAR,
		NVL({$this->namatabel}.{$this->namatabel}_BLNKURANGBAYAR,0) AS {$this->namatabel}_BLNKURANGBAYAR,
		...
		NVL({$this->namatabel}.{$this->namatabel}_TGLSKPD,0) AS {$this->namatabel}_TGLSKPD,
		NVL({$this->namatabel}.{$this->namatabel}_BLNSKPD,0) AS {$this->namatabel}_BLNSKPD,
		NVL({$this->namatabel}.{$this->namatabel}_THNSKPD,0) AS {$this->namatabel}_THNSKPD,
		...
		$data[''.$this->namatabel.'_REGKURANGBAYAR'] = $model[''.$this->namatabel.'_REGKURANGBAYAR'];
		$data[''.$this->namatabel.'_NOMORPERIKSA'] = $model[''.$this->namatabel.'_NOMORPERIKSA'];
		$data[''.$this->namatabel.'_PAJAKPERIKSA'] = $model[''.$this->namatabel.'_PAJAKPERIKSA'];
		$data[''.$this->namatabel.'_BUNGAKRGBAYAR'] = $model[''.$this->namatabel.'_BUNGAKRGBAYAR'];
		$data[''.$this->namatabel.'_KENAIKANKRGBAYAR'] = $model[''.$this->namatabel.'_KENAIKANKRGBAYAR'];
		$data[''.$this->namatabel.'_DENDAKRGBAYAR'] = $model[''.$this->namatabel.'_DENDAKRGBAYAR'];
		$data[''.$this->namatabel.'_RUPIAHKRGBAYAR'] = LokalIndonesia::ribuan($model[''.$this->namatabel.'_RUPIAHKRGBAYAR']);
		$data[''.$this->namatabel.'_BLNKBAWAL'] = $model[''.$this->namatabel.'_BLNKBAWAL'];
		$data[''.$this->namatabel.'_BLNKBAKHIR'] = $model[''.$this->namatabel.'_BLNKBAKHIR'];
		$data[''.$this->namatabel.'_TGLKURANGBAYAR'] = sprintf("%02d",$model[''.$this->namatabel.'_TGLKURANGBAYAR']);
		$data[''.$this->namatabel.'_BLNKURANGBAYAR'] = sprintf("%02d",$model[''.$this->namatabel.'_BLNKURANGBAYAR']);
		$data[''.$this->namatabel.'_THNKURANGBAYAR'] = $model[''.$this->namatabel.'_THNKURANGBAYAR'];

		$data[''.$this->namatabel.'_TGLBTSKRGBAYAR'] = sprintf("%02d",$model[''.$this->namatabel.'_TGLBTSKRGBAYAR']);
		$data[''.$this->namatabel.'_BLNBTSKRGBAYAR'] = sprintf("%02d",$model[''.$this->namatabel.'_BLNBTSKRGBAYAR']);
		$data[''.$this->namatabel.'_THNBTSKRGBAYAR'] = $model[''.$this->namatabel.'_THNBTSKRGBAYAR'];
		...
		$data['last_no_setor'] = $this->getLastNoSurat(array('thp'=>$tahun));
	...
	public function actionSimpan()
	{
		...
		$pokoktambahan = trim($_POST[''.$this->namatabel.'_TOTALANGSUR']);
	...
	public function simpanDetailAngsuran($arrayDATA)
	{
		...
		$ke = 1; foreach ($arrayDATA['tblketetapanangsuran'] as $rowDetail) :
			...
			$arrayOfData = array(
				...
				'TBLANGSURAN_THNBATASKETETAPAN' => (int)$rowDetail['thnjatuhtempoangsur'],
	...
	public function actionCetakKetetapan()
	{
        ...
        $namatpl = $path_tpl . 'TPL_KETETAPANANGS.docx';
        ...
        $skpd['tglskpd'] = $kolom[$this->namatabel .'_TGLSKPD'] .' '. LokalIndonesia::getBulan($kolom[$this->namatabel .'_BLNSKPD']);
        $skpd['blnskpd'] = LokalIndonesia::getBulan($kolom[$this->namatabel .'_BLNSKPD']);
        $skpd['thnskpd'] = '20'. $kolom[$this->namatabel .'_THNSKPD'];
        ...
        foreach ($data['rinci'] as $angs) :
        	...
        	$rincian = array(
        		...
        		'rptotalangsuran' => LokalIndonesia::ribuan($angs['TBLANGSURAN_KETETAPANTOTAL'], 0, 0),
        		'tglskp' => $angs['TBLANGSURAN_TGLBATASKETETAPAN'] . '/' . (sprintf('%02d', $angs['TBLANGSURAN_BLNBATASKETETAPAN'])) . '/' . ($angs['TBLANGSURAN_THNBATASKETETAPAN']),
	...
	{
        $data['filter'] = isset($_REQUEST['form_perjanjian_data']) ? base64_decode($_REQUEST['form_perjanjian_data']) : '';
        ...
        $path_tpl =  dirname(Yii::app()->basePath) . DIRECTORY_SEPARATOR . 'file' . DIRECTORY_SEPARATOR . 'tpl_angsuran' . DIRECTORY_SEPARATOR;
        $namatpl = $path_tpl . 'TPL_PERJANJIANANGS.docx';
        ...
        $skpd['alamatpemilik'] = $kolom['TBLDAFTAR_PEMILIKALAMAT'];
        $skpd['tahunpajak'] = '20'. $kolom['TBLPENDATAAN_THNPAJAK'];
        $skpd['bulanpajak'] = $kolom['TBLPENDATAAN_BLNPAJAK'];
        $skpd['tahunpajakkb'] = '20'. $kolom["TBLPENDATAAN_THNPAJAK"];
        $skpd['bulanpajakkb'] =  $kolom["{$this->namatabel}_BLNKURANGBAYAR"];
        $skpd['tglpajakkb'] =  $kolom["{$this->namatabel}_TGLKURANGBAYAR"] . '/' . (sprintf('%02d', $kolom["{$this->namatabel}_BLNKURANGBAYAR"])) . '/' . (2000+$kolom["{$this->namatabel}_THNKURANGBAYAR"]);
        $skpd['nokb'] =  $kolom["{$this->namatabel}_REGKURANGBAYAR"];
        $skpd['rupiahkb'] =  LokalIndonesia::ribuan($kolom["{$this->namatabel}_RUPIAHKRGBAYAR"]);
        $skpd['bulanawal'] = '';
        $skpd['bulanakhir'] = '';
        $skpd['noskp'] = $kolom["{$this->namatabel}_REGSURATANGSUR"];
        $skpd['tglsuratangsur'] =  $kolom["{$this->namatabel}_TGLSURATANGSUR"] . '/' . (sprintf('%02d', $kolom["{$this->namatabel}_BLNSURATANGSUR"])) . '/' . (2000+$kolom["{$this->namatabel}_THNSURATANGSUR"]);
        ...
        $skpd['jabatan'] = 'Ka Bid Pelayanan Pendaftaran dan Penetapan Pendapatan Daerah';
        $skpd['pejabat_nama'] = 'Ratna Andriati Harini, SH, M.Si';
        $skpd['pejabat_nip'] = '19641202 199403 2 007';
        ...
        $bungaangsur = 0;
        $RPTOTALANGSURAN = $RPANGSURAN = 0;
        foreach ($data['rinci'] as $angs) :
        	$bungaangsurr = $angs['TBLANGSURAN_BUNGAANGSURAN'];
        	$bungaangsur += $angs['TBLANGSURAN_BUNGAANGSURAN'];
        	$rptotalangsuran = $angs['TBLANGSURAN_ANGSURAN']+$bungaangsurr;
        	$RPANGSURAN += $angs['TBLANGSURAN_ANGSURAN'];
        	$RPTOTALANGSURAN += $rptotalangsuran;
        	$rincian = array(
        		'ke' => $angs['TBLANGSURAN_PAJAKKE'],
        		'rpangsuran' => LokalIndonesia::ribuan($angs['TBLANGSURAN_ANGSURAN'], 0, 0),
        		'rpbunga' => LokalIndonesia::ribuan($bungaangsurr, 0, 0),
        		'rptotalangsuran' => LokalIndonesia::ribuan($rptotalangsuran, 0, 0),
        		'tglskp' => $angs['TBLANGSURAN_TGLBATASKETETAPAN'] . '/' . (sprintf('%02d', $angs['TBLANGSURAN_BLNBATASKETETAPAN'])) . '/' . ($angs['TBLANGSURAN_THNBATASKETETAPAN']),
        		'tglbatasskp' => $angs['TBLANGSURAN_TGLBATASKETETAPAN'] . '/' . (sprintf('%02d', $angs['TBLANGSURAN_BLNBATASKETETAPAN'])) . '/' . ($angs['TBLANGSURAN_THNBATASKETETAPAN']),
        	); 
        	array_push($rows, $rincian);
        ...
        endforeach;
        $skpd['rpangsuran'] = LokalIndonesia::ribuan($RPANGSURAN, 0, 0);
        $skpd['rpbunga'] = LokalIndonesia::ribuan($bungaangsur, 0, 0);
        $skpd['rptotalangsur'] = LokalIndonesia::ribuan($RPTOTALANGSURAN, 0, 0);
        $skpd['rptotalangsur_terbilang'] = LokalIndonesia::terbilangangka($RPTOTALANGSURAN);
        $skpd['datenow'] = date('d') .' '. LokalIndonesia::getbulan(date('m')) .' '. date('Y');
        ...
        // Merge data in the first sheet
        $namafileDL = "PERJANJIAN-ANGSURAN.docx";
        $otbs->MergeBlock('angsuran', $rows);
        $otbs->MergeField('header', $skpd);
        // $otbs->PlugIn(OPENTBS_DEBUG_XML_CURRENT); // debug the template
	...
	public function getLastNoSurat($data)
	{
		$thp = (int) (isset($data['thp']) ? $data['thp'] : '');
		$thp = substr($thp, -2);
		if (!empty($thp)) {
			$sql = "
			SELECT NVL(MAX(TBLANGSURAN_NOKETETAPAN), 0)+1
			FROM TBLANGSURAN
			WHERE TBLPENDATAAN_THNPAJAK={$thp}
			GROUP BY TBLPENDATAAN_THNPAJAK
			ORDER BY TBLPENDATAAN_THNPAJAK DESC
			";
			return $xsql = Yii::app()->db->createCommand($sql)->queryScalar();
		}
	}
}
