...
<div class="well well-sm well-light text-center">
	<h4><i class="fa fa-tasks"></i> Nota Perhitungan Monitoring Pajak Self Assesment</h4>
</div>
					...
					<form class="smart-form" novalidate="novalidate" id="form-pencarian-airtanah">
						<fieldset>
							...
							<div class="row">
								...
								<section class="col col-md-3">
									<label class="input">
										<input class="input-sm" readonly="" style="background: #e4e4e4;" type="text" id="TBLDAFTHOTEL_NOMORSKPD" name="TBLDAFTHOTEL_NOMORSKPD">
									</label>
								</section>
							</div>
							...
							<div class="row" style="display: none;">
								...
								<section class="col col-md-3">
									<label class="select">
										<select id="TBLDAFTHOTEL_ISJNSPENETAPAN" name="TBLDAFTHOTEL_ISJNSPENETAPAN" class="select2">
							...
							<div class="row">
								<section class="col col-md-2">
									<p>Tgl. Entry Monitoring</p>
								</section>
								<section class="col col-md-3">
									<label class="input"> <i class="icon-append fa fa-calendar"></i>
										<input type="text" value="<?= date('d-m-Y') ?>" id="TBLDAFTHOTEL_TGLSPTPD" name="TBLDAFTHOTEL_TGLSPTPD" class="datepicker" data-dateformat="dd-mm-yy">
									</label>
								</section>
							</div>
							<div class="row">
								...
								<section class="col col-md-3">
									<label class="input"> <i class="icon-append fa fa-calendar"></i>
										<input type="text" value="<?= date('d-m-Y') ?>" id="TBLDAFTHOTEL_TGLSKPD" name="TBLDAFTHOTEL_TGLSKPD" class="datepicker" data-dateformat="dd-mm-yy">
									</label>
								</section>
							</div>
							<div class="row">
								...
								<section class="col col-md-3">
									<label class="input"> <i class="icon-append fa fa-calendar"></i>
										<input type="text" id="TBLDAFTHOTEL_TGLBATASSKPD" name="TBLDAFTHOTEL_TGLBATASSKPD" class="datepicker" data-dateformat="dd-mm-yy">
									</label>
								</section>
							</div>
...
<script type="text/javascript">
	...
	function setMoment() {
		moment.locale('id');
		$("#TBLDAFTHOTEL_TGLSKPD").trigger('change');
	}
	...
	jQuery(document).ready(function($) {
		...
		$('#TBLDAFTHOTEL_TGLSKPD').val(today);
	...
	function cari() {
		...
		var TGLBATASSKPD = $('#TBLDAFTHOTEL_TGLBATASSKPD').val();
		var TGLSPTPD = $('#TBLDAFTHOTEL_TGLSPTPD').val();
		var NOMORSKPD = $('#TBLDAFTHOTEL_NOMORSKPD').val();
		...
		if (THNPAJAK=='' || BLNPAJAK=='') {
			...
			$.ajax({
				url: 'penetapan/monitoring_pajak/GetData',
				type: 'POST',
	...
	function tetapkan() {
		...
		if (count>0) {
			...
			if (ButtonPressed === "Ya") {
				$.ajax({
					url: 'penetapan/monitoring_pajak/TetapkanPajakAir',
					type: 'POST',
					dataType: 'json',
					data: {
						...
						,TBLDAFTHOTEL_TGLSKPD: $('#TBLDAFTHOTEL_TGLSKPD').val()
						,TBLDAFTHOTEL_TGLBATASSKPD: $('#TBLDAFTHOTEL_TGLBATASSKPD').val()
					},
					beforeSend: function() {
	...
	function getNoNota(tahun) {
		$.ajax({
			url: 'penetapan/monitoring_pajak/GetNoNotaAkhir',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun: tahun
			...
			success: function(respon) {
				if (respon=='') {
					$('#TBLDAFTHOTEL_NOMORSKPD').val(1);
					
				}
				else{
					$('#TBLDAFTHOTEL_NOMORSKPD').val(respon.NOMORSKPD);
					if ($('#TBLDAFTHOTEL_TGLBATASSKPD').val()=='') {
						$("#TBLDAFTHOTEL_TGLSKPD").trigger('change');
					}
				}
			}
	...
	$("#TBLDAFTHOTEL_TGLSKPD").change(function(event) {
		// var tglbatas = moment( $("#TBLDAFTHOTEL_TGLSKPD").val().split("-").reverse().join("-") ).add(1, "months");
		// $("#TBLDAFTHOTEL_TGLBATASSKPD").val( tglbatas.format('L') );

		var tglbts1 = moment( $("#TBLDAFTHOTEL_TGLSKPD").val().split("-").reverse().join("-") ).add(1, "months");

		var tanggal_batas = $("#TBLDAFTHOTEL_TGLSKPD").val();
		...
		tglbts = new Date(tglbatas_cnvrt);
			...
			$("#TBLDAFTHOTEL_TGLBATASSKPD").val( tglbatas );
		});

	function cetakNota() {
		var q = btoa(daftaridPajakAir);
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/monitoring_pajak/cetakskpd/?data=' + q);
	}
	...
	function cetakSKPDWord() {
		var q = btoa(daftaridPajakAir);
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/penetapan/monitoring_pajak/cetakskpdword/?data=' + q);
	}
