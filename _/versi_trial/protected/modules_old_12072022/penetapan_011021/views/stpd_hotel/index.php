							...
							<form id="form-stpd-hotel" class="smart-form">
								<fieldset>
									...
									<div>
										...
										<div class="row">
											<section class="col col-md-2" style="margin-left: 2%;">Tanggal SPT</section>
											<section class="col col-md-2">
												<label class="input">
...
<br>
	...
	<script type="text/javascript">
		...
		function getNoSTPD(tahun) {
			$.ajax({
				url: 'penetapan/stpd_hotel/GetNoSTPD',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
				...
				$.ajax({
					...
					success: function(respon) {
						if (respon.data=='sudah entri STPD') {
							...
							$.SmartMessageBox({
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										$('#res_tanggal_pajak_tetap_wp').val(respon.TBLDAFTHOTEL_TGLSPTPD +'-'+ respon.TBLDAFTHOTEL_BLNSPTPD +'-20'+ respon.TBLDAFTHOTEL_THNSPTPD);
										$('#res_nomor_pajak_tetap_wp').val('');
										$('#res_jumlah_pajak_tetap_wp').val(respon.TBLDAFTHOTEL_PAJAK);
										$('#TBLDAFTHOTEL_TGLSURATTAGIHAN').val('');
										$('#TBLDAFTHOTEL_TGLBTSTAGIHAN').val('');
							...
							$('#no_subrek').select2('val',respon.REKENING_KODE);
							$('#res_tanggal_pajak_tetap_wp').val(respon.TBLDAFTHOTEL_TGLSPTPD +'-'+ respon.TBLDAFTHOTEL_BLNSPTPD +'-20'+ respon.TBLDAFTHOTEL_THNSPTPD);
							$('#res_nomor_pajak_tetap_wp').val(respon.NOMOR);
							$('#res_jumlah_pajak_tetap_wp').val(respon.TBLDAFTHOTEL_PAJAK);
								if (respon.TBLDAFTHOTEL_PAJAK == 0) {
									notifikasi('Data Tidak Ditemukan', 'failed', 1,0);
								}
		...
		function simpanstpd () {
			...
			$.ajax({
				url: 'penetapan/stpd_hotel/SimpanSTPDHotel',
				type: 'post',
				dataType: "json",
