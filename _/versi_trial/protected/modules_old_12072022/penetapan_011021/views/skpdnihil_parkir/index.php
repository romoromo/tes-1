	...
	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="well well-sm well-light bg-color-greenLight txt-color-white">
				<h4><i class="fa fa-file-text-o"></i> SKPD Nihil - Pajak Parkir </h4>
			</div>
		</div>
	</div>
	...
	<section id="widget-grid" class="" >
		...
		<div class="row">
			...
			<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				<div class="jarviswidget jarviswidget-color-teal" id="wid-id-Pajak_Parkir" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
					<header>
					...
					<div>
						...
						<div class="widget-body no-padding">
							<form id="form-skpdn-parkir" class="smart-form" novalidate="">
								<fieldset>
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="select">
												<select class="select2" name="TBLDAFTPARKIR_BLNKBAWAL" id="TBLDAFTPARKIR_BLNKBAWAL">
													<option value="0">-- Silahkan Pilih --</option>
										...
										<section class="col col-md-2">
											<label class="select">
												<select class="select2" name="TBLDAFTPARKIR_BLNKBAKHIR" id="TBLDAFTPARKIR_BLNKBAKHIR">
													<option value="0">-- Silahkan Pilih --</option>
									...
									<hr><br>
									<div class="row">
										<section class="col col-md-12"><h4 align="center">Entry SKPDN Pajak Parkir</h4></section>
									</div>
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" name="TBLDAFTPARKIR_TGLPERIKSA" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTPARKIR_TGLPERIKSA">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTPARKIR_RUPIAHPERIKSA" name="TBLDAFTPARKIR_RUPIAHPERIKSA">
											</label>
										</section>
									</div>
									<div class="row">
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input">
												<input class="form-control" type="text" id="TBLDAFTPARKIR_REGNIHIL" name="TBLDAFTPARKIR_REGNIHIL">
											</label>
										</section>
										<section class="col col-md-2">Tanggal SKPDKN</section>
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" name="TBLDAFTPARKIR_TANGGLNIHIL" class="datepicker" value="<?= date('d-m-Y') ?>"  data-dateformat="dd-mm-yy" id="TBLDAFTPARKIR_TANGGLNIHIL">
											</label>
										</section>
									</div>
								</fieldset>
								<footer id="footer-parkir">
									<div id="tbl_simpan">
	...
	<script type="text/javascript">
		...
		function generateAutocomplete() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'penetapan/skpdnihil_parkir/autocomplete',
		...
		function runFormValidation() {
			var $FormData = $("#form-skpdn-parkir").validate({
			...
			rules : {
				...
				"TBLDAFTPARKIR_RUPIAHPERIKSA" : {
					required : true
				},
				"TBLDAFTPARKIR_REGNIHIL" : {
					required : true
				},
				"TBLDAFTPARKIR_TGLPERIKSA" : {
					required : true
				},
				"TBLDAFTPARKIR_TANGGLNIHIL" : {
					required : true
				},
			},
			...
			var param = jQuery.param(
				{
					...
					, TBLPENDATAAN_BLNPAJAK: $("#TBLDAFTPARKIR_BLNKBAKHIR").select2('val')
					, TBLPENDATAAN_THNPAJAK: $("#TBLPENDATAAN_THNPAJAK").val()
				}
			);
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/skpdnihil_parkir/cetakword?'+param);
		}

		function cari () {
			var CARI_NOPOK = $('#TBLDAFTAR_NOPOK').val();
			var BULAN_AKHIR = $('#TBLDAFTPARKIR_BLNKBAKHIR').select2('val');
			...
			if (CARI_NOPOK=='') {
				...
				$.ajax({
					url: 'penetapan/skpdnihil_parkir/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
						TBLDAFTAR_NOPOK : CARI_NOPOK,
						bulan_akhir : $('#TBLDAFTPARKIR_BLNKBAKHIR').select2('val'),
						...
						if (respon.data=='sudah entri') {
							$.SmartMessageBox({
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									$('#TBLDAFTPARKIR_RUPIAHPERIKSA').val(respon.TBLDAFTPARKIR_RUPIAHPERIKSA);
									$('#TBLDAFTPARKIR_NOMORPERIKSA').val(respon.TBLDAFTPARKIR_NOMORPERIKSA);
									$('#TBLDAFTPARKIR_REGNIHIL').val(respon.TBLDAFTPARKIR_REGNIHIL);

									$('#TBLDAFTPARKIR_PAJAKPERIKSA').attr('disabled', 'disabled');
									$('#TBLDAFTPARKIR_PAJAKPERIKSA').attr('style', 'background:#eee');

									if (respon.TBLDAFTPARKIR_TGLPERIKSA==undefined) {
										$('#TBLDAFTPARKIR_TGLPERIKSA').val('<?= date('d-m-Y') ?>');
									} else {
									...
									// $('#TBLDAFTPARKIR_TANGGLNIHIL').val(respon.TBLDAFTPARKIR_TANGGLNIHIL +'-'+ respon.TBLDAFTPARKIR_BULANNIHIL +'-20'+ respon.TBLDAFTPARKIR_TAHUNNIHIL);
							...
							if (respon.TBLDAFTPARKIR_TGLPERIKSA==undefined || respon.TBLDAFTPARKIR_TGLPERIKSA==0) {
								$('#TBLDAFTPARKIR_TGLPERIKSA').val('<?= date('d-m-Y') ?>');
							} else {
								$('#TBLDAFTPARKIR_TGLPERIKSA').val(respon.TBLDAFTPARKIR_TGLPERIKSA +'-'+ respon.TBLDAFTPARKIR_BLNPERIKSA +'-20'+respon.TBLDAFTPARKIR_THNPERIKSA);
							}
		...
		function getNoSKPDNihil(tahun) {
			$.ajax({
				url: 'penetapan/skpdnihil_parkir/getNoSKPDNihil',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
				...
				success: function(respon) {
					if (respon=='') {
						$('#TBLDAFTPARKIR_REGNIHIL').val(1);
						
					}
					else{
						$('#TBLDAFTPARKIR_REGNIHIL').val(respon.NOLB);
		...
		function simpanskpdnihil () {
			$.ajax({
				url: 'penetapan/skpdnihil_parkir/SimpanSKPDNParkir',
				type: 'post',
				dataType: "json",
				data:  $("#form-skpdn-parkir").serialize(),
