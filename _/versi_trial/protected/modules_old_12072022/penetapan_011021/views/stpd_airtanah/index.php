				...
				<div class="jarviswidget jarviswidget-color-teal" id="wid-id-kiosss" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
					...
					<div>
						...
						<div class="widget-body no-padding">
							<form id="form-stpd-air" class="smart-form">
								<fieldset>
...
<br>
	...
	<script type="text/javascript">
		...
		function getNoSTPD(tahun) {
			$.ajax({
				url: 'penetapan/stpd_airtanah/GetNoSTPD',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
		...
		function generateAutocompleteWP() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'penetapan/stpd_airtanah/autocompletewp',
		...
		function reset_form(){
			$("#form-stpd-air")[0].reset();
			$("#form-stpd-air .select2").select2('val', '');
			setMoment();
		...
		function runFormValidation() {
			var $FormData = $("#form-stpd-air").validate({
		...
		function cetak () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/stpd_airtanah/cetak?'+param);
		}
		...
		function cetakWord () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/stpd_airtanah/cetakword?'+param);
		}
		...
		function cari () {
			...
			if (CARI_NOPOK=='') {
				...
				$.ajax({
					url: 'penetapan/stpd_airtanah/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
					...
					success: function(respon) {
						if (respon.data=='sudah entri STPD') {
							...
							$.SmartMessageBox({
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										$('#res_tanggal_pajak_tetap_wp').val(respon.TBLDAFTTANAH_TGLSPTPD +'-'+ respon.TBLDAFTTANAH_BLNSPTPD +'-20'+ respon.TBLDAFTTANAH_THNSPTPD);
							...
							$('#res_jumlah_pajak_tetap_wp').val(respon.TBLDAFTTANAH_PAJAK);
								if (respon.TBLDAFTTANAH_PAJAK == 0) {
		...
		function simpanstpd () {
			...
			$.ajax({
				url: 'penetapan/stpd_airtanah/SimpanSTPDair',
				type: 'post',
				dataType: "json",
				data:  $("#form-stpd-air").serialize(),
		...
		function hapuskb() {
			...
			$.SmartMessageBox({
				...
				buttons : '[Tidak][Ya]' // pengaturan tombol
					...
					if (ButtonPressed === "Ya") {
						$.ajax({
							url: 'penetapan/stpd_airtanah/HapusSKPDKB',
							type: 'post',
							dataType: "json",
							data:  $("#form-stpd-air").serialize(),
							success: function(data) {
