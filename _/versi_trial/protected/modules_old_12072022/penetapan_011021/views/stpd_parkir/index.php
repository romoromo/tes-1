	...
	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="well well-sm well-light bg-color-greenLight txt-color-white">
				<h4><i class="fa fa-file-text-o"></i> STPD - Pajak Parkir </h4>
			</div>
		</div>
	</div>
				...
				<div class="jarviswidget jarviswidget-color-teal" id="wid-id-kiosss" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
					...
					<div>
						...
						<div class="widget-body no-padding">
							<form id="form-stpd-parkir" class="smart-form">
								<fieldset>
									...
									<div class="row">
										...
										<section class="col col-md-2">
											...
											<label class="select">
												<select class="select2" name="JENIS" id="JENIS">
									...
									<hr><br>
									<div class="row">
										<section class="col col-md-12"><h4 align="center">Entry STPD Pajak Parkir</h4></section>
									</div>
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input style="background: #e4e4e4;" type="text" value="<?= date('d-m-Y') ?>" name="TBLDAFTPARKIR_TGLPERIKSA" id="TBLDAFTPARKIR_TGLPERIKSA" readonly="">
											</label>
										</section>
										<section class="col col-md-2" style="margin-left: 2%;">Nomor Pemeriksaan</section>
										<section class="col col-md-2">
											<label class="input">
												<input style="background: #e4e4e4;" class="form-control" type="text" id="TBLDAFTPARKIR_NOMORPERIKSA" name="TBLDAFTPARKIR_NOMORPERIKSA" readonly="">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input style="background: #e4e4e4;" class="form-control format-rupiah" type="text" id="TBLDAFTPARKIR_PAJAKPERIKSA" name="TBLDAFTPARKIR_PAJAKPERIKSA" readonly="">
											</label>
										</section>
									</div>
									<div class="row">
									...
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input">
												<input class="form-control" type="text" id="TBLDAFTPARKIR_NOSURATTAGIHAN" name="TBLDAFTPARKIR_NOSURATTAGIHAN">
											</label>
										</section>
										<section class="col col-md-2">Tanggal STPD</section>
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" value="<?= date('d-m-Y') ?>" name="TBLDAFTPARKIR_TGLSURATTAGIHAN" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTPARKIR_TGLSURATTAGIHAN">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTPARKIR_BUNGATAGIHAN" name="TBLDAFTPARKIR_BUNGATAGIHAN">
											</label>
										</section>
									...
									<div class="row">
										...
										<section class="col col-md-4">
											<label class="input">
												<input class="form-control format-rupiah" type="text" id="TBLDAFTPARKIR_RUPIAHTAGIHAN" name="TBLDAFTPARKIR_RUPIAHTAGIHAN">
											</label>
										</section>
									</div>
									<div class="row">
										...
										<section class="col col-md-2">
											<label class="input"> <i class="icon-append fa fa-calendar"></i>
												<input type="text" name="TBLDAFTPARKIR_TGLBTSTAGIHAN" class="datepicker" data-dateformat="dd-mm-yy" id="TBLDAFTPARKIR_TGLBTSTAGIHAN">
											</label>
										</section>
									</div>
								</fieldset>
...
<br>
	...
	<script type="text/javascript">
		...
		function setMoment() {
			moment.locale('id');
			$("#TBLDAFTPARKIR_TGLSURATTAGIHAN").trigger('change');
		}
		...
		jQuery(document).ready(function($) {
			...
			$('#TBLDAFTPARKIR_TGLSURATTAGIHAN').val(today);
		...
		function getNoSTPD(tahun) {
			$.ajax({
				url: 'penetapan/stpd_parkir/GetNoSTPD',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
				...
				success: function(respon) {
					if (respon=='') {
						$('#TBLDAFTPARKIR_NOSURATTAGIHAN').val(1);
						
					}
					else{
						$('#TBLDAFTPARKIR_NOSURATTAGIHAN').val(respon.NOKB);
						if ($('#TBLDAFTPARKIR_TGLBTSTAGIHAN').val()=='') {
							$("#TBLDAFTPARKIR_TGLSURATTAGIHAN").trigger('change');
						}
					}
				}
		...
		$("#TBLDAFTPARKIR_TGLSURATTAGIHAN").change(function(event) {
			var tglbatas = moment( $("#TBLDAFTPARKIR_TGLSURATTAGIHAN").val().split("-").reverse().join("-") ).add(1, "days");
			$("#TBLDAFTPARKIR_TGLBTSTAGIHAN").val( tglbatas.format('L') );
		});

		function generateAutocompleteWPParkir() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'penetapan/stpd_parkir/autocompletewpparkir',
		...
		function reset_form(){
			$("#form-stpd-parkir")[0].reset();
			$("#form-stpd-parkir .select2").select2('val', '');
			setMoment();
		...
		function runFormValidation() {
			var $FormData = $("#form-stpd-parkir").validate({
		...
		function cetak () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/stpd_parkir/cetak?'+param);
		}
		...
		function cetakWord () {
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1); ?>/penetapan/stpd_parkir/cetakword?'+param);
		}
		...
		function HitungSKPDKB() {
			...
			$(".TBLDAFTPARKIR_BUNGATAGIHAN").html(toRp(RUPIAHKB));
			$("#TBLDAFTPARKIR_BUNGATAGIHAN").val(RUPIAHKB.toFixed(0));

			console.log('RUPIAHKB='+RUPIAHKB);
			// $("#TBLDAFTPARKIR_RUPIAHTAGIHAN").val( RUPIAHKB.toFixed(2) );
		}

		function Hitungtotal(){
			PAJAK = toAngka($('#res_jumlah_pajak_tetap_wp').val());
			
			$(".TBLDAFTPARKIR_RUPIAHTAGIHAN").html(toRp(PAJAK));
			$("#TBLDAFTPARKIR_RUPIAHTAGIHAN").val(PAJAK);
		...
		function cari () {
			...
			if (CARI_NOPOK=='') {
				...
				$.ajax({
					url: 'penetapan/stpd_parkir/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
					...
					success: function(respon) {
						if (respon.data=='sudah entri') {
							...
							$.SmartMessageBox({
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										$('#res_tanggal_pajak_tetap_wp').val(respon.TBLDAFTPARKIR_TGLSKPD +'-'+ respon.TBLDAFTPARKIR_BLNSKPD +'-20'+ respon.TBLDAFTPARKIR_THNSKPD);
										$('#res_nomor_pajak_tetap_wp').val('');
										$('#res_jumlah_pajak_tetap_wp').val(respon.TBLDAFTPARKIR_PAJAK);
										$('#res_tanggal_pajak_setor_wp').val(respon.TBLDAFTPARKIR_TGLENTRISETOR +'-'+ respon.TBLDAFTPARKIR_BLNENTRISETOR +'-20'+ respon.TBLDAFTPARKIR_THNENTRISETOR);
										$('#res_nomor_pajak_setor_wp').val(respon.TBLDAFTPARKIR_REGSETOR);
										$('#res_jumlah_pajak_setor_wp').val(respon.TBLDAFTPARKIR_RUPIAHSETOR);
										$('#TBLDAFTPARKIR_PAJAKPERIKSA').val(respon.TBLDAFTPARKIR_PAJAKPERIKSA);
										$('#TBLDAFTPARKIR_BUNGATAGIHAN').val(respon.TBLDAFTPARKIR_BUNGATAGIHAN);
										$('#TBLDAFTPARKIR_KENAIKANKRGBAYAR').val(respon.TBLDAFTPARKIR_KENAIKANKRGBAYAR);
										$('#TBLDAFTPARKIR_DENDAKRGBAYAR').val(respon.TBLDAFTPARKIR_DENDAKRGBAYAR);
										$('#TBLDAFTPARKIR_RUPIAHTAGIHAN').val(respon.TBLDAFTPARKIR_RUPIAHTAGIHAN);
										...
										$('#TBLDAFTPARKIR_TGLPERIKSA').val(respon.TBLDAFTPARKIR_TGLPERIKSA +'-'+ respon.TBLDAFTPARKIR_BLNPERIKSA +'-20'+ respon.TBLDAFTPARKIR_THNPERIKSA);
										$('#TBLDAFTPARKIR_NOSURATTAGIHAN').val(respon.TBLDAFTPARKIR_NOSURATTAGIHAN);
										$('#TBLDAFTPARKIR_TGLSURATTAGIHAN').val(respon.TBLDAFTPARKIR_TGLSURATTAGIHAN +'-'+ respon.TBLDAFTPARKIR_BLNSURATTAGIHAN +'-20'+respon.TBLDAFTPARKIR_THNSURATTAGIHAN);
										$('#TBLDAFTPARKIR_TGLBTSTAGIHAN').val(respon.TBLDAFTPARKIR_TGLBTSTAGIHAN +'-'+ respon.TBLDAFTPARKIR_BLNBTSKRGBAYAR +'-20'+ respon.TBLDAFTPARKIR_THNBTSTAGIHAN);
							...
							$('#res_nomor_pajak_setor_wp').val(respon.TBLDAFTPARKIR_REGSETOR);
							$('#res_jumlah_pajak_setor_wp').val(respon.TBLDAFTPARKIR_RUPIAHSETOR);
							$('#TBLDAFTPARKIR_BUNGATAGIHAN').val('');
							$('#TBLDAFTPARKIR_RUPIAHTAGIHAN').val('');
							$('#TBLDAFTPARKIR_PAJAKPERIKSA').val(respon.TBLDAFTPARKIR_PAJAKPERIKSA);
							$('#TBLDAFTPARKIR_NOMORPERIKSA').val(respon.TBLDAFTPARKIR_NOMORPERIKSA);
							$('#TBLDAFTPARKIR_TGLPERIKSA').val(respon.TBLDAFTPARKIR_TGLPERIKSA +'-'+ respon.TBLDAFTPARKIR_BLNPERIKSA +'-20'+ respon.TBLDAFTPARKIR_THNPERIKSA);
							$('#footer-parkir').show('fast');
							$('#btncetak').show('fast');
							setPriceFormat();
							var tglbatas = moment( $("#TBLDAFTPARKIR_TGLSURATTAGIHAN").val().split("-").reverse().join("-") ).add(1, "months");
							$("#TBLDAFTPARKIR_TGLBTSTAGIHAN").val( tglbatas.format('L') );
							$('#btnsimpan').removeAttr('disabled');
		...
		function simpanstpd () {
			...
			$.ajax({
				url: 'penetapan/stpd_parkir/SimpanSTPDParkir',
				type: 'post',
				dataType: "json",
				data:  $("#form-stpd-parkir").serialize(),
		...
		function hapuskb() {
			...
			$.SmartMessageBox({
				...
				buttons : '[Tidak][Ya]' // pengaturan tombol
					...
					if (ButtonPressed === "Ya") {
						$.ajax({
							url: 'penetapan/stpd_parkir/HapusSTPD',
							type: 'post',
							dataType: "json",
							data:  $("#form-stpd-parkir").serialize(),
							success: function(data) {
