...
class Cetak_ulang_ketetapan_bytanggalController extends Controller
{
	...
	public function actionCari()
	{
		...
		$TGLSKPD = isset($_REQUEST['TGLSKPD']) && strtotime($_REQUEST['TGLSKPD']) ? date('Y-m-d', strtotime($_REQUEST['TGLSKPD'])) : date('Y-m-d');
		$TGLSKPD2 = isset($_REQUEST['TGLSKPD2']) && strtotime($_REQUEST['TGLSKPD2']) ? date('Y-m-d', strtotime($_REQUEST['TGLSKPD2'])) : date('Y-m-d');
		...
		$TBLPENDATAAN_THNPAJAK = DMOrcl::getRequest('TBLPENDATAAN_THNPAJAK') ? (int)DMOrcl::getRequest('TBLPENDATAAN_THNPAJAK') : '';
		$TAHUNPAJAK = !empty($TBLPENDATAAN_THNPAJAK) ? substr($TBLPENDATAAN_THNPAJAK, -2) : '';
		...
		if ( !empty($TGLSKPD) ) {
			...
			$tglskpda = $exptglskpd[0];
			$blnskpda = $exptglskpd[1];
			$thnskpda = $exptglskpd[2];
			$tglskpd = $tglskpda;
			$blnskpd = $blnskpda;
			$thnskpd = substr($thnskpda, -2);
		}
		...
		$thnskpd2 = '';
		if ( !empty($TGLSKPD2) ) {
			$exptglskpdb = explode('-', $TGLSKPD2);
			$tglskpdb = $exptglskpdb[0];
			$blnskpdb = $exptglskpdb[1];
			$thnskpdb = $exptglskpdb[2];
			$tglskpd = $tglskpdb;
			$blnskpd = $blnskpdb;
			$thnskpd = substr($thnskpdb, -2);
		}
		...
		if ($JENIS_PAJAK=='AIRTANAH') {
			...
			$from = 'TBLDAFTTANAH';
			$filter = array(
				'EQ__TBLDAFTTANAH.TBLPENDATAAN_THNPAJAK' => $TAHUNPAJAK
			...
			$otherquery = array(
				...
				// ,'andwhere_thnpajak'=> "TBLDAFTTANAH.TBLPENDATAAN_THNPAJAK BETWEEN 17 AND {$yearnow}"
				,'andwhere_nvlsudahtetap'=> "
					NVL(TBLDAFTTANAH_THNSKPD, 0)<>0
					AND NVL(TBLDAFTTANAH_BLNSKPD, 0)<>0
					AND NVL(TBLDAFTTANAH_TGLSKPD, 0)<>0
				"
				,'andwhere_range'=> "
					TO_DATE ( NVL(TBLDAFTTANAH_THNSKPD,2000) || '/' || NVL(TBLDAFTTANAH_BLNSKPD,1)  || '/' || NVL(TBLDAFTTANAH_TGLSKPD,1),  'YY/MM/DD') 
					BETWEEN TO_DATE('{$TGLSKPD}', 'YYYY-MM-DD') AND TO_DATE('{$TGLSKPD2}', 'YYYY-MM-DD')
				"
			...
			$from = 'TBLDAFTREKLAME';
			$filter = array(
				'EQ__TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK' => $TAHUNPAJAK
			...
			$otherquery = array(
				...
				,'andwhere_thnpajak'=> "TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK BETWEEN 17 AND {$yearnow}"
				,'andwhere_nvlsudahtetap'=> "
				...
				"
				,'andwhere_range'=> "
					TO_DATE (TBLDAFTREKLAME_THNSKPD|| '/' || TBLDAFTREKLAME_BLNSKPD  || '/' || TBLDAFTREKLAME_TGLSKPD,  'YY/MM/DD') 
					BETWEEN TO_DATE('{$TGLSKPD}', 'YYYY-MM-DD') AND TO_DATE('{$TGLSKPD2}', 'YYYY-MM-DD')
				"
	...
	public function actionCetakDaftarWord()
	{
		...
		$TBLDAFTREKLAME_JNSREKLAME = isset($_REQUEST['TBLDAFTREKLAME_JNSREKLAME']) ? $_REQUEST['TBLDAFTREKLAME_JNSREKLAME'] : '';
		$TGLSKPD = $TANGGALSKPD = isset($_REQUEST['TGLSKPD']) && strtotime($_REQUEST['TGLSKPD']) ? date('Y-m-d', strtotime($_REQUEST['TGLSKPD'])) : date('Y-m-d');
		$TGLSKPD2 = isset($_REQUEST['TGLSKPD2']) && strtotime($_REQUEST['TGLSKPD2']) ? date('Y-m-d', strtotime($_REQUEST['TGLSKPD2'])) : date('Y-m-d');
		// error_reporting(0);
		...
		$WHERETAHUNPAJAK = '';
		if (!empty($tahunpjk)) {
			$tahunpjk = substr($tahunpjk, -2);
			$WHERETAHUNPAJAK = " AND TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK={$tahunpjk}";
		}
		$query = "
		SELECT
		...
		AND (TBLDAFTREKLAME.TBLPENDATAAN_THNPAJAK BETWEEN 17 AND {$yearnow})
		{$WHERETAHUNPAJAK}
		AND (
		NVL(TBLDAFTREKLAME_THNSKPD, 0)<>0
		AND NVL(TBLDAFTREKLAME_BLNSKPD, 0)<>0
	...
	public function actionCetakDaftarWordAirtanah()
	{
		...
		if (!empty($tahunpjk)) {
			$tahunpjk = substr($tahunpjk, -2);
			$WHERETAHUNPAJAK = " AND TBLDAFTTANAH.TBLPENDATAAN_THNPAJAK={$tahunpjk}";
		}
		$query = "
		SELECT
