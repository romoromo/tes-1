...
class Skpdkbjabatan_daftarController extends Controller
{
	...
	public function actionCetak()
	{
		...
		$range = "AND TO_DATE ( NVL(THKBJAB,99) || '/' || NVL(BLKBJAB,01)  || '/' || NVL(HRKBJAB,01),  'YY/MM/DD') 
		...
		NVL(".$NAMATABEL.".NOKBJAB, 0) AS NOKBJAB,
		...
		NVL(".$NAMATABEL.".".$NAKB.", 0) AS KENAIKANKRGBAYAR,
		NVL(".$NAMATABEL.".BUKBJAB, 0) AS ".$NAMATABEL."_BUNGAKRGBAYAR,
		NVL(".$NAMATABEL.".RPKBJAB, 0) AS ".$NAMATABEL."_RUPIAHKRGBAYAR,
		...
		CASE WHEN ".$NAMATABEL.".".$NAMATABEL."_REGSURATANGSUR > 0 THEN 'DIANGSUR' ELSE '' END AS STATUS_ANGSUR
		FROM
			".$NAMATABEL."
		...
		ORDER BY
			TO_NUMBER (TO_CHAR(NVL(NOKBJAB, 0)))
		";
	...
	public function actionCetakWord()
	{
		...
		".$NAMATABEL.".HRBKBJAB,
		NVL(".$NAMATABEL.".NOKBJAB, 0) AS NOKBJAB,
		NVL(".$NAMATABEL.".PAJAKJAB, 0) AS ".$NAMATABEL."_POKOK,
		NVL(".$NAMATABEL.".NAKBJAB, 0) AS ".$NAMATABEL."_DENDAKRGBAYAR,
		NVL(".$NAMATABEL.".BUKBJAB, 0) AS ".$NAMATABEL."_BUNGAKRGBAYAR,
		NVL(".$NAMATABEL.".RPKBJAB, 0) AS ".$NAMATABEL."_RUPIAHKRGBAYAR,
		TBLSETORANBPD.TBLSETORANBPD_JNSKETETAPAN,
		...
		$namatpl = $path_tpl . 'daftar_skpdkb_jab.docx';
		...
		$totalkb = array('totalkb'=>0); $totalsetor = array('totalsetor'=>0); $totalbunga = array('totalbunga'=>0); $totaldenda = array('totaldenda'=>0); $totalperiksa = array('totalperiksa'=>0); $no=1; foreach ($data['cetak'] as $kolom) :
			...
			$dt['no'] = $no++;
			$dt['tglkb'] = $kolom['HRKBJAB'] .'-'. $kolom['BLKBJAB'] .'-20'. $kolom['THKBJAB'];
			// $dt['tglkb'] = '';
			$dt['nokb'] = $kolom['NOKBJAB'];
			...
			$dt['pokok'] = LokalIndonesia::ribuan($kolom[''.$NAMATABEL.'_POKOK']);
			...
			$totalperiksa['totalperiksa'] += $kolom[''.$NAMATABEL.'_POKOK'];
		...
		$namafileDL = "DAFTAR-SKPDKB-".$NAMAPAJAK.".docx";
