...
class Angsuran_hotelController extends Controller
{
	...
	{
		$model = Yii::app()->db->createCommand("SELECT {$this->namatabel}_BLNKBAWAL FROM {$this->namatabel} WHERE TBLDAFTAR_NOPOK = ".$TBLDAFTAR_NOPOK." AND TBLPENDATAAN_THNPAJAK = ".$tahun." AND TBLPENDATAAN_BLNPAJAK = ".$bulan." AND NVL(TBLPENDATAAN_TGLPAJAK,0) = 0")->queryScalar();
		return $model;
	}
	...
	public function actiongetdata()
	{
		...
		NVL({$this->namatabel}.{$this->namatabel}_POKOKTAMBAHAN,0) AS {$this->namatabel}_POKOKTAMBAHAN,
		NVL({$this->namatabel}.{$this->namatabel}_BUNGATAMBAHAN,0) AS {$this->namatabel}_BUNGATAMBAHAN,
		...
		$otherquery = array(
			...
			,'andwhere_hrp'=> 'NVL('.$this->namatabel.'.TBLPENDATAAN_TGLPAJAK,0) = 0'
		);
		...
		$data[''.$this->namatabel.'_POKOKTAMBAHAN'] = LokalIndonesia::ribuan($model[''.$this->namatabel.'_POKOKTAMBAHAN']);
		$data[''.$this->namatabel.'_BUNGATAMBAHAN'] = LokalIndonesia::ribuan($model[''.$this->namatabel.'_BUNGATAMBAHAN']);
	...
	public function actionHapus()
	{
		...
		$column = array(''.$this->namatabel.'_THNSURATANGSUR' => new CDbExpression('NULL')
				...
				,''.$this->namatabel.'_BUNGATAMBAHAN' => new CDbExpression('NULL')
				,''.$this->namatabel.'_POKOKTAMBAHAN' => new CDbExpression('NULL')
			);
	...
	public function actionCetakKetetapan()
	{
        ...
        $skpd['bulanawal'] = LokalIndonesia::getBulan($kolom["{$this->namatabel}_BLNKBAWAL"]);
        $skpd['bulanakhir'] = LokalIndonesia::getBulan($kolom["{$this->namatabel}_BLNKBAKHIR"]);
        ...
        foreach ($data['rinci'] as $angs) :
        	...
        	$rincian = array(
        		...
        		'tglskp' => (sprintf('%02d', $angs['TBLPENDATAAN_BLNPAJAK'])) . ' ' . '00',
    ...
    public function actionCetakPerjanjian()
	{
		...
		$TBLDAFTAR_NOPOK = intval($PARAMS['TBLDAFTAR_NOPOK']);
		$keterlambatan = LokalIndonesia::toAngka($PARAMS[''.$this->namatabel.'_BUNGATAMBAHAN']);
		$bulan = intval($PARAMS['TBLPENDATAAN_BLNPAJAK']);
		$tahun = intval($PARAMS['TBLPENDATAAN_THNPAJAK']);
		$tanggal = isset($PARAMS['TBLPENDATAAN_TGLPAJAK']) ? intval($PARAMS['TBLPENDATAAN_TGLPAJAK']) : '0' ;
		$tahun_substr = substr($tahun, 2,4);
        ...
        $skpd['thnkb'] = '20'.$kolom["{$this->namatabel}_THNKURANGBAYAR"];
        $skpd['rpkb'] = LokalIndonesia::ribuan($kolom["{$this->namatabel}_RUPIAHKRGBAYAR"]);
        $skpd['keterlambatan'] = LokalIndonesia::ribuan($keterlambatan);
        $skpd['kbjumlah'] = LokalIndonesia::ribuan($keterlambatan + $kolom["{$this->namatabel}_RUPIAHKRGBAYAR"]);
        ...
        $skpd['tglkb'] = $kolom["{$this->namatabel}_TGLKURANGBAYAR"].'-'. $kolom["{$this->namatabel}_BLNKURANGBAYAR"].'-'.'20'.$kolom["{$this->namatabel}_THNKURANGBAYAR"];
        ...
        foreach ($data['rinci'] as $angs) :
        	$bungaangsurr = $angs['TBLANGSURAN_BUNGAANGSURAN'];
        	$totalbunga += $angs['TBLANGSURAN_BUNGAANGSURAN'];
        	$totalpokok += $angs['TBLANGSURAN_ANGSURAN'];
        	$totalpokokdanbunga = $angs['TBLANGSURAN_ANGSURAN'] + $angs['TBLANGSURAN_BUNGAANGSURAN'];
        	$totalangsuran += $totalpokokdanbunga;
        	$rincian = array(
        ...
        $skpd['rpangsuran'] = LokalIndonesia::ribuan($angs['TBLANGSURAN_ANGSURAN'], 0, 0);
        $skpd['rptotalpokok'] = LokalIndonesia::ribuan($totalpokok, 0, 0);
        $skpd['rptotalbunga'] = LokalIndonesia::ribuan($totalbunga, 0, 0);
        $skpd['rptotalangsur'] = LokalIndonesia::ribuan($totalangsuran, 0, 0);
        $skpd['rptotalangsur_terbilang'] = LokalIndonesia::terbilangangka($totalangsuran);
        $skpd['datenow'] = date('d') .' '. LokalIndonesia::getbulan(date('m')) .' '. date('Y');
