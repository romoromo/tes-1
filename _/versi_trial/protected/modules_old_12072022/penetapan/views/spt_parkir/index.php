...
<script type="text/javascript">
	...
	function generateAutocompleteWP() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'penetapan/spt_parkir/autocompletewp',
	...
	if (TBLDAFTAR_NOPOK=='') {
		notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
			...
			$.ajax({
				url: 'penetapan/spt_parkir/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'penetapan/spt_parkir/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										if (respon.data=='proses selanjutnya') {
											...
											$('#hit_totalsetor').val(respon.TBLDAFTPARKIR_PAJAK);
	...
	$.ajax({
		url: 'penetapan/spt_parkir/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			 THNPAJAK : THNPAJAK
	...
	function simpan() {
		...
		$.ajax({
			url: 'penetapan/spt_parkir/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
