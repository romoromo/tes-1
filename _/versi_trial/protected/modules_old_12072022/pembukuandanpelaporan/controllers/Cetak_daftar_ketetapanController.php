...
class Cetak_daftar_ketetapanController extends Controller
{
    ...
    public function actionCari()
    {
        ...
        $TGLENTRI2 = $_REQUEST['TGLENTRI2'] ? $_REQUEST['TGLENTRI2'] : '';

        $TGLENTRI_AW = isset($_REQUEST['TGLENTRI']) && strtotime($_REQUEST['TGLENTRI']) ? date('Y-m-d', strtotime($_REQUEST['TGLENTRI'])) : date('Y-m-d');
        $TGLENTRI_AK = isset($_REQUEST['TGLENTRI2']) && strtotime($_REQUEST['TGLENTRI2']) ? date('Y-m-d', strtotime($_REQUEST['TGLENTRI2'])) : date('Y-m-d');
        ...
        if ($JENIS_PAJAK=='AIRTANAH') {
            ...
            $otherquery = array(
                ...
                ,'andwhere_range'=> "
                    TO_DATE ( NVL(TBLDAFTREKLAME_THNENTRI,2000) || '/' || NVL(TBLDAFTREKLAME_BLNENTRI,1)  || '/' || NVL(TBLDAFTREKLAME_TGLENTRI,1),  'YY/MM/DD') 
                    BETWEEN TO_DATE('{$TGLENTRI_AW}', 'YYYY-MM-DD') AND TO_DATE('{$TGLENTRI_AK}', 'YYYY-MM-DD')
                "
