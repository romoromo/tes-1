...
<table id="dt_pipeline"  border="0" style="border-collapse:collapse;" cellpadding="0" cellspacing="0" class="table table-striped table-bordered table-hover smart-form" width="100%" >
	...
	<?php $no = 1; foreach ($data['cetak'] as $rowtap): ?>
      ...
      <td align="center"><?= $no++; ?></td>
			<td align="center"><?= $rowtap['TBLDAFTAR_JENISPENDAPATAN'] ?>.<?= $rowtap['TBLDAFTAR_GOLONGAN'] ?>.<?= $rowtap['TBLDAFTAR_NOPOK'] ?>.<?= sprintf("%02d",$rowtap['TBLKECAMATAN_IDBADANUSAHA']) ?>.<?= sprintf("%02d",$rowtap['TBLKELURAHAN_IDBADANUSAHA']) ?></td> <!-- N.P.W.P.D --> <!-- TBLKECAMATAN_IDBADANUSAHA Dan TBLKELURAHAN_IDBADANUSAHA -->
