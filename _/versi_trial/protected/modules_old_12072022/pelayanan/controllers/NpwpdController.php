...
{
	var $MODULE_URL = 'pelayanan/npwpd';
	...
	public function simpanNPWPD($listrekening = array(), $data = array())
	{
		...
		foreach ($listrekening as $row) {
			...
			if ($_REQUEST['param']['TNPWPD_PROSENTASEOMZET'][$row]!='') {
				$model->TNPWPD_STSOMZET = $_REQUEST['param']['TNPWPD_STSOMZET'];
			}

			$model->TNPWPD_PROSENTASEOMZET = isset( $_REQUEST['param']['TNPWPD_PROSENTASEOMZET'][$row] ) ? LokalIndonesia::toAngka($_REQUEST['param']['TNPWPD_PROSENTASEOMZET'][$row]) : '0';
