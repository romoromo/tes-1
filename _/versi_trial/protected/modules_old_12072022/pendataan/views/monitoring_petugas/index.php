...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entry Pendataan Monitoring Pajak</h4>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	$("#nomor_pajak").change(function(event) {
		generateAutocompleteWPHotel();
	});

	function generateAutocompleteWPHotel() {
		var koderek = $('#TREKENING_KODE').select2('val');
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'pendataan/monitoring_petugas/autocompletewphotel?koderek='+koderek,
	...
	if (TBLDAFTAR_NOPOK=='') {
		notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
			...
			$.ajax({
				url: 'pendataan/monitoring_petugas/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'pendataan/monitoring_petugas/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
					...
					tgl : ($('#is_tanggal').prop('checked') ? $('#masapajak_tanggal').val() : '0'),
					rekening : $('#TREKENING_KODE').select2('val')
				},
				success: function(respon) {
					if (respon.data=='ada') {
						if (respon==false) {
						...
						/*} else if(respon.data=='teguran'){
							...
							if (parseInt(respon.TGLENTRI)==0) {
								$.SmartMessageBox({
									...
									if (ButtonPressed === "Ya") {
										$.ajax({
											url: 'pendataan/monitoring_petugas/delt',
											type: 'POST',
											dataType: 'json',
											data: {
												nomor_pajak : CARI_NOPOK,
										...
										.done(function(respon) {
											if (respon.status=='success') {
												...
												notifikasi('Informasi', 'Silakan isikan pendataan', 'success', 1, 0);
											}
										})
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										if (respon.data=='proses selanjutnya') {
											...
											$('#hit_penjualanbulan').val(respon.OMSETPAJAK);
											$('#hit_pajak').val(respon.PAJAK);
											$('#hit_bungaspt').val(respon.BUNGASPTPD);

											$('#hit_totalsetor').val(eval(respon.PAJAK)+eval(respon.BUNGASPTPD));

											$('#wp_input_tgl_spt').val((respon.TGLSPTPD)+'-'+(respon.BLNSPTPD)+'-'+'20'+(respon.THNSPTPD));
											
											$('#hit_tglterimaspt').val((respon.TGLTERIMA)+'-'+(respon.BLNTERIMA)+'-'+'20'+(respon.THNTERIMA));

											$('#hit_tglbatasspt').val((respon.TGLBATASSPTPD)+'-'+(respon.BLNBATASSPTPD)+'-'+'20'+(respon.THNBATASSPTPD));
	...
	function kosongkan_rupiah(NOPOK,TGLENTRI){
		$.ajax({
			url: 'pendataan/monitoring_petugas/kosongkan_rp',
			type: 'POST',
			dataType: 'json',
			data: {
				 NOPOK : NOPOK
	...
	function getTglBatas(THNPAJAK,BLNPAJAK,KODEREK) {
		$.ajax({
			url: 'pendataan/monitoring_petugas/GetTGLBatas',
			type: 'POST',
			dataType: 'json',
			data: {
				 THNPAJAK : THNPAJAK
	...
	function simpan() {
		...
		$.ajax({
			url: 'pendataan/monitoring_petugas/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
				rekening : $('#TREKENING_KODE').select2('val'),
			...
			success:function (respon) {
				if (respon.data=='tidak' && respon.status=='success') {
					...
					loadingTransfer('close');
					notifikasi('Sukses','Data Berhasil Diperbarui', 'success',1,0);
				} else {
					//loadingTransfer('close');
