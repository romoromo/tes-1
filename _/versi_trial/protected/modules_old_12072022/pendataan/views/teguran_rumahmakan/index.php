...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i>  Teguran SPTPD Pajak Rumah Makan</h4>
		</div>
	</div>
</div>
				...
				<header role="heading">
					...
					<div role="content">
						...
						<div class="widget-body">
									...
									<fieldset id="form_perhitungan" style="display: none;">
										...
										<div class="row">
											...
											<section class="col col-md-4">
												<label class="input">
													<input style="background: #e4e4e4;" disabled="disabled" class="input-sm format-rupiah text-align-right" type="text" id="hit_penjualanhari" name="hit_penjualanhari">
												</label>
											</section>
											<section class="col col-md-2">
												<p>Jumlah Hari</p>
											...
											<section class="col col-md-2">
												<label class="input">
													<input style="background: #e4e4e4;" disabled="disabled" class="input-sm text-align-right" type="text" value="30" id="hit_jumlahhari" name="hit_jumlahhari">
								...
								<div class="row">
									...
									<section class="col col-md-2">
										<label class="input-group"> 
											...
											<input style="background: #e4e4e4;" disabled="disabled" class="form-control text-align-right format-rupiah" type="text" name="hit_pajak" id="hit_pajak">
										</label>
									</section>
									<section class="col col-md-1">
										Bunga SPT 
									...
									<section class="col col-md-2">
										<label class="input-group"> 
											<span class="input-group-addon">Rp</span>
											<input style="background: #e4e4e4;" disabled="disabled" class="form-control text-align-right format-rupiah" type="text" name="hit_bungaspt" id="hit_bungaspt">
										</label>
									</section>
									<section class="col col-md-1">
										Total Setor 
									...
									<section class="col col-md-2">
										<label class="input-group"> 
											<span class="input-group-addon">Rp</span>
											<input style="background: #e4e4e4;" disabled="disabled" class="form-control text-align-right format-rupiah" type="text" name="hit_totalsetor" id="hit_totalsetor" disabled="disabled" style="background: #e4e4e4;">
										</label>
									</section>
								</div>
							</fieldset>
							...
							<footer id="footer-rumahmakan" style="display: none;">
								<section class="col col-md-2">
								</section>
								<section class="col col-md-2">
...
<script type="text/javascript">
	...
	function cekPernahDaftar() {
		...
		if (TBLDAFTAR_NOPOK=='') {
			...
			$.ajax({
				url: '<?php echo $this->modul_url ?>/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				...
				success: function(respon) {
					if (respon.data=='ada') { // pernah lapor SPT
						if (respon==false) {
							notifikasi('Data yang anda cari tidak ditemukan', 'Data dengan nomor '+$('#nomor_pajak').val()+' Tidak ditemukan', 'failed', 1,0);

						} else {
							if (parseInt(respon.<?= $this->namatabel ?>_TGLENTRI)==0) {
								$.SmartMessageBox({
									title : "Informasi", // judul Smart Alert
									content : "Sudah ada Salinan SPT, Apakah data ini akan dihapus?", // konten dari smart alert
									buttons : '[Tidak][Ya]' // pengaturan tombol
								}, function(ButtonPressed) {
									if (ButtonPressed === "Ya") {
										$.ajax({
											url: '<?php echo $this->modul_url ?>/delt',
											type: 'POST',
											dataType: 'json',
											data: {
												nomor_pajak : CARI_NOPOK,
							...
							$.SmartMessageBox({
								title : "Informasi", // judul Smart Alert
								content : "Data dengan Nomor Pokok "+$('#nomor_pajak').val()+", Masa Pajak "+$('#masapajak_bulan').select2('val')+" "+$('#masapajak_tahun').val()+" sudah pernah melapor SPT. <br> Apakah anda ingin mengubah data ini?", // konten dari smart alert
								buttons : '[Tidak][Ya]' // pengaturan tombol
							}, function(ButtonPressed) {
								if (ButtonPressed === "Ya") {
									...
									if (respon.data=='proses selanjutnya') {
										...
										$('#no_subrek').select2('val',respon.REKENING_KODE);
											...
											$('#hit_penjualanbulan').val(respon.<?= $this->namatabel ?>_OMSETPAJAK);
											$('#hit_pajak').val(respon.<?= $this->namatabel ?>_PAJAK);
											$('#hit_bungaspt').val(respon.<?= $this->namatabel ?>_BUNGASPTPD);

											$('#hit_totalsetor').val(eval(respon.<?= $this->namatabel ?>_PAJAK)+eval(respon.<?= $this->namatabel ?>_BUNGASPTPD));

											$('#wp_input_tgl_spt').val((respon.<?= $this->namatabel ?>_TGLSPTPD)+'-'+(respon.<?= $this->namatabel ?>_BLNSPTPD)+'-'+'20'+(respon.<?= $this->namatabel ?>_THNSPTPD));
											
											$('#hit_tglterimaspt').val((respon.<?= $this->namatabel ?>_TGLTERIMA)+'-'+(respon.<?= $this->namatabel ?>_BLNTERIMA)+'-'+'20'+(respon.<?= $this->namatabel ?>_THNTERIMA));

											$('#hit_tglbatasspt').val((respon.<?= $this->namatabel ?>_TGLBATASSPTPD)+'-'+(respon.<?= $this->namatabel ?>_BLNBATASSPTPD)+'-'+'20'+(respon.<?= $this->namatabel ?>_THNBATASSPTPD));

											//$('#hit_tglentryspt').val((respon.<?= $this->namatabel ?>_TGLENTRI)+'-'+(respon.<?= $this->namatabel ?>_BLNENTRI)+'-'+'20'+(respon.<?= $this->namatabel ?>_THNENTRI));
											$('#hit_tglentryspt').val('<?php echo date('d-m-Y');?>');

											if (respon.<?= $this->namatabel ?>_TGLENTRI==0) {
					...
					} else { // belum pernah lapor SPT
						if (respon.data=='proses selanjutnya') { // sudah bayar
...
function kosongkan_rupiah(NOPOK,TGLENTRI){
	$.ajax({
		url: '<?php echo $this->modul_url ?>/kosongkan_rp',
		type: 'POST',
		dataType: 'json',
		data: {
			NOPOK : NOPOK
...
function getTglBatas(THNPAJAK,BLNPAJAK,KODEREK) {
	$.ajax({
		url: '<?php echo $this->modul_url ?>/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			THNPAJAK : THNPAJAK
...
function simpan() {
	...
	if($('#is_tanggal').is(':checked')){
		...
		isi_masapajak_tanggal = '';
	}
	$.ajax({
		url: '<?php echo $this->modul_url ?>/simpandata',
		type: 'POST',
		dataType: 'json',
		data: {
