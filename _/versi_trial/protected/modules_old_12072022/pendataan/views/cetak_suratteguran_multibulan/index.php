...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Cetak Surat Teguran Multi Masa Pajak</h4>
		</div>
	</div>
</div>
						...
						<form action="" id="form_sumber_pajak" class="smart-form" novalidate>
							<fieldset>
								...
								<div class="row">
									...
									<section class="col col-md-2">
										<label class="select">
											<select class="select2" id="TBLPENDATAAN_BLNPAJAK2" name="TBLPENDATAAN_BLNPAJAK2">
												<option value="01">Januari</option>
												<option value="02">Februari</option>
												<option value="03">Maret</option>
												<option value="04">April</option>
...
<script type="text/javascript">
    ...
    function cetak () {
    	...
    	if ($('#nomor_surat').val()=='') {
			...
			var q = daftaridPajak;
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/cetak_suratteguran_multibulan/cetaksuratteguran/?data=' + q +"&TBLPENDATAAN_THNPAJAK="+$('#TBLPENDATAAN_THNPAJAK').val()+"&TBLPENDATAAN_BLNPAJAK="+$('#TBLPENDATAAN_BLNPAJAK').select2('val')+"&TBLPENDATAAN_BLNPAJAK2="+$('#TBLPENDATAAN_BLNPAJAK2').select2('val')+"&nomor_surat="+$('#nomor_surat').val() );
    	}    	
    	// window.open(window.APP_URL+'/pendataan/cetak_suratteguran_multibulan/cetaksuratteguran');
	}
    ...
    function cetaksemua() {
    	...
    	if ($('#nomor_surat').val()=='') {
    		...
    		/*$.ajax({
				url: 'pendataan/cetak_suratteguran_multibulan/cetaksemua',
				type: 'POST',
				data: {
			...
			// var q = daftaridPajak;
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/cetak_suratteguran_multibulan/cetaksemua/?data='+"&TBLPENDATAAN_THNPAJAK="+$('#TBLPENDATAAN_THNPAJAK').val()+"&TBLPENDATAAN_BLNPAJAK="+$('#TBLPENDATAAN_BLNPAJAK').select2('val')+"&TBLPENDATAAN_BLNPAJAK2="+$('#TBLPENDATAAN_BLNPAJAK2').select2('val')+"&nomor_surat="+$('#nomor_surat').val()+"&rekening="+$("#TREKENINGSUB_KODE").val() );
    	}
	}	
	...
	function cari() {
		...
		$.ajax({
			url: 'pendataan/cetak_suratteguran_multibulan/cari',
			type: 'POST',
			data: {
				rekening: $("#TREKENINGSUB_KODE").val()
				, TBLPENDATAAN_THNPAJAK: $("#TBLPENDATAAN_THNPAJAK").val()
				, TBLPENDATAAN_BLNPAJAK: $("#TBLPENDATAAN_BLNPAJAK").select2('val')
				, TBLPENDATAAN_BLNPAJAK2: $("#TBLPENDATAAN_BLNPAJAK2").select2('val')
				, TBLDAFTAR_NOPOK : $("#TBLDAFTAR_NOPOK").val()
			},
		})
