...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i>  Teguran SPTPD Pajak Air Tanah</h4>
		</div>
	</div>
</div>
								...
								<div id="myTabContent1" class="tab-content padding-10" style="">
									<div class="tab-pane fade active in" id="s1">
										...
										<div class="row">
											...
											<section class="col col-md-3">
												<label class="input">
													<input style="background-color: #d8ddcf;" readonly="readonly" type="text" name="TBLDAFTTANAH_BUNGASPTPD" id="TBLDAFTTANAH_BUNGASPTPD">
												</label>
											</section>
...
function generateAutocompleteWPAirTanah() {
	...
	serviceUrl: 'pendataan/pajak_airtanah2017/autocompletewpairtanah',
...
function WPAirTanah2(nopokok) {
	$.ajax({
		url: 'pendataan/pajak_airtanah2017/autocompletewpairtanah2',
		type: 'POST',
		dataType: 'json',
		data: {
			nopokok : nopokok
...
function cekPernahDaftar() {
	...
	if (TBLDAFTAR_NOPOK=='') {
		...
		$.ajax({
			url: 'pendataan/pajak_airtanah2017/CekPernahDaftar',
			type: 'POST',
			dataType: 'json',
			data: {
...
function cari() {
	...
	if (TBLDAFTAR_NOPOK=='') {
		...
		$.ajax({
		url: 'pendataan/pajak_airtanah2017/GetWPAirTanah',
		type: 'POST',
		dataType: 'json',
		data: {
			 nopokok: $('#TBLDAFTAR_NOPOK').val()
		...
		success: function(respon) {
			...
			exclude = [];
			toDuit = ['TBLDAFTTANAH_NPA','TBLDAFTTANAH_NPATARIF','TBLDAFTTANAH_NPA','TBLDAFTTANAH_PAJAK','TBLDAFTTANAH_RUPIAHSETOR'];
			...
			$("#TBLDAFTTANAH_NPA").val( formatRibuan(parseInt(respon['TBLDAFTTANAH_OMSETPAJAK'])) );
			$("#TBLDAFTTANAH_NPA").trigger('keyup');
			...
			$("#TBLDAFTTANAH_TGLSPTPD").val( isikiri(respon['TBLDAFTTANAH_TGLSPTPD'], '00') + '-' +isikiri(respon['TBLDAFTTANAH_BLNSPTPD'], '00') + '-' +isikiri(respon['TBLDAFTTANAH_THNSPTPD'], '2000')  );
			hitungpajak();
			setTimeout(function() {
				hitungpajak();
			}, 1000);
...
function getTarifRekening(kodesubrek) {
	$.ajax({
		url: 'pendataan/pajak_airtanah2017/GetKodeRek',
		type: 'POST',
		dataType: 'json',
		data: {
...
function simpanairtanah () {
	$.ajax({
		url: 'pendataan/pajak_airtanah2017/SimpanAirTanah',
		type: 'post',
		dataType: "json",
...
function getTglBatas(THNPAJAK,BLNPAJAK,KODEREK) {
	$.ajax({
		url: 'pendataan/pajak_airtanah2017/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			 THNPAJAK : THNPAJAK
...
function hitungpajak() {
	...
	if (NPA=='') {
		...
		if ($('#tanpa_bunga').is(':checked')) {
			BUNGABLN = "";
			BUNGARP = "";
			$('#TBLDAFTTANAH_BUNGASPTPD').val(BUNGARP);
		} else {
