...
class Monitoring_pajakController extends Controller
{
	...
	public function actionautocomplete()
	{
		...
		$filter_AND = array(
			'EQ__REFBADANUSAHA.REFBADANUSAHA_REKAYAT' => $rekkode
	...
	public function actionSimpanDetailResto()
	{
		...
		$qcekexist = 'SELECT COUNT(*) FROM TBLMONITORING_DETAIL WHERE NOPOK='.$nopok.' AND THP='.$thn.' AND BLP='.$bln;
		$cekexist = Yii::app()->db->createCommand($qcekexist)->queryScalar();
		...
		if ($cekexist>0) {
			...
			$simpan = $command->update('TBLMONITORING_DETAIL', $arrayOfData ,'NOPOK =:nopok AND THP=:tahun AND BLP=:bulan',array(':nopok' => $nopok,':tahun' => $thn,':bulan' => $bln));
		} else {
			$arrayOfData = array(
				'NOPOK'=>$nopok,
				'TGL_MONITORING'=>$this->dateformat($tglentri),
