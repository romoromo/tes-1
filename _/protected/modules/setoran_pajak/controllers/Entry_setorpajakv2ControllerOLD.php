...
class Entry_setorpajakv2Controller extends Controller
{
	...
	public function actiongetDataSetorBPD()
	{
		...
		if (!$setoranbpd) {
			echo CJSON::encode(array('status'=>'notfound', 'message'=>'Referensi Belum ADA.....!!!!!, Check Tanggal Penetapan...!!!'));
			Yii::app()->end();
		}
		...
		if (strtoupper($jenis_setoran)=='SKPD-A') {
			...
			$buskp = $setoranbpd['TBLSETORANBPD_BUNGAKETETAPAN'];
			$ddskp = $setoranbpd['TBLSETORANBPD_DENDAKETETAPAN'];
			$caption = !empty($jenis_setoran) ? $jenis_setoran : 'SPTPD';
		} elseif (strtoupper($jenis_setoran)=='STPD') {
			# SKPD/SPTPD
			$bunga = $setoranbpd['TBLSETORANBPD_BUNGATAGIHAN'];
			$denda = $setoranbpd['TBLSETORANBPD_DENDATAGIHAN'];
	...
	public function actionSimpan()
	{
		...
		$whereUpdate = 
			...
			AND TBLSETORANBPD_JNSKETETAPAN = :jnsketetapan
			";
		;
		...
		$arrayOfDataInsert = array(
			...
			,'TBLSETOR_JNSKETETAPAN' => $setbpd[$NAMATABEL.'_JNSKETETAPAN']
			,'KDSU' => ''
			,'TBLSETOR_JNSBAYAR' => $caption
			,'TBLSETOR_TGLSURAT' => ''
			,'TBLSETOR_BLNSURAT' => ''
			,'TBLSETOR_THNSURAT' => ''
			,'TBLSETOR_NOMORSURAT' => ''
		...
		if (!empty($bunga_setor)) {
			...
			$arrayOfDataInsert['TBLSETOR_JNSKETETAPAN'] = $setbpd[$NAMATABEL.'_JNSKETETAPAN'];
