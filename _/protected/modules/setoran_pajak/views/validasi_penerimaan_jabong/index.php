...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="well well-sm well-light bg-color-greenLight txt-color-white">
            <h4><i class="fa fa-file-text-o"></i> Validasi Penerimaan Jabong</h4>
        </div>
    </div>
</div>
						...
						<form action="" id="form-setor" class="smart-form" novalidate>
							<fieldset>
								...
								<div class="row">
									...
									<section class="col col-md-2">
										<label class="input">
											<input class="input-sm" type="number" value="" id="tanggalpajak" name="tanggalpajak">
										</label>
									</section>
								</div>
								<div class="row">
									...
									<div class=""><br>
										...
										<div  style="padding: 3%;">
											...
											<div class="row">
												...
												<section class="col col-md-4">
													<label class="input">
														<input disabled="" type="text" name="TBLJABONGBPD_NOMORSSPD" id="TBLJABONGBPD_NOMORSSPD">
													</label>
												</section>
											</div>
											<div class="row">
												...
												<section class="col col-md-2">
													<label class="input"> <i class="icon-append fa fa-calendar"></i>
														<input disabled="" type="text" value="<?= date('d-m-Y') ?>" name="TBLJABONGBPD_TGLSSPD" class="datepicker" data-dateformat="dd-mm-yy" id="TBLJABONGBPD_TGLSSPD">
													</label>
												</section>
											</div>
											<div class="row">
										...
										<footer>
											...
											<button type="button" onclick="location.reload()" class="btn btn-sm btn-warning">
												Batal
											</button>
...
<script type="text/javascript">
	...
	function generateAutocompleteWP() {
		$('#nopok').autocomplete({
			serviceUrl: 'setoran_pajak/validasi_penerimaan_jabong/autocompletewp',
	...
	function cari() {
		$.ajax({
			url: 'setoran_pajak/validasi_penerimaan_jabong/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
			...
			success:function (respon) {
				...
				if (respon.data.validate=='kurang') {
                    ...
                    notifikasi('Data tidak ditemukan', 'data dengan nomor pokok '+$('#nopok').val()+' lokasi '+$('#pajakke').val()+' tidak ditemukan', 'fail', 1,0);
                    $("#btnSimpan").hide();
                    return false;
                } else if (respon.data.exist!=='ada') {
					notifikasi('Maaf', 'Referensi dari BPD Belum ADA!!!', 'fail', 1,0);
					$("#btnSimpan").hide();
					return false;
				} else if (respon.data.exist_validasi=='ada') {
                    notifikasi('Data sudah pernah diinputkan', 'data dengan nomor pokok '+$('#nopok').val()+' lokasi '+$('#pajakke').val()+' sudah divalidasi', 'fail', 1,0);
                    $("#btnSimpan").hide();
	...
	function simpan() {
		var addparam = {
            TBLJABONGBPD_NOMORSSPD: $("#TBLJABONGBPD_NOMORSSPD").val()
		};
        $("#btnSimpan").attr('disabled', 1);
        $.ajax({
            url: 'setoran_pajak/validasi_penerimaan_jabong/simpandata',
            type: 'POST',
            dataType: 'json',
	...
	function cetak() {
		...
		window.open('setoran_pajak/validasi_penerimaan_jabong/cetak/?'+jQuery.param(param));
	}
