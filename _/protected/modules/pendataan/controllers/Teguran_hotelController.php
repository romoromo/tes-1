...
class Teguran_hotelController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 1;
	var $PAJAK_REK = '4.1.1.1.0';
	var $namatabel = 'TBLDAFTHOTEL';
	var $modul_url = 'pendataan/teguran_hotel';
	...
	public function actiongetdata()
	{
		...
		$tglawal = '1-'.$bulan.'-'.$tahun;
		$tglakhir = date('t-m-Y',strtotime($tglawal));
		$data['AWAL_PAJAK'] = $tglawal;
		$data['AKHIR_PAJAK'] = $tglakhir;
		$data['JUMLAH_HARI'] = date('t',strtotime($tglawal));
		$jmlhari = 30;
	...
	public function actionsimpandata()
	{
		...
		if ($cek>0) {
			...
			$simpan = $insert->insert(''.$this->namatabel.'', array(
				...
				,''.$this->namatabel.'_OMSETPAJAK'=>$omzet
				,''.$this->namatabel.'_PERSENTARIF'=>$tarif_rekening
				,''.$this->namatabel.'_THNSPTPD'=>substr($tahun_pajak,-2)
				,''.$this->namatabel.'_BLNSPTPD'=>$bulan_pajak
				,''.$this->namatabel.'_TGLSPTPD'=>'1'
				,''.$this->namatabel.'_THNTERIMA'=>substr($tahun_pajak,-2)
				,''.$this->namatabel.'_BLNTERIMA'=>$bulan_pajak
				,''.$this->namatabel.'_TGLTERIMA'=>'1'
				,''.$this->namatabel.'_THNBATASSPTPD'=>substr($tahun_pajak,-2)
				,''.$this->namatabel.'_BLNBATASSPTPD'=>$bulan_pajak
				,''.$this->namatabel.'_TGLBATASSPTPD'=>$arr_tglbatassptpd[0]
				,''.$this->namatabel.'_THNENTRI'=>substr($tahun_pajak,-2)
