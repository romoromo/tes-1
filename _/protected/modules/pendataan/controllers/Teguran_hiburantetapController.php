...
class Teguran_hiburantetapController extends Controller
{
    var $KODE_GOL = 4;
    var $PAJAK_AYAT = 0;
    ...
    public function actionGetWPHiburan()
    {
        ...
        $data = Yii::app()->db->createCommand('SELECT * FROM TBLDAFTHIBURAN WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryRow();
    ...
    public function actionSimpanHiburan()
    {
        ...
        $REKENING_KODE = isset($_REQUEST['REKENING_KODE']) && !empty($_REQUEST['REKENING_KODE']) ? $_REQUEST['REKENING_KODE'] : 0;
        ...
        $TBLDAFTHIBURAN_ISJNSPENETAPAN = isset($_REQUEST['TBLDAFTHIBURAN_ISJNSPENETAPAN']) && !empty($_REQUEST['TBLDAFTHIBURAN_ISJNSPENETAPAN']) ? $_REQUEST['TBLDAFTHIBURAN_ISJNSPENETAPAN'] : 0;
        ...
        if($ada_data=='belum'){
            ...
            $simpan = $insert->insert('TBLDAFTHIBURAN', array(
                ...
                // 2020-11-17 15:15; update tgl sptpd 1 & entri adalah 0
                // 'TBLDAFTHIBURAN_TGLSPTPD' => $exp_TGLSPTPD[0],
                'TBLDAFTHIBURAN_TGLSPTPD' => 1,
                // 2020-11-17 15:55; update thn bulan sptpd & entri dari thn bulan pajak
                // 'TBLDAFTHIBURAN_BLNSPTPD' => $exp_TGLSPTPD[1],
                // 'TBLDAFTHIBURAN_THNSPTPD' => substr($exp_TGLSPTPD[2], -2),
                'TBLDAFTHIBURAN_BLNSPTPD' => $TBLPENDATAAN_BLNPAJAK,
                'TBLDAFTHIBURAN_THNSPTPD' => substr($TBLPENDATAAN_THNPAJAK, -2),
                ...
                'TBLDAFTHIBURAN_TGLENTRI' => 0,
                // 2020-11-17 15:55; update thn bulan sptpd & entri dari thn bulan pajak
                // 'TBLDAFTHIBURAN_BLNENTRI' => $exp_TGLENTRI[1],
                // 'TBLDAFTHIBURAN_THNENTRI' => substr($exp_TGLENTRI[2], -2),
                'TBLDAFTHIBURAN_BLNENTRI' => $TBLPENDATAAN_BLNPAJAK,
                'TBLDAFTHIBURAN_THNENTRI' => substr($TBLPENDATAAN_THNPAJAK, -2),
                ...
                'TBLDAFTHIBURAN_ISJNSPENETAPAN' => $TBLDAFTHIBURAN_ISJNSPENETAPAN,
                'TBLDAFTHIBURAN_REKJENIS' => $TBLREKENING_REKJENIS,
                'TBLDAFTHIBURAN_JNSHIBURAN' => 'T',
            ...
            $simpan = $command->update('TBLDAFTHIBURAN', $column ,'TBLDAFTAR_NOPOK =:NOPOK AND TBLPENDATAAN_THNPAJAK =:THNPAJAK AND TBLPENDATAAN_BLNPAJAK =:BLNPAJAK',array(':NOPOK' => $TBLDAFTAR_NOPOK, ':THNPAJAK' =>substr($TBLPENDATAAN_THNPAJAK, -2), ':BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK));
            if ($simpan>0) {
                echo CJSON::encode(array('status'=>'success'));
            }
            else{
    ...
    public function actionCekPernahDaftar()
    {
        ...
        $cek = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTHIBURAN WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryScalar();
        if ($cek) {
