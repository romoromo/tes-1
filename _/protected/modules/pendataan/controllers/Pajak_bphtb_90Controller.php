...
class Pajak_bphtb_90Controller extends Controller
{
	var $KODE_GOL = 1;
	var $PAJAK_AYAT = 16;
	var $PAJAK_SUBJEN = 1;
	var $PAJAK_REK = '4.1.1.16.0';
	var $MODULE_URL = 'pendataan/pajak_bphtb_90';
	public function actionIndex()
	{
		// $data['sub_rek'] = TRekening::model()->getRekeningSubAktif('4160100');
		/*SELECT * 
		FROM TBLREKENING
		...
		WHERE TBLREKENING_KODE LIKE '4.1.1.16.%'
		ORDER BY TBLREKENING_KODE
		")
		->queryAll();
	...
	public function actiongetdata()
	{
		...
		/*} else if ($teguran==0) {
			...
			$otherquery = array(
				...
				'limit'=> 30,
				'join'=> array('REFBADANUSAHA_90 REFBADANUSAHA', 'REFBADANUSAHA.REFBADANUSAHA_ID = TBLDAFTAR.REFBADANUSAHA_IDPEMILIK'),
				'join_2'=> array('TBLDAFTBPHTB', 'TBLDAFTAR.TBLDAFTAR_NOPOK = TBLDAFTBPHTB.TBLDAFTAR_NOPOK'),
				'order'=> 'TBLDAFTAR.TBLDAFTAR_NOPOK ASC',
			);
			$model = DBFetch::query( array('select'=>$select,'from'=>$from,'filter'=>$filter,'filter_AND'=>$filter_AND,'filterOR'=>true,'otherquery'=>$otherquery,'mode'=>'DETAIL') );
	...
	public function actionsimpandata()
	{
		...
		$column = array('TBLPENDATAAN_THNPAJAK'=>substr($tahun_pajak, -2)
				    ...
				    ,'TBLDAFTBPHTB_REKAYAT'=>$this->PAJAK_AYAT
				    ,'TBLDAFTBPHTB_REKJENIS'=>$refbadanusaha_rekjenis
				    ,'TBLDAFTBPHTB_REKSUBJENIS'=>$refbadanusaha_reksubjenis
	...
	public function cekNPOPTKP($nopok)
	{
		...
		from TBLDAFTAR t1 
		join REFBADANUSAHA_90 t2 on t2.REFBADANUSAHA_ID = t1.REFBADANUSAHA_IDPEMILIK
		join \"refbphtbtarif\" t3 on t3.AYT=t2.REFBADANUSAHA_REKAYATOLD and t3.JEN=t2.REFBADANUSAHA_REKJENISOLD
