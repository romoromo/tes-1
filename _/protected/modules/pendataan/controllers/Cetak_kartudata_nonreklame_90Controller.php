...
class Cetak_kartudata_nonreklame_90Controller extends Controller
{
	var $MODULE_URL = 'pendataan/cetak_kartudata_nonreklame_90';
	...
	public function getData()
	{
		...
		$wherethnpajak = $tahunpjk!=0 ? (' AND '.$namaTBL.'.TBLPENDATAAN_THNPAJAK='.substr($tahunpjk, -2)) : '';
		$whereblnpajak = $bulanpjk!=0 ? (' AND '.$namaTBL.'.TBLPENDATAAN_BLNPAJAK='.$bulanpjk) : '';
		$wheretglpajak = $tglpjk!=0 ? (' AND '.$namaTBL.'.TBLPENDATAAN_TGLPAJAK='.$tglpjk) : '';
		...
		$ISONLINE = $_REQUEST['ISONLINE'] ? $_REQUEST['ISONLINE'] : 'X';
		if ($ISONLINE=='T') {
			$WHEREISONLINE = (" AND IS_CREATE LIKE '%ESPTPD%'") ? (" AND IS_CREATE LIKE '%ESPTPD%'") : "";
		} elseif ($ISONLINE=='F') {
			$WHEREISONLINE = (" AND IS_CREATE LIKE '%MAPATDA%'") ? (" AND IS_CREATE LIKE '%MAPATDA%'") : "";
		} else {
			$WHEREISONLINE = "";
		}
		...
		$wheretahunsptpd = $tahunsptpd!=0 ? (' AND '.$namaTBL.'.'.$namaTBL.'_THNENTRI='.$tahunsptpd) : '';
		$wherebulansptpd = $bulansptpd!=0 ? (' AND '.$namaTBL.'.'.$namaTBL.'_BLNENTRI='.$bulansptpd) : '';
		// $wheretanggalsptpd = (' AND '.$namaTBL.'_TGLENTRI='.$_REQUEST['ENTRI_TGLPAJAK']) ? (' AND '.$namaTBL.'_TGLENTRI='.$_REQUEST['ENTRI_TGLPAJAK']) : '';
		if ($_REQUEST['ENTRI_TGLPAJAK']=='') {
			$wheretanggalsptpd = '';
		} else {
			$wheretanggalsptpd = (' AND '.$namaTBL.'.'.$namaTBL.'_TGLENTRI='.$_REQUEST['ENTRI_TGLPAJAK']);
		}
		// $rek = isset($_REQUEST['rekening']) && !empty($_REQUEST['rekening']) ? $_REQUEST['rekening'] : '';
		// switch ( $rek ) {
		// 	case '4.1.1.1.0':
