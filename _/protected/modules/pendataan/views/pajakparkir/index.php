...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entry Pendataan Pajak Parkir</h4>
		</div>
	</div>
</div>
						...
						<form id="form_sumber_pajak" class="smart-form" novalidate="">
							...
							<fieldset id="form_perhitungan" style="display: none;">
								...
								<div class="row">
									...
									<section class="col col-md-2">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input type="text" name="persen_bunga" id="persen_bunga">
										</label>
									</section>
									<!-- <section class="col col-md-2"></section>
									<section class="col col-md-2">
...
<script type="text/javascript">
	...
	function generateAutocompleteWP() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'pendataan/pajakparkir/autocompletewp',
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'pendataan/pajakparkir/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
								...
								buttons : '[Tidak][Ya]' // pengaturan tombol
									...
									if (ButtonPressed === "Ya") {
										...
										if (respon.data=='proses selanjutnya') {
											...
											$('#persen_bunga').val(respon.PERSEN_DENDA);
											...
											window.pajak = respon.TBLDAFTPARKIR_PAJAK;
	...
	function simpan() {
		...
		$.ajax({
			url: 'pendataan/pajakparkir/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
	...
	// $("#persen_bunga").change(function(event) {
	// 	hitungPajak();
	// });

	$('#persen_bunga').keyup(function(){
	  hitungPajak();
	});
	...
	function hitungPajak() {
		...
		if ($('#tanpa_bunga').is(':checked')) {
			if (bungabln>0) {
				window.bunga_spt = window.pajak* (persen_bunga/100);
