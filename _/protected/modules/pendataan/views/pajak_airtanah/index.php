...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file fa-fw"></i> Enrty Pendataan Pajak Air Tanah</h4>
		</div>
	</div>
</div>
						...
						<fieldset id="form-data-perhitungan" style="display:none;">
							<div class="widget-body">
								<ul id="myTab1" class="nav nav-tabs bordered" style="">
									<li class="active">
										<a href="#s1" data-toggle="tab" ><span class="badge bg-color-blue txt-color-white">1</span> PDAM</a>
									</li>
								</ul>
								...
								<div id="myTabContent1" class="tab-content padding-10" style="">
									<div class="tab-pane fade active in" id="s1">
										...
										<div class="row">
											...
											<section class="col col-md-2">
												<label class="input">
													<input type="text" class="format-ribuan" name="TBLDAFTTANAH_NPA" id="TBLDAFTTANAH_NPA">
												</label>
											</section>
										</div>
										<div class="row">
										...
										<div class="row">
											...
											<section class="col col-md-3">
												<label class="input">
													<input style="background-color: #d8ddcf;" readonly="readonly" type="text" name="TBLDAFTTANAH_DENDA" id="TBLDAFTTANAH_DENDA">
												</label>
											</section>
										</div>
										<div class="row">
											...
											<section class="col col-md-3">
												<label class="input">
													<input type="text" name="TBLDAFTTANAH_PAJAK" id="TBLDAFTTANAH_PAJAK">
												</label>
											</section>
										</div>
										<div class="row">
											...
											<section class="col col-md-3">
												<label class="input">
													<input type="text" readonly="readonly" style="background-color: #d8ddcf;" name="TBLDAFTTANAH_RUPIAHSETOR" id="TBLDAFTTANAH_RUPIAHSETOR">
												</label>
											</section>
											<section class="col col-md-3">
...
jQuery(document).ready(function($) {
	...
	window.mm = today.getMonth(); //January is 0! rahmat 16-02-2022 set bulan pajak
...
function generateAutocompleteWPAirTanah() {
	...
	serviceUrl: 'pendataan/pajak_airtanah/autocompletewpairtanah',
...
function cekPernahDaftar() {
	...
	if (TBLDAFTAR_NOPOK=='') {
		...
		$.ajax({
			url: 'pendataan/pajak_airtanah/CekPernahDaftar',
			type: 'POST',
			dataType: 'json',
			data: {
...
function cari() {
	...
	if (TBLDAFTAR_NOPOK=='') {
		...
		$.ajax({
		url: 'pendataan/pajak_airtanah/GetWPAirTanah',
		type: 'POST',
		dataType: 'json',
		data: {
			 nopokok: window.nopokok
		...
		success: function(respon) {
			...
			$('#TBLDAFTTANAH_TOTALVOLUME').val(respon.TBLDAFTTANAH_TOTALVOLUME);
			$('#TBLDAFTTANAH_NPA').val(respon.TBLDAFTTANAH_OMSETPAJAK);
			$('#TBLDAFTTANAH_NPATARIF').val(respon.TBLDAFTTANAH_OMSETPAJAK);
			$('#TBLDAFTTANAH_DENDA').val(respon.TBLDAFTTANAH_DENDA);
			$('#TBLDAFTTANAH_PAJAK').val(respon.TBLDAFTTANAH_PAJAK);
			$('#TBLDAFTTANAH_RUPIAHSETOR').val(respon.TBLDAFTTANAH_RUPIAHSETOR);
...
function setMasaPajak() {
	...
	/*el = "#tbltransaksiketetapan_tglentrisptpd, #tbltransaksiketetapan_tglterimasptpd, #tbltransaksiketetapan_omzettotal, #tbltransaksiketetapan_rupiahdenda, #tbltransaksiketetapan_pajak";
		...
		$("#TBLDAFTTANAH_JUMLAHHARIJUAL").val(masapajak.endOf('month').format('L').split('-')[0]);
		// if (c!='') {
...
function getTarifRekening(kodesubrek) {
	$.ajax({
		url: 'pendataan/pajak_airtanah/GetKodeRek',
		type: 'POST',
		dataType: 'json',
		data: {
...
function simpanairtanah () {
	$.ajax({
		url: 'pendataan/pajak_airtanah/SimpanAirTanah',
		type: 'post',
		dataType: "json",
		data:  $("#form-pendataan-airtanah").serialize()+'&idkecamatan='+window.idkecamatan+'&idkelurahan='+window.idkelurahan,
...
$('#TBLDAFTTANAH_NPA').keyup(function() {
	setPriceFormat();
	var NPA = $('#TBLDAFTTANAH_NPA').val();
	$('#TBLDAFTTANAH_NPATARIF').val(NPA);
});

function getTglBatas(THNPAJAK,BLNPAJAK,KODEREK) {
	$.ajax({
		url: 'pendataan/pajak_airtanah/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			 THNPAJAK : THNPAJAK
...
function hitungpajak() {
	...
	var NPA = toAngka($('#TBLDAFTTANAH_NPA').val());
	var NPATARIF = toAngka($('#TBLDAFTTANAH_NPA').val());

	if (NPA=='') {
		notifikasi('Informasi','Mohon isikan Nilai Perolehan Air','failed',1,0);
	}
	else{
		PAJAK = Math.round((TARIF/100)*NPATARIF);
		BUNGARP = 0;
		// BUNGABLN = window.bulanbunga; rahmat 16-02-2022
		// if (BUNGABLN > 0) {
		// 	BUNGARP = Math.ceil(((BUNGABLN*2)/100)* PAJAK);
		// 	$('#TBLDAFTTANAH_DENDA').val(BUNGARP);
		// }
		...
		$('#TBLDAFTTANAH_PAJAK').val(PAJAK);
		$('#TBLDAFTTANAH_RUPIAHSETOR').val(TOTALSETOR);
	}
}
</script>