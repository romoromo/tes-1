...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entry Pendataan Pajak Reklame Kendaraan, Selebaran</h4>
		</div>
	</div>
</div>
	...
	<script type="text/javascript">
		...
		function generateAutocompleteWP() {
			$('#TBLDAFTAR_NOPOK').autocomplete({
				serviceUrl: 'pendataan/reklame_dll/autocompletewp',
		...
		function cari() {
			...
			if (TBLDAFTAR_NOPOK=='') {
				...
				$.ajax({
					url: 'pendataan/reklame_dll/getdata',
					type: 'POST',
					dataType: 'json',
					data: {
...
function cetakPengambilan() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklame_dll/cetakPengambilan?');
}

function cetakJabong() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklame_dll/cetakJabong?');
}

function cetakIzinReklame() {
	window.open('<?= Yii::app()->getBaseUrl(1); ?>/pendataan/reklame_dll/cetakIzinReklame?');
}
...
function simpanPendataan() {
	...
	$.ajax({
		url: 'pendataan/reklame_dll/simpanPendataan',
		type: 'POST',
		dataType: 'json',
