						...
						<table id="dt_basic_pipeline" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr>
									...
									<th data-hide="phone,tablet">Kode Rekening</th>
									<th></th>
								</tr>
							</thead>
							<tbody>
			...
			<div class="modal-body no-padding">
				...
				<form id="form-data" class="smart-form">
					...
					<fieldset>
						...
						<section>
							<div class="row">
								<label class="label col col-4">Nama Jabatan</label>
								<div class="col col-10">
									<label class="input"> <i class="icon-append fa fa-square"></i>
										<input value="" type="text" name="param[REFJABATAN_NAMA]" id="REFJABATAN_NAMA">
									</label>
						...
						<section>
							<div class="row">
								...
								<div class="col col-10">
									<label class="select">
										<select style="width:100%" class="select2" id="TBLREKENING_KODE" name="param[TBLREKENING_KODE]" >
											<option value="">== Pilih Rekening ==</option>
												<?php foreach ($data['list_rekening'] as $key): ?>
												<option value="<?php echo $key['TBLREKENING_KODE'] ?>"><?php echo $key['TBLREKENING_KODE'] ?> | <?php echo $key['TBLREKENING_NAMAREKENING'] ?></option>
											<?php endforeach ?>
										</select>
									</label>
								</div>
...
// DO NOT REMOVE : GLOBAL FUNCTIONS!
	...
	function runFormValidation() {
		var $FormData = $("#form-data").validate({
				...
				rules : {
					"param[REFJABATAN_NAMA]" : {
						required : true
					}
					,"param[TBLREKENING_KODE]" : {
						required : false
					}
				},
				...
				messages : {
					"param[REFJABATAN_NAMA]" : {
						required : 'Mohon isikan nama jabatan'
					}
					,"param[TBLREKENING_KODE]" : {
						required : 'Mohon isikan rekening'
					}
				},
...
/*//form validation*/
	...
	function loadDataTable() {
		...
		var ajaxUrl = "app/jabatan/DTJSON" // url that fetches the data for the table
		// drawTable($('#dt_basic_pipeline'), 1, totalRecords, ajaxUrl);  //Initializes the datatable
		COLUMN  = [
			{ "data": "num" }
			,{ "data": "REFJABATAN_NAMA" }
			,{ "data": "TBLREKENING_KODE" }
			,{ "data": "REFJABATAN_ID", "render": function ( data, type, full, meta ) {
