<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Imbauan SKPD Angsuran </h4>
		</div>
	</div>
</div>
...
<section id="widget-grid">
	...
	<div class="row">
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget jarviswidget-color-teal" id="wid-id-w463ejwsehjdekjswjedk" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-custombutton="false" data-widget-fullscreenbutton="false" data-widget-colorbutton="false" data-widget-togglebutton="false" data-widget-sortable="false">
				<!-- widget options:
...
<script type="text/javascript">
	...
	function cari() {
		if ($('#TBLREKENING_KODE').val() == '') {
			...
			$.ajax({
				url: 'penagihan/Himbauan_skpda_2021/cari',
				type: 'POST',
				data: {
	...
	function cetakWord() {
		...
		if ($('#nomor_surat').val() == '') {
			...
			var q = daftaridPajak;
			$.ajax({
					url: 'penagihan/Himbauan_skpda_2021/simpan',
					type: 'POST',
					data: {
						data: daftaridPajak,
						TBLREKENING_KODE: $("#TBLREKENING_KODE").val(),
	...
	function cetakdaftar() {
		...
		var q = daftaridPajak;
		window.open('<?= Yii::app()->getBaseUrl(1); ?>/penagihan/Himbauan_skpda_2021/cetakdaftar/?data=' + q + "&TBLPENDATAAN_THNPAJAK=" + $('#TBLPENDATAAN_THNPAJAK').val() + "&TBLPENDATAAN_THNPAJAK_AKHIR=" + $('#TBLPENDATAAN_THNPAJAK_AKHIR').val() + "&TBLDAFTAR_NOPOK=" + $('#TBLDAFTAR_NOPOK').val() + "&TBLREKENING_KODE=" + $('#TBLREKENING_KODE').val() + "&nomor_surat=" + $('#nomor_surat').val() + "&tglawal=" + $('#tglawal').val() + "&tglakhir=" + $('#tglakhir').val() + "&waktuawal=" + $('#waktuawal').val() + "&waktuakhir=" + $('#waktuakhir').val() + "&tempt_undangan=" + $('#tempt_undangan').val() + "&tanggalsurat=" + $('#tanggalsurat').val());
	}
	// window.open(window.APP_URL+'/pendataan/cetak_suratteguran/cetaksuratteguran');
