...
<table width="100%" border="1" style="border-collapse:collapse;" cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed table-hover smart-form has-tickbox">
    <thead>
        <tr>
            <th colspan="18" style="text-align-last:left;">
                LAPORAN TUNGGAKAN <?php echo $data['judul'] ?> TAHUN <?php echo $masapajak = $_REQUEST['masapajak'] ?> SD <?php echo $masapajakakhir = $_REQUEST['masapajakakhir'] ?><br>
    ...
    <tbody style="text-align: center;">
        <?php $no = 1;
        foreach ($data['tunggakan'] as $list) : ?>
            <tr>
                <td>
                    <?php echo $no++; ?>
                    <!-- NO -->
                ...
                <td>
                    <?php echo $list['TBLDAFTAR_BADANUSAHANAMA'] ?>
                    <!-- NAMA -->
                </td>
                <td>
                    <?php echo $list['TBLDAFTAR_GOLONGAN'] ?>.<?php echo sprintf('%07d', $list['TBLDAFTAR_NOPOK']) ?>.<?php echo sprintf('%02d', $list['TBLKECAMATAN_IDBADANUSAHA']) ?>.<?php echo sprintf('%02d', $list['TBLKELURAHAN_IDBADANUSAHA']) ?>
                    <!-- NPWPD -->
                </td>
                <td>
                    <?php echo $list['TBLDAFTAR_BADANUSAHAALAMAT'] ?>
                    <!-- ALAMAT -->
                </td>
                <td><?php
                    ...
                    ?></td>
                <td>
                    20<?php echo sprintf('%02d', $list['TBLPENDATAAN_THNPAJAK']) ?>
                    <!-- TAHUN -->
                </td>
                <td>
                    <?php echo LokalIndonesia::ribuan($list['TUNGAKAN_JANUARI']) ?>
                    <!-- JANUARI -->
                </td>
                <td>
