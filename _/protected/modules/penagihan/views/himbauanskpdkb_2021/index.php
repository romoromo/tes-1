			...
			$.ajax({
				...
				.done(function() {
					...
					for (var i = '#'; i.length <= 7; i += '#') {
						// console.log(value4);
						window.location.assign('<?= Yii::app()->getBaseUrl(1); ?>/penagihan/himbauanskpdkb_2021/cetakwordall/?data=' + q + "&TBLPENDATAAN_THNPAJAK=" + i + "&TBLDAFTAR_NOPOK=" + $('#TBLDAFTAR_NOPOK').val() + "&TBLREKENING_KODE=" + $('#TBLREKENING_KODE').val() + "&no_surat=" + $('#no_surat').val() + "&tglawal=" + $('#tglawal').val() + "&tglakhir=" + $('#tglakhir').val() + "&waktuawal=" + $('#waktuawal').val() + "&waktuakhir=" + $('#waktuakhir').val() + "&tempat_undangan=" + $('#tempat_undangan').val() + "&tanggalcutoff=" + $('#tanggalcutoff').val() + "&tanggalsurat=" + $('#tanggalsurat').val());
					}
				})
				.fail(function() {
					notifikasi('Informasi System', 'Simpan histori himbauan gagal', 'failed', 1, 0);
