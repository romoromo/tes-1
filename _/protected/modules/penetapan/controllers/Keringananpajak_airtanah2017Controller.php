...
class Keringananpajak_airtanah2017Controller extends Controller
{
	...
	// public $namatabel = 'TBLDAFTTANAH_TESTBED'; // untuk tes ke sptibt_testbed
	public $namatabelesptpd = 'T_TRANSAKSI';
	// public $namatabelesptpd = 'T_TRANSAKSI_TESTBED'; // untuk tes ke t_transaksi_testbed
	...
	public function actionGetWPAirTanah()
	{
		...
		$data = Yii::app()->db->createCommand('SELECT '.$this->namatabel.'.*,NVL(TBLDAFTTANAH_NOMORSKPD,0) AS NOKOHIR FROM '.$this->namatabel.' WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK.' AND NVL(TBLPENDATAAN_TGLPAJAK,0) = 0')->queryRow();
		echo CJSON::encode($data);
	}
	...
	public function actionSimpanAirTanah()
	{
		...
		$PENGGUNA_ID = Yii::app()->user->getId();

		if($ada_data=='sudah_ditetapkan' || $ada_data=='sudah_pernah_ringan'){
			...
			$filter = array(
				...
				';
				$simpan = Yii::app()->db->createCommand()->update(''.$this->namatabel.'', $arrayUpdate, $condition, $filter);
			
				$simpanlog = $insert->insert('TBLLOG_PENETAPAN', $arrayInsert);

			$tahun4digit = '20'.substr($TBLPENDATAAN_THNPAJAK, -2);

			$cekesptpd = Yii::app()->db->createCommand('SELECT COUNT(T_IDWPOBJEK) AS JML FROM ESPTPD.'.$this->namatabelesptpd.' WHERE ESPTPD.'.$this->namatabelesptpd.'.T_PERIODEPAJAK ='.$tahun4digit.' AND EXTRACT(MONTH FROM ESPTPD.'.$this->namatabelesptpd.'.T_MASAAWAL) = '.$TBLPENDATAAN_BLNPAJAK.' AND ESPTPD.'.$this->namatabelesptpd.'.T_IDWPOBJEK='.$TBLDAFTAR_NOPOK)->queryScalar();

			if ($cekesptpd) {
				$updateesptpd = Yii::app()->db->createCommand();
				$arrayUpdateesptpd = array(
					'T_JMLHPAJAK' => LokalIndonesia::toAngka($TBLDAFTTANAH_RUPIAHSETOR),
				);
				$filteresptpd = array(
					':thnpajake' => $tahun4digit
					,':blnpajake' => $TBLPENDATAAN_BLNPAJAK
					,':nopoke' => $TBLDAFTAR_NOPOK
				);
				$conditionesptpd = '
					ESPTPD.'.$this->namatabelesptpd.'.T_PERIODEPAJAK  = :thnpajake
					AND EXTRACT(MONTH FROM ESPTPD.'.$this->namatabelesptpd.'.T_MASAAWAL)  = :blnpajake
					AND ESPTPD.'.$this->namatabelesptpd.'.T_IDWPOBJEK = :nopoke
					';
				$simpanesptpd = Yii::app()->db->createCommand()->update('ESPTPD.'.$this->namatabelesptpd.'', $arrayUpdateesptpd, $conditionesptpd, $filteresptpd);
	...
	public function cekPernahData($filter=array())
	{
		...
		return $cek = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM '.$this->namatabel.' WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryScalar();
	}
	...
	public function actionCekPernahDaftar()
	{
		...
		if ($cekPernahKeringanan) {
				...
				$cek = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM '.$this->namatabel.' WHERE TBLPENDATAAN_THNPAJAK ='.$THNPAJAK.' AND TBLPENDATAAN_BLNPAJAK='.$BLNPAJAK.' AND TBLDAFTAR_NOPOK='.$NOPOKOK)->queryScalar();
			if ($cek) {
				...
				if ($cekSKPD) {
					...
					echo CJSON::encode(array('status'=>'belum_ditetapkan'));
				}
				
			} else {
				echo CJSON::encode(array('status'=>'belum_spt'));
			}
		}
