...
class Keringanan_restoranController extends Controller
{
	var $KODE_GOL = 3;
	var $PAJAK_AYAT = 2;
	...
	{
		$model = Yii::app()->db->createCommand('SELECT TBLDAFTRMAKAN_RUPIAHSETOR FROM TBLDAFTRMAKAN WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND TBLPENDATAAN_BLNPAJAK='.$bulan.' AND NVL(TBLPENDATAAN_BLNPAJAK,0)='.(int)$tanggal.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	...
	public function actionsimpandata()
	{
		...
		if ($cek>0) {
		  ...
		  //   	'TBLKELURAHAN_ID'=>$kelurahan_pajak,
			    ...
			    'TBLDAFTRMAKAN_PERSENRINGAN'=>$hit_keringanan,
			...
			$simpan = $command->update('TBLDAFTRMAKAN', $column ,'TBLDAFTAR_NOPOK =:NOPOK AND TBLPENDATAAN_THNPAJAK =:THNPAJAK AND TBLPENDATAAN_BLNPAJAK =:BLNPAJAK AND NVL(TBLPENDATAAN_TGLPAJAK,0) =:TGLPAJAK',array(':NOPOK' => $nopokok, ':THNPAJAK' =>substr($tahun_pajak, -2), ':BLNPAJAK' => $bulan_pajak, ':TGLPAJAK' => (int)$tanggal_pajak));
			if ($simpan) {
				$res['status'] = 'update';
			}else{
				$res['status'] = 'failed';
	...
	{
		$model = Yii::app()->db->createCommand('SELECT COUNT(TBLPENDATAAN_THNPAJAK) AS JML FROM TBLDAFTRMAKAN WHERE TBLPENDATAAN_THNPAJAK ='.$thn.' AND TBLPENDATAAN_BLNPAJAK='.$bulan.' AND NVL(TBLPENDATAAN_TGLPAJAK,0)='.(int)$tanggal.' AND TBLDAFTAR_NOPOK='.$nopok)->queryScalar();
		return $model;
	}
