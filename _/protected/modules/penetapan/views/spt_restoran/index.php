...
<script type="text/javascript">
	...
	jQuery(document).ready(function($) {
		...
		if(thnpajak>0) {$('#masapajak_tahun').val(thnpajak);}
	...
	function generateAutocompleteWPResto() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'penetapan/spt_restoran/autocompletewphotel',
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'penetapan/spt_restoran/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function simpan() {
		...
		$.ajax({
			url: 'penetapan/spt_restoran/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
