	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			<div class="well well-sm well-light bg-color-greenLight txt-color-white">
				<h4><i class="fa fa-file-text-o"></i> Angsuran SPTPD - Pajak Restoran </h4>
			</div>
		</div>
	</div>
	...
	<section id="widget-grid" class="" >
		...
		<div class="row">
			...
			<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
				...
				<div class="jarviswidget jarviswidget-color-teal" id="wid-id-kiorssrestoran" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-sortable="false">
					<header>
					...
					<div>
						...
						<div class="widget-body no-padding">
							<form id="form_angsuran_pajakrestoran" class="smart-form" novalidate="">
								<br><fieldset>
									<div class="row">
										...
										<section class="col col-md-4">
											<h4 align="left">Entry Penetapan Angsuran Pajak Restoran</h4>
										</section>
									</div>
								...
								<div class="row">
									...
									<section class="col col-md-2">Tanggal SPTPD</section>
									<section class="col col-md-2">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input type="text" readonly="" name="<?= $this->namatabel ?>_TGLSPTPD" class="datepicker" data-dateformat="dd-mm-yy" id="<?= $this->namatabel ?>_TGLSPTPD" style="
								...
								<div class="row">
									...
									<section class="col col-md-2">Total SPTPD</section>
									<section class="col col-md-2">
										<label class="input">
											<input class="form-control format-rupiah" readonly="" type="text" id="<?= $this->namatabel ?>_PAJAK" name="<?= $this->namatabel ?>_PAJAK" style="
											background-color: beige;
											">
										</label>
									</section>
								...
								<div class="row">
								<section class="col col-md-2" style="margin-left: 2%;">Tanggal Batas SPTPD </section>

								<section class="col col-md-2">
									<label class="input"> <i class="icon-append fa fa-calendar"></i>
										<input type="text" readonly="" name="<?= $this->namatabel ?>_TGLBATASSPTPD" class="datepicker" data-dateformat="dd-mm-yy" id="<?= $this->namatabel ?>_TGLBATASSPTPD" style="
										background-color: beige;
										">
									</label>
								</section>
								...
								<section class="col col-md-2">
									<label class="input">
										<input class="form-control format-rupiah" readonly="" type="text" id="<?= $this->namatabel ?>_TOTALPAJAK" name="<?= $this->namatabel ?>_TOTALPAJAK" style="
										background-color: beige;
										">
									</label>
								</section>
	...
	function generateAutocompleteWPrestoran() {
		$('#TBLDAFTAR_NOPOK').autocomplete({
			serviceUrl: '<?php echo $this->modul_url ?>/autocompletewprestoran',
	...
	function hitungtotal() {
		...
		tglbts = $('#<?= $this->namatabel ?>_TGLBATASSPTPD').val();
		tglentr = $('#TGLENTRI').val();
		rupiahkb = toAngka($('#<?= $this->namatabel ?>_PAJAK').val());
		...
		bunga = 0; // yang membuat bunga keterlambatan jadi 0s
		...
		$.ajax({
			...
			success: function(respon) {
				if (respon.data=="pajak > 0") {
					...
					$('#<?= $this->namatabel ?>_PAJAK').val(respon.<?= $this->namatabel ?>_PAJAK);
					$('#<?= $this->namatabel ?>_TOTALPAJAK').val(respon.<?= $this->namatabel ?>_PAJAK);
					$('#<?= $this->namatabel ?>_TGLSPTPD').val(respon.<?= $this->namatabel ?>_TGLSPTPD +'-'+ respon.<?= $this->namatabel ?>_BLNSPTPD +'-20'+respon.<?= $this->namatabel ?>_THNSPTPD);
					$('#<?= $this->namatabel ?>_TGLSPTPD2').val(respon.<?= $this->namatabel ?>_TGLSPTPD +'-'+ respon.<?= $this->namatabel ?>_BLNSPTPD +'-20'+respon.<?= $this->namatabel ?>_THNSPTPD);
					$('#<?= $this->namatabel ?>_TGLBATASSPTPD').val(respon.<?= $this->namatabel ?>_TGLBATASSPTPD +'-'+ respon.<?= $this->namatabel ?>_BLNBATASSPTPD +'-20'+ respon.<?= $this->namatabel ?>_THNBATASSPTPD);
					...
					if (respon.cekAngsur=="ada" || respon.cekAngsurSetor=="ada") {
						...
						$('#<?= $this->namatabel ?>_TGLBATAS').val(respon.<?= $this->namatabel ?>_TGLBATASSPTPD +'-'+ respon.<?= $this->namatabel ?>_BLNBATASSPTPD +'-20'+ respon.<?= $this->namatabel ?>_THNBATASSPTPD);
									...
									$.SmartMessageBox({
										...
										if (ButtonPressed=='Ya') {
											$.ajax({
												...
												type: 'POST',
												dataType: 'json',
												data: $("#form_angsuran_pajakrestoran").serialize()+'&id='+window.idSurat+'&idObyek='+window.idObyek+'&cmd='+window.cmd+"&tblketetapanangsuran="+tblketetapanangsuran+"&totalkb="+window.totalkb,
											})
					...
					notifikasi('Informasi', 'Pajak 0 Rupiah', 'failed', 1,0);
	...
	function listDetail() {
		...
		for(var i in tblketetapanangsuran){
			...
			total_tarifangsur += Number( (rs.pokokangsur) );
			$("#_listDetail_").append(
					/*<tr>
						<td>Angsuran ke</td>
				...
				'<tr>'
					...
					+'<td><div align="right">'+formatRibuan( Number((rs.rpskp) )) +'</div></td>'
	...
	function hitungAngsuran() {
		tabeldetail = [];
		byterhutangsekarang = byterhutang = Number( toAngka( $("#<?= $this->namatabel ?>_TOTALPAJAK").val() ) );
		kaliangsur = parseInt( $("#<?= $this->namatabel ?>_JUMLAHANGSUR").val() );
		totalangs = toAngka($("#<?= $this->namatabel ?>_TOTALPAJAK").val() );
		...
		for (var i = 0; i <= kaliangsur-1; i++) {
			...
			tarifangsur2 = 0; // yang bikin bunga menempel jadi 0
	...
	function cetakketetapan() {
		tblketetapanangsuran = localStorage["tblketetapanangsuran"];
		var data = $("#form_angsuran_pajakrestoran").serialize()+'&id='+window.idSurat+'&idObyek='+window.idObyek+'&cmd='+window.cmd+"&tblketetapanangsuran="+tblketetapanangsuran+"&totalkb="+window.totalkb;
	...
	function cetakperjanjian() {
		tblketetapanangsuran = localStorage["tblketetapanangsuran"];
		var data2 = $("#form_angsuran_pajakrestoran").serialize()+'&id='+window.idSurat+'&idObyek='+window.idObyek+'&cmd='+window.cmd+"&tblketetapanangsuran="+tblketetapanangsuran+"&totalkb="+window.totalkb;
	...
	function getNoANG(tahun) {
			$.ajax({
				url: 'penetapan/angsuranspt_restoran/GetNoANG',
				type: 'POST',
				dataType: 'json',
				data: {
					tahun: tahun
