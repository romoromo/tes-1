									...
									<div class="row">
										...
										<section class="col col-md-1">
											<label class="checkbox pull-right">
												<input type="checkbox" name="is_minus" id="is_minus">
												<i></i>
											</label>
										</section>
										<section class="col col-md-1">
...
<br>
	...
	<script type="text/javascript">
		...
		$("#TBLDAFTHOTEL_BUNGAKRGBAYAR, #TBLDAFTHOTEL_PAJAKPERIKSA, #TBLDAFTHOTEL_BUNGAKRGBAYAR, #TBLDAFTHOTEL_KENAIKANKRGBAYAR, #is_minus").change(function(event) {
			HitungSKPDKB();
			setPriceFormat();
		});
		...
		function HitungSKPDKB() {
			...
			if($('#is_minus').is(':checked')){
				RUPIAHKB = Number(BUNGAKB) + Number(KENAIKAN) - Number(PJKPERIKSA);
			} else {
							...
							$.SmartMessageBox({
								...
								buttons : '[Perubahan][Cetak ulang]' // pengaturan tombol
									...
									if (ButtonPressed === "Perubahan") {
										...
										if (respon.CEKMINUS=='minus') {
											$('#is_minus').prop('checked',true);
										}
		...
		function simpanskpdkb () {
			...
			var serialize = $("#form-skpdkb-hotel").serialize()
		
			$.ajax({
				url: 'penetapan/skpdkb_hotel/SimpanSKPDKBHotel1',
				type: 'post',
				dataType: "json",
				data:  `${serialize}&${paramurl}`,
				// data:  `${serialize}&${paramurl}`,
