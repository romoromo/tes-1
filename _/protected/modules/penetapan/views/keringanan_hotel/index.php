...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file fa-fw"></i> Keringanan Pajak Hotel</h4>
		</div>
	</div>
</div>
						...
						<form id="form_sumber_pajak" class="smart-form" novalidate="">
							<fieldset>
								...
								<div class="row">
									...
									<section class="col col-md-5">
										<label class="input">
											<input readonly="" class="input-sm" type="text" id="wp_input_no_spt" name="wp_input_no_spt">
										</label>
									</section>
								</div>
								<div class="row">
									...
									<section class="col col-md-3">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input readonly="" type="text" name="wp_input_tgl_spt" value="<?php echo date('d-m-Y'); ?>" class="datepicker" data-dateformat="dd-mm-yy" id="wp_input_tgl_spt">
										</label>
									</section>
								</div>
							</fieldset>
							<fieldset id="form_perhitungan" style="display: none;">
								...
								<div class="row">
									...
									<section class="col col-md-2">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input readonly="" type="text" name="hit_tanggalawal" class="datepicker" data-dateformat="dd-mm-yy" id="hit_tanggalawal">
										</label>
									</section>
									...
									<section class="col col-md-2">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input readonly="" type="text" name="hit_tanggalakhir" class="datepicker" data-dateformat="dd-mm-yy" id="hit_tanggalakhir">
										</label>
									</section>
								</div>
							...
							<fieldset id="form_dokumentasi_tanggal" style="display: none;">
								...
								<div class="row">
									...
									<section class="col col-md-4">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input readonly="" class="form-control datepicker" value="<?php echo date('d-m-Y'); ?>" type="text" name="hit_tglterimaspt" data-dateformat="dd-mm-yy" id="hit_tglterimaspt">
										</label>
									</section>
								</div>
								<div class="row">
								...
								<div class="row">
									...
									<section class="col col-md-4">
										<label class="input"> <i class="icon-append fa fa-calendar"></i>
											<input readonly="" class="form-control datepicker" value="<?php echo date('d-m-Y'); ?>" type="text" name="hit_tglentryspt" data-dateformat="dd-mm-yy" id="hit_tglentryspt">
										</label>
									</section>
								</div>
								<div class="row">
...
<script type="text/javascript">
	...
	jQuery(document).ready(function($) {
		...
		if (nopokok > 0) {
			$('#nomor_pajak').val(nopokok);
		}
		if (thnpajak > 0) {
			$('#masapajak_tahun').val('20' + thnpajak);
		}
		if (blnpajak > 0) {
			$('#masapajak_bulan').select2('val', blnpajak);
		}
		if (tglpajak > 0) {
			$('#masapajak_tanggal').select2('val', tglpajak);
		}
		if (nopokok > 0 || thnpajak > 0 || blnpajak > 0 || tglpajak > 0) {
			cari();
		}
	});
	...
	function generateAutocompleteWPHotel() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'penetapan/keringanan_hotel/autocompletewphotel',
	...
	function cekPernahDaftar() {
		...
		var TGLPAJAK = ($('#is_tanggal').prop('checked') ? $('#masapajak_tanggal').val() : '0')
		...
		if (TBLDAFTAR_NOPOK == '') {
			...
			$.ajax({
				url: 'penetapan/keringanan_hotel/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK == '') {
			...
			$.ajax({
				url: 'penetapan/keringanan_hotel/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
									...
									buttons : '[Tidak][Ya]' // pengaturan tombol
										...
										if (ButtonPressed === "Ya") {
											if (respon.data=='proses selanjutnya') {
												...
												kosongkan_rupiah(respon.TBLDAFTAR_NOPOK,respon.TBLDAFTHOTEL_TGLENTRI);
											}
									}
								});
								return false;*/
						} else {
							// if (parseInt(respon.TBLDAFTHOTEL_TGLENTRI)==0 && respon.) {
							if (parseInt(respon.TBLDAFTHOTEL_TGLENTRI) == 0) {
								$.SmartMessageBox({
									...
									if (ButtonPressed === "Ya") {
										$.ajax({
												url: 'penetapan/keringanan_hotel/delt',
												type: 'POST',
												dataType: 'json',
												data: {
													nomor_pajak: CARI_NOPOK,
							...
							$.SmartMessageBox({
								...
								if (ButtonPressed === "Ya") {
									...
									if (respon.data == 'proses selanjutnya') {
										...
										// $('#hit_penjualanbulan').val(respon.TBLDAFTHOTEL_OMSETPAJAK);
										$('#hit_penjualanbulan').val(respon.TBLDAFTHOTEL_PAJAK);
										$('#hit_pajak').val(respon.TBLDAFTHOTEL_PAJAK);
										$('#hit_bungaspt').val(respon.TBLDAFTHOTEL_BUNGASPTPD);

										$('#hit_totalsetor').val(eval(respon.TBLDAFTHOTEL_PAJAK) + eval(respon.TBLDAFTHOTEL_BUNGASPTPD));

										$('#wp_input_tgl_spt').val((respon.TBLDAFTHOTEL_TGLSPTPD) + '-' + (respon.TBLDAFTHOTEL_BLNSPTPD) + '-' + '20' + (respon.TBLDAFTHOTEL_THNSPTPD));

										$('#hit_tglterimaspt').val((respon.TBLDAFTHOTEL_TGLTERIMA) + '-' + (respon.TBLDAFTHOTEL_BLNTERIMA) + '-' + '20' + (respon.TBLDAFTHOTEL_THNTERIMA));

										$('#hit_tglbatasspt').val((respon.TBLDAFTHOTEL_TGLBATASSPTPD) + '-' + (respon.TBLDAFTHOTEL_BLNBATASSPTPD) + '-' + '20' + (respon.TBLDAFTHOTEL_THNBATASSPTPD));

										//$('#hit_tglentryspt').val((respon.TBLDAFTHOTEL_TGLENTRI)+'-'+(respon.TBLDAFTHOTEL_BLNENTRI)+'-'+'20'+(respon.TBLDAFTHOTEL_THNENTRI));
										$('#hit_tglentryspt').val('<?php echo date('d-m-Y'); ?>');

										if (respon.TBLDAFTHOTEL_TGLENTRI == 0) {
										...
										window.BUNGASPTPD = respon.TBLDAFTHOTEL_BUNGASPTPD
	...
	function kosongkan_rupiah(NOPOK, TGLENTRI) {
		$.ajax({
			url: 'penetapan/keringanan_hotel/kosongkan_rp',
			type: 'POST',
			dataType: 'json',
			data: {
				NOPOK: NOPOK,
	...
	function getTglBatas(THNPAJAK, BLNPAJAK, KODEREK) {
		$.ajax({
			url: 'penetapan/keringanan_hotel/GetTGLBatas',
			type: 'POST',
			dataType: 'json',
			data: {
				THNPAJAK: THNPAJAK,
	...
	function simpan() {
		...
		$.ajax({
			url: 'penetapan/keringanan_hotel/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
