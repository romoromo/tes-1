	...
	<script type="text/javascript">
		...
		function simpanskpdkb () {
			...
			$.ajax({
				...
				type: 'post',
				dataType: "json",
				data:  $("#form-skpdkb-hiburan").serialize()+'&idkecamatan='+window.idkecamatan+'&idkelurahan='+window.idkelurahan+'&tanggal='+tanggal,
				...
				buttons : '[Tidak][Ya]' // pengaturan tombol
					...
					if (ButtonPressed === "Ya") {
						$.ajax({
							...
							type: 'post',
							dataType: "json",
							data:  $("#form-skpdkb-hiburan").serialize()+'&TANGGAL='+TANGGAL,
							success: function(data) {
								if (data.status=="success") {
									notifikasi('Sukses','Data Berhasil Disimpan', 'success',1,0);
									$('#btnsimpan').attr('disabled','');
