...
<script type="text/javascript">
	...
	function generateAutocompleteWPHotel() {
		$('#nomor_pajak').autocomplete({
			serviceUrl: 'penetapan/spt_hotel/autocompletewphotel',
	...
	if (TBLDAFTAR_NOPOK=='') {
		notifikasi('Informasi','Mohon isikan No Pokok WP','failed',1,0);
			...
			$.ajax({
				url: 'penetapan/spt_hotel/CekPernahDaftar',
				type: 'POST',
				dataType: 'json',
				data: {
	...
	function cari() {
		...
		if (CARI_NOPOK=='') {
			...
			$.ajax({
				url: 'penetapan/spt_hotel/getdata',
				type: 'POST',
				dataType: 'json',
				data: {
				...
				success: function(respon) {
					if (respon.data=='ada') {
						notifikasi('Sudah pernah lapor SPT', 'Data SPT dengan nopok '+$('#nomor_pajak').val()+' pernah dilaporkan', 'failed', 1,0);
						// if (respon==false) {
						...
						// 					$('#hit_totalsetor').val(respon.TBLDAFTHOTEL_PAJAK);

						// 					$('#wp_input_tgl_spt').val((respon.TBLDAFTHOTEL_TGLSPTPD)+'-'+(respon.TBLDAFTHOTEL_BLNSPTPD)+'-'+'20'+(respon.TBLDAFTHOTEL_THNSPTPD));
											
						// 					$('#hit_tglbatasspt').val((respon.TBLDAFTHOTEL_TGLTERIMA)+'-'+(respon.TBLDAFTHOTEL_BLNTERIMA)+'-'+'20'+(respon.TBLDAFTHOTEL_THNTERIMA));

						// 					$('#hit_tglterimaspt').val((respon.TBLDAFTHOTEL_TGLBATASSPTPD)+'-'+(respon.TBLDAFTHOTEL_BLNBATASSPTPD)+'-'+'20'+(respon.TBLDAFTHOTEL_THNBATASSPTPD));
						...
						if (respon.data=='proses selanjutnya') {
							...
							notifikasi('Data sudah dibayar', 'Data SPT dengan nopok '+$('#nomor_pajak').val()+' Tidak bisa di edit', 'failed', 1,0);
						} else {
							$('#res_nama_wp').val(respon.TBLDAFTAR_BADANUSAHANAMA);
							$('#res_alamat_wp').val(respon.TBLDAFTAR_BADANUSAHAALAMAT);
							$('#no_subrek').select2('val',respon.REKENING_KODE);
	...
	$.ajax({
		url: 'penetapan/spt_hotel/GetTGLBatas',
		type: 'POST',
		dataType: 'json',
		data: {
			 THNPAJAK : THNPAJAK
	...
	function simpan() {
		...
		$.ajax({
			url: 'penetapan/spt_hotel/simpandata',
			type: 'POST',
			dataType: 'json',
			data: {
