...
<div class="well well-sm well-light text-center">
	<h4><i class="fa fa-tasks"></i> Salinan Cetak SK Pengukuhan Dan NPWPD</h4>
</div>
...
<script type="text/javascript">
	...
	function loadDataTable() {
		...
		var ajaxUrl = "pendaftaran/cetak_skpengukuhan/DTJSON?"+url_param // url that fetches the data for the table
		...
		COLUMN  = [
			{ "data": "TNPWPD_ID", "orderable": false, "render": function ( data, type, row, meta ) {
				...
				return '<button rel="tooltip" data-placement="right" data-original-title="Digunakan untuk mengkukuhkan obyek pajak"  class="btn btn-circle btn-warning btn-sm" onclick="pengukuhan(\''+row.TNPWPD_ID+'\')"><i class="fa fa-check"></i></button>'
					...
					return '<a rel="tooltip" data-placement="right" data-original-title="Digunakan untuk mencetak surat pengukuhan obyek pajak" target="_blank"  class="btn btn-circle btn-primary btn-sm" href="<?= Yii::app()->getBaseUrl(1); ?>/pendaftaran/cetak_skpengukuhan/cetak/?id='+ btoa( row.TNPWPD_ID )+')"><i class="fa fa-print"></i></button>'
					+ '<a rel="tooltip" data-placement="right" data-original-title="Digunakan untuk mencetak kartu NPWPD" target="_blank"  class="btn btn-circle bg-color-magenta txt-color-white btn-sm" href="<?= Yii::app()->getBaseUrl(1); ?>/pendaftaran/cetak_skpengukuhan/cetakkartu?npwpd='+ btoa( row.TNPWPD_MILIKNAMA )+'&namawp='+ btoa( row.TNPWPD_MILIKNAMA )+'&alamatwp='+ btoa( row.TNPWPD_MILIKALAMAT )+'&tgltap='+ btoa( row.TNPWPD_TGLKUKUH )+'&jenispajak='+ btoa( row.TBLMASTERREKENING_NAMA )+')"><i class="fa fa-print"></i></button>'
				}
				;
			 }
			}
	...
	function pengukuhan(id) {
		...
		$.ajax({
			url: 'pendaftaran/cetak_skpengukuhan/getdata',
			type: 'post',
			dataType: "json",
			data: {
				id: id,
	...
	function simpan() {
		$.ajax({
			url: 'pendaftaran/cetak_skpengukuhan/simpan',
			type: 'post',
			dataType: 'json',
	...
	function getNoKukuh(tgl) {
		$.post('pendaftaran/cetak_skpengukuhan/generateNOKUKUH', {tgl: tgl,obyid: window.idKukuh}, function(respon) {
