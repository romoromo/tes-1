...
class EntripajakreklametetapController extends Controller
{
		...
		if ($tahun=="") {
			...
			$model = Yii::app()->db->createCommand("SELECT ".$this->namatabel.".*, TBLDAFTAR.*, ".$this->namatabel."_REKURUSAN || '.' || 
							...
							WHERE (
								...
								AND NVL(TBLPENDATAAN_BLNPAJAK, 0)=0
	...
	public function cekpernahsetor($tahun,$lokasi,$nopok)
	{
		$model = Yii::app()->db->createCommand("
			...
			WHERE TBLPENDATAAN_THNPAJAK = ".(int) substr($tahun,-2)." AND NVL(TBLPENDATAAN_PAJAKKE, 0) = {$lokasi} AND NVL(TBLPENDATAAN_BLNPAJAK, 0) = 0 AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = 0 AND TBLDAFTAR_NOPOK =".$nopok." AND ".$this->namatabel."_REGSETOR IS NOT NULL AND ".$this->namatabel."_RUPIAHSETOR > 0")->queryRow();
		if ($model['JML']>0) {
			return 'ada';
		}else{
			return 'tidak';
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$namafileDL = "SSPD-Reklame-Tetap.docx"; 
