...
class EntrisetoranpajakparkirController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		if($jenis_setoran=='SPTPD') {
			...
			$kolomdenda = "".$this->namatabel."_NAKB";
		...
		if ($exist=='ada') {
			...
			if ($update) {
				...
				if ($jenis_setoran == 'SPTPD') {
					...
					$TBLSETORANBPD_DENDAKETETAPAN= (isset($model[$this->namatabel . '_NAKB']) ? $model[$this->namatabel . '_NAKB'] : 0) + $dendasetor;
					$TBLSETORANBPD_DENDAKURANGBAYAR = isset($model[$this->namatabel . '_NAKB']) ? $model[$this->namatabel . '_NAKB'] : 0;
	...
	public function actionCetakSSPD()
	{
        ...
        if ($data['jenis_setoran']=='STPD') {
        ...
        } else if ($data['jenis_setoran'] == 'SPTPD') {
        	$filter = array(
	            'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK
	            ,'EQ__'.$NAMATABEL.'.TBLPENDATAAN_BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK
			...
			$filter = array(
				'EQ__TBLDAFTAR.TBLDAFTAR_NOPOK' => $TBLDAFTAR_NOPOK, 'EQ__' . $NAMATABEL . '.TBLPENDATAAN_THNPAJAK' => $TBLPENDATAAN_THNPAJAK, 'EQ__' . $NAMATABEL . '.TBLPENDATAAN_BLNPAJAK' => $TBLPENDATAAN_BLNPAJAK, 'EQ__TBLSETORANBPD.TBLSETORANBPD_JNSBAYAR' => 'SKPD'
			);
		}
