										...
										<div class="col-md-6">
											...
											<div style="padding: 3%;">
												<div class="row">
													...
													<section class="col col-md-4">
														<label class="input"> <i class="icon-append fa fa-calendar"></i>
															<input readonly="" style="background: #eee;" type="text" name="<?php echo $this->namatabel; ?>_TGLBTSTAGIHAN" class="datepicker form-control" data-dateformat="dd-mm-yy" id="<?php echo $this->namatabel; ?>_TGLBTSTAGIHAN">
														</label>
													</section>
												</div>
												<div class="row">
...
<script type="text/javascript">
	...
	function cari() {
		...
		$.ajax({
			...
			success: function(respon) {
				console.log(respon.model.URUTSKPD, parseInt(respon.model.URUTSKPD), parseInt(respon.model.URUTSKPD) <= 0);
				if (jenis_setoran == 'SPTPD' && respon.model.TOTALOMZET < 500000 && parseInt(respon.model.URUTSKPD) <= 0) {
					...
					if (respon.data.validate == 'kurang') {
						...
						if (respon.model.<?php echo $this->namatabel; ?>_THNSURATTAGIHAN) {
							...
							var tahunbatassurattagihan = respon.model.<?php echo $this->namatabel; ?>_THNBTSTAGIHAN;
							var bulanbatassurattagihan = respon.model.<?php echo $this->namatabel; ?>_BLNBTSTAGIHAN;
							...
							viewtahunbatassurattagihan = isikiri(tahunbatassurattagihan, "2000");
							viewbulanbatassurattagihan = isikiri(bulanbatassurattagihan, "00");
							...
							$('#<?php echo $this->namatabel; ?>_TGLBTSTAGIHAN').val(respon.model.<?php echo $this->namatabel; ?>_TGLBTSTAGIHAN + '-' + viewbulanbatassurattagihan + '-' + viewtahunbatassurattagihan);
						} else {
							$('#<?php echo $this->namatabel; ?>_TGLSURATTAGIHAN').val('');
							$('#<?php echo $this->namatabel; ?>_TGLBTSTAGIHAN').val('');
						}
	...
	function simpan() {
		if ($('#is_tanggal').is(':checked')) {
			tgl_data = ($('#is_tanggal').prop('checked') ? $('#tgl').val() : '0');
		} else {
			tgl_data = '0';
		}
		// $("#btn-Simpan").attr('disabled', 1);
		...
		if (NOMOR_SSPD == '') {
			...
			$.ajax({
					...
					data: {
						...
						totalsetor: toAngka($('#totalsetor').val()),
						bungasetor: toAngka($('#bungasetor').val()),
						dendasetor: toAngka($('#dendasetor').val()),
						pajak: toAngka($('#<?php echo $this->namatabel; ?>_PAJAK').val()),
	...
	function hitungDenda() {
		...
		if (jns_setoran == 'SKPDKB') {
			var tglbatas = $("#<?= $this->namatabel ?>_TGLBTSKRGBAYAR").val();
			var tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();
			...
			rpdenda = Number(toAngka($("#<?= $this->namatabel ?>_NAKB").val()));
			...
			var tglbatas = $("#<?= $this->namatabel ?>_TGLBATASSPTPD").val();
			var tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();
			...
			rpbunga_setor = Number(toAngka($("#bungasetor").val()));
			$("#dendasetor").val(0);
			$("#totalsetor").val((rpbunga_setor + totalbayar).toFixed(2));
			setPriceFormat();
		} else if (jns_setoran == 'STPD') {
			var tglbatas = $("#<?= $this->namatabel ?>_TGLBTSTAGIHAN").val();
			var tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();

			window.BULAN_DENDAX = getBulanDenda(convDate(tglbatas), convDate(tgldenda));

			totalpajak = Number(toAngka($('#<?= $this->namatabel ?>_BUNGATAGIHAN').val()));
			if (BULAN_DENDAX > 24) {
				BULAN_DENDAX = 24;
			} else {
				BULAN_DENDAX = BULAN_DENDAX;
			...
			rpdenda_tambahan = ((BULAN_DENDAX * 2) / 100) * totalpajak;
			rpdenda_setor = Math.ceil(rpdenda_tambahan);
			// $("#dendasetor").val(0);
			$("#dendasetor").val(rpdenda_setor.toFixed(2));
			$("#totalsetor").val((rpdenda_setor + totalpajak).toFixed(2));
			setPriceFormat();
			console.log(rpdenda_setor);
		}
	}
	...
	function cetakSSPD() {
		var param = {
			...
			TBLPENDATAAN_TGLPAJAK: btoa(btoa($('#is_tanggal').prop('checked') ? $('#tgl').val() : '0')),
