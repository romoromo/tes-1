...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entri Setoran Pajak Parkir</h4>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	...
	function generateAutocompleteWPparkir() {
		$('#nopok').autocomplete({
			serviceUrl: 'penyetoran/entrisetoranpajakparkir/autocomplete',
	...
	function cari() {
		...
		$.ajax({
			url: 'penyetoran/entrisetoranpajakparkir/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun: $('#tahun').val(),
			...
			success: function(respon) {
				if (respon.data.validate == 'kurang') {
					...
					if (respon.data.exist == 'adasetbpd') {
					...
					} else if (respon.data.exist == 'adaseto') {
						notifikasi('Data SSPD diinputkan', 'data SSPD dengan nomor pokok ' + $('#nopok').val() + ' sudah diinputkan', 'fail', 1, 0);
						$('#footer_simpandata').hide('fast');
						$("#btnCetakSSPD").show();
						$('#NOMOR_SSPD').attr('disabled', 'disabled');
	...
	function simpan() {
		...
		if (NOMOR_SSPD == '') {
			...
			$.ajax({
					url: 'penyetoran/entrisetoranpajakparkir/simpandata',
					type: 'POST',
					dataType: 'json',
					data: {
						tahun: $('#tahun').val(),
	...
	function getrekening(kode) {
		$.ajax({
			url: 'penyetoran/entrisetoranpajakparkir/getrekening',
			type: 'POST',
			dataType: 'json',
			data: {
				kode: kode,
	...
	function hitungDenda() { //_TGLBATASSPTPD
		var jns_setoran = $('#jenis_setoran').select2('val');
		if (jns_setoran == 'SKPDKB') {
			...
			window.BULAN_DENDAX = getBulanDenda(convDate(tglbatas), convDate(tgldenda));
			var totalpajak = Number(toAngka($('#<?= $this->namatabel ?>_PAJAKSKPD').val()));
			totalbayar = Number(toAngka($("#<?= $this->namatabel ?>_PAJAKSKPD").val()));
			if (BULAN_DENDAX > 24) {
				BULAN_DENDAX = 24;
			} else {
				BULAN_DENDAX = BULAN_DENDAX;
	...
	function cetakSSPD() {
		...
		window.open('penyetoran/entrisetoranpajakparkir/cetakSSPD/?' + jQuery.param(param));
	}
</script>