...
class Jabong_insidentController extends Controller
{
	...
	public function actionsimpandata()
	{
		...
		$tgl = isset($_REQUEST['tanggalpajak']) && !empty($_REQUEST['tanggalpajak']) ? $_REQUEST['tanggalpajak'] : 0;
	...
	public function CetakWordValidasiJabong($data=array())
	{
		...
		$utama['cetak_validasi_jabong_atas'] = "{$THP} {$BLP} {$HRP}    {$KE} J";
		$utama['cetak_validasi_jabong_bawah'] = "{$NOPOK} {$KODEPEND} {$KODEPAD} {$KODEPAJAK} {$KODEAYAT} {$KODEJENIS}";
