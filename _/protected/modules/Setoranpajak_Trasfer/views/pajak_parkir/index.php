...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entri Setoran Transfer Pajak Parkir</h4>
		</div>
	</div>
</div>
										...
										<div class="col-md-6">
											...
											<div style="padding: 3%;">
												...
												<div class="row">
													...
													<section class="col col-md-8">
														<label class="input">
															<input style="background: #eee;" class="form-control" readonly="" type="text" id="<?php echo $this->namatabel; ?>_NAKB">
														</label>
													</section>
												</div>
...
<script type="text/javascript">
	...
	function generateAutocompleteWP() {
		$('#nopok').autocomplete({
			serviceUrl: 'Setoranpajak_Trasfer/pajak_parkir/autocomplete',
	...
	function cari_old() {
		...
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajak_parkir/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun: $('#tahun').val(),
			...
			success: function(respon) {
				if (respon.data.validate == 'kurang') {
					...
					if (respon.model.<?php echo $this->namatabel; ?>_NAKB) {
						$('#<?php echo $this->namatabel; ?>_NAKB').val(toRp(respon.model.<?php echo $this->namatabel; ?>_NAKB));
					}
		...
		$.ajax({
			...
			success: function(respon) {
				if (respon.data.validate == 'kurang') {
					...
					if (respon.data.exist_bpd == 'ada') {
						notifikasi('Data pernah disetor di menu Pembayaran Langsung', 'data dengan nomor pokok ' + $('#nopok').val() + ' disetor melalui menu setor pajak langsung. Silahkan gunakan menu <b>Setoran Pajak Langsung</b>', 'fail', 1, 0);
						$('#id_respencarian').hide();
						$('#footer_simpandata').hide();
						return false;
					}
	...
	function simpan() {
		...
		if (NOMOR_SSPD == '') {
			...
			$.ajax({
					url: 'Setoranpajak_Trasfer/pajak_parkir/simpandata',
					type: 'POST',
					dataType: 'json',
					data: {
						tahun: $('#tahun').val(),
	...
	function getrekening(kode) {
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajak_parkir/getrekening',
			type: 'POST',
			dataType: 'json',
			data: {
				kode: kode,
	...
	function hitungDenda_old() {
		...
		if (jns_setoran == 'SKPD' || jns_setoran == 'SKPDKB') {
			...
			rpdenda_setor = Math.ceil(rpdenda + rpdenda_tambahan);
	...
	function hitungDenda() {
		...
		if (jns_setoran == 'SKPDKB') {
			...
			if ($('#is_bayarnakb').is(':checked')) {
				var rpdenda = Number(toAngka($("#<?= $this->namatabel ?>_NAKB").val()));
			} else {
				var rpdenda = 0;
			}
	...
	function cetakSSPD() {
		...
		window.open('Setoranpajak_Trasfer/pajak_parkir/cetakSSPD/?' + jQuery.param(param));
	}
</script>