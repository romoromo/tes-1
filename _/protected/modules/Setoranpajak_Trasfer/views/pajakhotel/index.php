...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Entri Setoran Transfer Pajak Hotel</h4>
		</div>
	</div>
</div>
...
<section id="widget-grid" class="">
	...
	<div class="row">
		...
		<article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
			...
			<div class="jarviswidget jarviswidget-color-teal" id="wid-id-validasi_penyetoran" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-custombutton="false" data-widget-fullscreenbutton="false" data-widget-colorbutton="false" data-widget-togglebutton="false">
				<header>
						...
						<form action="" id="form_sumber_pajak" class="smart-form" novalidate>
							<fieldset>
								...
								<div class="row">
									...
									<?php if (Yii::app()->user->username == 'dmsuryo') : ?>
										<section class="col col-md-2">
											<a class="btn btn-sm btn-primary" onclick="cari()">
												Enter Simulasi <i class="fa fa-forward"></i>
											</a>
										</section>
									<?php endif ?>
									<section class="col col-md-3">
										<!-- <a class="btn btn-sm btn-primary" onclick="caridata()"> -->
										<a class="btn btn-sm btn-primary" onclick="cari()"> <!-- add ceklist KB rahmat 9-5-2022 -->
									...
									<div class="row">
										...
										<div class="col-md-6">
											...
											<div style="padding: 3%;">
												...
												<div class="row">
													...
													<section class="col col-md-1">
														<label class="checkbox pull-right">
															<input type="checkbox" checked="" name="is_bayarpokok" id="is_bayarpokok">
															<i></i>
														</label>
													</section>
													<section class="col col-md-8">
														<label class="input">
															<input style="background: #eee;" class="form-control" readonly="" type="text" id="<?php echo $this->namatabel; ?>_PAJAKPERIKSA">
														</label>
													</section>
												</div>
												<div class="row">
													<section class="col col-md-3">Bunga LHP</section>
													<section class="col col-md-1">
														<label class="checkbox pull-right">
															<input type="checkbox" checked="" name="is_bayarbunga" id="is_bayarbunga">
															<i></i>
														</label>
													</section>
													<section class="col col-md-8">
												...
												<div class="row">
													<section class="col col-md-3">Denda LHP</section>
													<section class="col col-md-1">
														<label class="checkbox pull-right">
															<input type="checkbox" checked="" name="is_bayarnakb" id="is_bayarnakb">
															<i></i>
														</label>
													</section>
													<section class="col col-md-8">
														<label class="input">
															<input style="background: #eee;" class="form-control" readonly="" type="text" id="<?php echo $this->namatabel; ?>_KENAIKANKRGBAYAR">
														</label>
													</section>
												</div>
										...
										<div class="col col-md-12">
											...
											<div class="row">
												...
												<section class="col col-md-4">
													<button type="button" style="display: none;" id="btnCetakSSPD" class="btn btn-default btn-sm" onclick="cetakSSPD()"><i class="fa fa-print"></i> Cetak Ulang SSPD</button>
												</section>
											</div>
										</div>
									</div>
...
<script type="text/javascript">
	...
	$("#is_bayarpokok, #is_bayarbunga, #is_bayarnakb").click(function(event) {
		hitungDenda();
	});
	...
	function cari() {
		...
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajakhotel/getdata',
			type: 'POST',
			dataType: 'json',
			data: {
				tahun: $('#tahun').val(),
			...
			success: function(respon) {
				if (respon.data.validate == 'kurang') {
					...
					if (jns_setoran != 'SKPDKB') {
						...
						window.cekbungaspthasilentri = respon.model.BUNGA_SETOR;
					}

					$('#TANGGAL_ENTRY').val('<?= date('d-m-Y') ?>');
					if (jns_setoran == 'SPTPD') {
						$('#TANGGAL_HITUNGAN_DENDA').val(respon.model.<?php echo $this->namatabel; ?>_TGLSPTPD + '-' + bulansptpd + '-' + viewtahunthnsptpd);
					} else {
					...
					if (respon.model.<?php echo $this->namatabel; ?>_PAJAKPERIKSA) {
						$('#<?php echo $this->namatabel; ?>_PAJAKPERIKSA').val(toRp(respon.model.<?php echo $this->namatabel; ?>_PAJAKPERIKSA));
					}
					if (respon.model.<?php echo $this->namatabel; ?>_KENAIKANKRGBAYAR) {
						$('#<?php echo $this->namatabel; ?>_KENAIKANKRGBAYAR').val(toRp(respon.model.<?php echo $this->namatabel; ?>_KENAIKANKRGBAYAR));
					}
	...
	function simpan() {
		...
		if (NOMOR_SSPD == '') {
			...
			$.ajax({
					url: 'Setoranpajak_Trasfer/pajakhotel/simpandata',
					type: 'POST',
					dataType: 'json',
					data: {
						tahun: $('#tahun').val(),
						...
						jmlsetor: toAngka($('#totalsetor').val()),
	...
	function getrekening(kode) {
		$.ajax({
			url: 'Setoranpajak_Trasfer/pajakhotel/getrekening',
			type: 'POST',
			dataType: 'json',
			data: {
				kode: kode,
	...
	function hitungDenda() {
		...
		if (jns_setoran == 'SKPDKB') {
			...
			if ($('#is_bayarpokok').is(':checked')) {
				tglbatas = $("#<?= $this->namatabel ?>_TGLBTSKRGBAYAR").val();
				tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();
				window.BULAN_DENDAX = getBulanDenda(convDate(tglbatas), convDate(tgldenda));
				var totalpajak = Number(toAngka($('#<?= $this->namatabel ?>_PAJAKPERIKSA').val()));
				if (BULAN_DENDAX > 24) {
					BULAN_DENDAX = 24;
				} else {
					BULAN_DENDAX = BULAN_DENDAX;
				}
				rpdenda_tempo = ((BULAN_DENDAX * 2) / 100) * totalpajak;
				totalpokok = Number(toAngka($('#<?= $this->namatabel ?>_PAJAKPERIKSA').val()));
			} else {
				rpdenda_tempo = 0;
				totalpokok = 0;
			}
			if ($('#is_bayarbunga').is(':checked')) {
				var bunga = Number(toAngka($('#<?= $this->namatabel ?>_BUNGAKRGBAYAR').val()));
			} else {
				bunga = 0;
			}
			if ($('#is_bayarnakb').is(':checked')) {
				var rpdenda = Number(toAngka($("#<?= $this->namatabel ?>_KENAIKANKRGBAYAR").val()));
			} else {
				var rpdenda = 0;
			}

			$('#pajaksetor').val(toRp(totalpokok));
			$('#bunga_setor').val(toRp(bunga));
			rpdenda_total = rpdenda_tempo + rpdenda;
			$("#dendasetor").val(toRp(rpdenda_total.toFixed(2)));
			totalsetor = totalpokok + bunga + rpdenda + rpdenda_tempo;
			console.log(BULAN_DENDAX);
			// console.log(bunga)
			// console.log(rpdenda_total);

			$("#totalsetor").val((totalsetor).toFixed(2));
			setPriceFormat();
		} else if (jns_setoran == 'SKPD') {
			tglbatas = $("#<?= $this->namatabel ?>_TGLBATASSPTPD").val();
			tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();

			window.BULAN_DENDAX = getBulanDenda(convDate(tglbatas), convDate(tgldenda));
			var totalpajak = Number(toAngka($('#<?= $this->namatabel ?>_PAJAK').val()));
			totalbayar = Number(toAngka($("#<?= $this->namatabel ?>_PAJAK").val()));
			if (BULAN_DENDAX > 24) {
				BULAN_DENDAX = 24;
			} else {
				BULAN_DENDAX = BULAN_DENDAX;
			}
			rpdenda = ((BULAN_DENDAX * 2) / 100) * totalpajak;
			rpdenda_setor = Math.ceil(rpdenda);
			$("#dendasetor").val(rpdenda_setor.toFixed(2));
			$("#totalsetor").val((rpdenda_setor + totalbayar).toFixed(2));
			setPriceFormat();
		} else if (jns_setoran == 'SPTPD') {
			if (window.cekbungaspthasilentri > '0') {
			...
			} else { // hitung bunga jika lapor dari esptpd belum muncul bunga pdahal telat lapor
				tglbatas = $("#<?= $this->namatabel ?>_TGLBATASSPTPD").val();
				tgldenda = $("#TANGGAL_HITUNGAN_DENDA").val();

				window.BULAN_BUNGAX = getBulanDenda(convDate(tglbatas), convDate(tgldenda));
				var totalpajak = Number(toAngka($('#<?= $this->namatabel ?>_PAJAK').val()));
				var totalbayar = Number(toAngka($("#<?= $this->namatabel ?>_PAJAK").val()));


				// rpbunga_setor = Number(toAngka($("#<?= $this->namatabel ?>_DENDAKRGBAYAR").val()));
				if (BULAN_BUNGAX > 24) {
					BULAN_BUNGAX = 24;
				} else {
					BULAN_BUNGAX = BULAN_BUNGAX;
				}
				rpbunga = ((BULAN_BUNGAX * 2) / 100) * totalpajak;
				rpbunga_setor = Math.ceil(rpbunga);
				$('#<?php echo $this->namatabel; ?>_BUNGASPTPD').val(toRp(rpbunga_setor)); //ngisi div bunga jika bunga dari tabel null
				$("#bunga_setor").val(toRp(rpbunga_setor.toFixed(2)));
				$("#dendasetor").val(0);
				$("#totalsetor").val((rpbunga_setor + totalbayar).toFixed(2));
				setPriceFormat();
			}
	...
	function cetakSSPD() {
		...
		window.open('Setoranpajak_Trasfer/pajakhotel/cetakSSPD/?' + jQuery.param(param));
	}
</script>