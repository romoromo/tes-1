...
class Pajak_restoranController extends Controller
{
	...
	public function actiongetdata()
	{
		...
		public function cekpernahsetor($dataFilter)
		{
			...
			$model = Yii::app()->db->createCommand("
				...
				WHERE 
				TBLPENDATAAN_THNPAJAK = ".(int) substr($dataFilter[':tahun'], -2)."
				AND TRIM(TBLSETOR.TBLSETOR_JNSBAYAR)='SKPDKB'
				AND NVL(TBLPENDATAAN_BLNPAJAK, 0) = ".$dataFilter[':bln']." 
				AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$dataFilter[':tgl']." 
				AND NVL(TBLPENDATAAN_PAJAKKE, 0) = ".$dataFilter[':ke']." 
				AND TBLDAFTAR_NOPOK =".$dataFilter[':nopok']." 
			...
			$model = Yii::app()->db->createCommand("
				...
				WHERE 
				TBLPENDATAAN_THNPAJAK = ".(int) substr($dataFilter[':tahun'], -2)."
				AND TRIM(TBLSETOR.TBLSETOR_JNSBAYAR)='STPD'
				AND NVL(TBLPENDATAAN_BLNPAJAK, 0) = ".$dataFilter[':bln']." 
				AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$dataFilter[':tgl']." 
				AND NVL(TBLPENDATAAN_PAJAKKE, 0) = ".$dataFilter[':ke']." 
				AND TBLDAFTAR_NOPOK =".$dataFilter[':nopok']." 
			...
			$model = Yii::app()->db->createCommand("
				...
				WHERE 
				TBLPENDATAAN_THNPAJAK = ".(int) substr($dataFilter[':tahun'], -2)."
				AND TRIM(TBLSETOR.TBLSETOR_JNSBAYAR)='SPTPD'
				AND NVL(TBLPENDATAAN_BLNPAJAK, 0) = ".$dataFilter[':bln']." 
				AND NVL(TBLPENDATAAN_TGLPAJAK, 0) = ".$dataFilter[':tgl']." 
				AND NVL(TBLPENDATAAN_PAJAKKE, 0) = ".$dataFilter[':ke']." 
				AND TBLDAFTAR_NOPOK =".$dataFilter[':nopok']." 
		...
		if ($jenis_setoran == 'SPTPD') {
			...
			$TBLSETOR_RUPIAHTAGIHAN = 0;
			$TBLSETOR_BUNGAKETETAPAN = $bunga_setor;
			$TBLSETOR_DENDAKETETAPAN = isset($setbpd[$NAMATABEL . '_NAKB']) ? $setbpd[$NAMATABEL . '_NAKB'] : 0;
			$TBLSETOR_DENDAKURANGBAYAR = (isset($setbpd[$NAMATABEL . '_NAKB']) ? $setbpd[$NAMATABEL . '_NAKB'] : 0) + $dendasetor;
		} elseif ($jenis_setoran == 'STPD') {
			$TBLSETOR_RUPIAHPAJAK = 0;
			$TBLSETOR_RUPIAHSETOR = $pajaksetor;
...
if ($bunga_setor>'0' && $jenis_setoran!='STPD') {
	...
	$arrayOfDataInsert['TBLSETOR_RUPIAHSETOR'] = $TBLSETOR_BUNGAKETETAPAN;
	$arrayOfDataInsert['TBLSETOR_BUNGAKETETAPAN'] = $TBLSETOR_BUNGAKETETAPAN;
...
if ($dendasetor>'0') {
	...
	$arrayOfDataInsert['TBLSETOR_RUPIAHSETOR'] =
			$jenis_setoran == 'SKPDKB' ? $TBLSETOR_DENDAKURANGBAYAR : $TBLSETOR_DENDAKETETAPAN;
	$arrayOfDataInsert['TBLSETOR_DENDAKETETAPAN'] =
			$jenis_setoran == 'SKPDKB' ? $TBLSETOR_DENDAKURANGBAYAR : $TBLSETOR_DENDAKETETAPAN;
...
public function CetakWordSSPD($data=array())
{
	...
	$namafileDL = "SSPD-Transfer-Restoran.docx"; 
	...
	$otbs->NoErr = 0;
		...
		if ($jenis_setoran=='STPD' || $jenis_setoran == 'SKPD') {
			...
			$nominal3 = $rowWP['TBLSETOR_DENDAKURANGBAYAR'];
