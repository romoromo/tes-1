...
class PajakppjController extends Controller
{
	public $namatabel = 'TBLDAFTPJU';
	...
	public function actiongetdata()
	{
		...
		//$idgolongan_tarif = trim($_POST['idgolongan_tarif']);
		...
		$dataFilter[':bln'] = $bln;
		$dataFilter[':tgl'] = $tgl;
		//$dataFilter[':ke'] = $idgolongan_tarif;
		...
		if ($tahun=="") {
			...
			"
			;
				// AND TBLPENDATAAN_PAJAKKE='".$idgolongan_tarif."'
	...
	public function actionsimpandata()
	{
		...
		$pajaksetor = isset($DATA['pajaksetor']) && !empty($DATA['pajaksetor']) ? LokalIndonesia::toAngka($DATA['pajaksetor']) : 0;
		$jenis_setoran = isset($DATA['jenis_setoran']) && !empty($DATA['jenis_setoran']) ? LokalIndonesia::toAngka($DATA['jenis_setoran']) : 0;
		//$idgolongan_tarif = isset($DATA['idgolongan_tarif']) && !empty($DATA['idgolongan_tarif']) ? $DATA['idgolongan_tarif'] : 0;
		...
		$whereUpdate = 
			...
			";
			// AND NVL(TBLDAFTPJU_IDGOLTARIF, 0) = :idgolongan_tarif
		...
		$bindParam = array(
			...
			// ,':ke' => $idgolongan_tarif
			// ,':idgolongan_tarif' => $idgolongan_tarif
		...
		$arrayOfDataInsert = array(
			...
			,'TBLSETOR_THNTAGIHAN' => ''
			,'BAKB' => ''
			,'TBLSETOR_RUPIAHPAJAK' => isset($setbpd[$NAMATABEL.'_PAJAK']) ? $setbpd[$NAMATABEL.'_PAJAK'] : 0
			,'TBLSETOR_RUPIAHSETOR' => isset($setbpd[$NAMATABEL.'_PAJAK']) ? $setbpd[$NAMATABEL.'_PAJAK'] : 0
			,'TBLSETOR_KETETAPANPAJAK' => isset($setbpd[$NAMATABEL.'_PAJAK']) ? $setbpd[$NAMATABEL.'_PAJAK'] : 0
			,'RPRET' => ''
			,'TBLSETOR_RUPIAHTAGIHAN' => ''
			,'TBLSETOR_PAJAKKURANGBAYAR' => isset($setbpd[$NAMATABEL.'_RUPIAHKRGBAYAR']) ? $setbpd[$NAMATABEL.'_RUPIAHKRGBAYAR'] : ''
			,'TBLSETOR_BUNGAKETETAPAN' => isset($setbpd[$NAMATABEL.'_BUNGASPTPD']) ? $setbpd[$NAMATABEL.'_BUNGASPTPD'] : ''
			,'TBLSETOR_BUNGATAGIHAN' => ''
			,'TBLSETOR_BUNGAKURANGBAYAR' => isset($setbpd[$NAMATABEL.'_BUNGAKRGBAYAR']) ? $setbpd[$NAMATABEL.'_BUNGAKRGBAYAR'] : ''
			,'BURET' => "0"
			,'TBLSETOR_DENDAKETETAPAN' => ""
			,'TBLSETOR_DENDATAGIHAN' => "0"
			,'TBLSETOR_DENDAKURANGBAYAR' => ""
			,'DDRET' => ''
			,'NAIK' => ''
			,'ADM' => ''
		...
		if ($bunga_setor>0) {
			...
			//$arrayOfDataInsert['TBLPENDATAAN_PAJAKKE'] = $idgolongan_tarif;
	...
	public function actionCetakSSPD()
	{
		...
		$NAMATABEL = $this->namatabel;
         ...
         $select = "
         	...
         	,NVL({$NAMATABEL}_PAJAKSKPDINDUSTRI, 0) AS {$NAMATABEL}_PAJAKSKPDINDUSTRI
         	,NVL({$NAMATABEL}_PAJAKSKPDNON, 0) AS {$NAMATABEL}_PAJAKSKPDNON
			...
			(
				...
				AND TBLSETOR.TBLSETOR_REKAYAT = {$NAMATABEL}.{$NAMATABEL}_REKAYAT
	...
	public function CetakWordSSPD($data=array())
	{
		...
		$namatpl = $path_tpl . 'SSPD-TRANSFER-PPJ.docx';
		$namafileDL = "SSPD-Transfer-PPJ.docx"; 
		...
		$utama['pajakind'] = LokalIndonesia::ribuan($rowWP[$NAMATABEL.'_PAJAKSKPDINDUSTRI']);
		$utama['pajaknon'] = LokalIndonesia::ribuan($rowWP[$NAMATABEL.'_PAJAKSKPDNON']);
