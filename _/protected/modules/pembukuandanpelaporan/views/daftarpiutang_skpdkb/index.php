<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		<div class="well well-sm well-light bg-color-greenLight txt-color-white">
			<h4><i class="fa fa-file-text-o"></i> Daftar Piutang SKPDKB dan SKPD-A </h4>
		</div>
	</div>
</div>
    						...
    						<label class="select">
    							<select class="select2" id="TBLREKENING_KODE" name="para[TBLREKENING_KODE]">
    								...
    								<?php foreach ($data['rek'] as $list): ?>
    									<option value="<?php echo $list['TBLREKENING_KODE'] ?>"><?php echo $list['TBLREKENING_NAMAREKENING'] ?></option>
    								<?php endforeach ?>
    							</select>
    						</label>
    					</section>
...
function cari () {
			...
			$.ajax({
				url: 'pembukuandanpelaporan/daftarpiutang_skpdkb/Cari',
				type: 'POST',
				data: {
      ...
      var jenis_setoran = $('#jenis_setoran').val();
			...
			window.open('<?php echo Yii::app()->getBaseUrl(1) ?>/pembukuandanpelaporan/daftarpiutang_skpdkb/Cetak?TBLDAFTAR_NOPOK='+TBLDAFTAR_NOPOK+'jenis_setoran='+jenis_setoran+'&TBLREKENING_KODE='+TBLREKENING_KODE+'&tahunkb='+tahunkb+'&jenis='+jenis+'&tahunpjk='+tahunpjk+'&tahunpjk_akhir='+tahunpjk_akhir+'&tahunkb_akhir='+tahunkb_akhir);
		}
        ...
        , tahunkb_akhir : ($('#tahun_kb').prop('checked') ? $('#tahunkb_akhir').val() : '')
			...
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/pembukuandanpelaporan/daftarpiutang_skpdkb/Cetak?'+ param);
		}
       ...
       , tahunpjk : ($('#th_pajak').prop('checked') ? $('#tahunpjk').val() : '')
        ...
        , tahunkb_akhir : ($('#tahun_kb').prop('checked') ? $('#tahunkb_akhir').val() : '')
			...
			window.open('<?= Yii::app()->getBaseUrl(1); ?>/pembukuandanpelaporan/daftarpiutang_skpdkb/CetakWord?'+ param);
		}
