	...
	<table class="table table-bordered table-striped" id="tbl_daftarpermohonan" style="margin-bottom: -1px !important;">
		...
		<tbody>
			...
			<tr >
				...
				<td>
					...
					<button rel="tooltip" data-placement="left" data-original-title="Digunakan untuk mengentri ketetapan pajak" class="btn btn-sm btn-default btn-block <?= $rowNPWPD['TNPWPD_ISPENGUKUHAN']=='F' ? "BLMKUKUH" : "" ?>" onclick="entri_pendataan('<?php echo $rowNPWPD['TNPWPD_ID'] ?>','<?php echo $rowNPWPD['TBLREKENING_KODE'] ?>')">
						<i class="fa fa-money"></i> Entri Pendataan
					</button>
					<?php if (substr($rowNPWPD['TBLREKENING_KODE'],0,5)!='41104'): ?>
					<button rel="tooltip" data-placement="left" data-original-title="Digunakan untuk mengentri data detail obyek" class="btn btn-sm bg-color-orange txt-color-white btn-block" onclick="entri_detail('<?php echo $rowNPWPD['TNPWPD_ID'] ?>','<?php echo $rowNPWPD['TBLREKENING_KODE'] ?>')">
			...
			<tr>
				...
				<td>[<?= $rowNPWPD['TBLREKENING_KODE'] ?>] <?= $rowNPWPD['TBLREKENING_NAMA'] ?></td>
			...
			<tr>
				...
				<td><?php echo strtotime($rowNPWPD['TNPWPD_PENGUKUHANTGL']) ? LokalIndonesia::TanggalUmum( $rowNPWPD['TNPWPD_PENGUKUHANTGL'] ) : '<label class="txt-color-red">Belum Dikukuhkan</label>' ?></td>
			...
			<tr>
				...
				<td><?php echo $rowNPWPD['TNPWPD_ALAMAT'] ?><br><?php echo $rowNPWPD['REFKELURAHAN_NAMA'] ?>,<?php echo $rowNPWPD['REFKECAMATAN_NAMA'] ?></td>

				<td style="background-color: rgba(247, 233, 216, 0.4);width: 16%;"><strong>Telepon</strong></td>
				<td><?= $rowNPWPD['TNPWPD_TELPON'] ?></td>
				...
				<td>
					...
					if (!in_array(substr($rowNPWPD['TBLREKENING_KODE'],0,5), $pajaknocetakperbup)): ?>
					...
					<a target="_blank" href="<?= Yii::app()->getBaseUrl(1) ?>/cetak/cetak_kartudata/cetaksptpd?listData=<?= $rowNPWPD['TNPWPD_ID'] ?>&tahun=<?= date('Y') ?>&transid=<?= base64_encode($last['tbltransaksiketetapan_id']) ?>" rel="tooltip" data-placement="left" data-original-title="Digunakan untuk mencetak lembar SPTPD" class="btn btn-sm bg-color-magenta txt-color-white btn-block <?= $rowNPWPD['TNPWPD_ISPENGUKUHAN']=='F' ? "BLMKUKUH" : "" ?> <?= !$last ? "BLMKUKUH" : "" ?>">
		...
		<?php foreach ($listnosptpd2 as $nosptpd2): ?>
			...
			<tr >
				...
				<td>
					<?php /*button  rel="tooltip" data-placement="left" data-original-title="Digunakan untuk  melihat riwayat ketetapan pajak" class="btn btn-sm btn-default btn-block" onclick="entri_pendataan('<?php echo $rowNPWPD['TNPWPD_ID'] ?>','<?php echo $rowNPWPD['TBLREKENING_KODE'] ?>')">
						<i class="fa fa-money"></i> Entri Pendataan
					</button>
					<?php if (substr($rowNPWPD['TBLREKENING_KODE'],0,5)!='41104'): ?>
					<button rel="tooltip" data-placement="left" data-original-title="Digunakan untuk mengentri data detail obyek" class="btn btn-sm bg-color-orange txt-color-white btn-block" onclick="entri_detail('<?php echo $rowNPWPD['TNPWPD_ID'] ?>','<?php echo $rowNPWPD['TBLREKENING_KODE'] ?>')">
...
<script type="text/javascript">
	function pilih_pendataan (id) {
		...
		$.ajax({
			url: '<?= $this->URL_MODUL ?>/detailObyek',
			type: 'POST',
			data: {id: id},
	...
	function changeKeaktifanObyek(id, stat) {
		$.ajax({
			url: '<?= $this->URL_MODUL ?>/changeKeaktifanObyek',
			type: 'POST',
			dataType: 'json',
			data: {
				objid: id
