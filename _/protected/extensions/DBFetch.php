<?php
	/**
	* written by : DMG <Diginet Media Group>
			...
			if ( count($filter)>0 ) { // jika ada filtering
				...
				foreach ($filter as $key => $value) {
					...
					if ($filter[$key]!=='') { // jika mengandung namatabel.namakolom, maka kita pecah wherenya untuk :alias namakolom
						...
						if ( $comparatorType=='LKR') {//jika comparatornya LIKE Right maka beri suffix %
							$value = $value.'%';
							$kolom = 'lower(' . $kolom . ')';
						}
						if ( $comparatorType=='LKL') {//jika comparatornya LIKE Left maka beri prefix %
							$value = '%'.$value;
							$kolom = 'lower(' . $kolom . ')';
						}
