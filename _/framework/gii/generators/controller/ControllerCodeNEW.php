...
class ControllerCode extends CCodeModel
{
	...
	public function getControllerFile()
	{
		...
		// __var_dump($this->getControllerRelativePath($this->controller));
		$id=$this->getControllerID();
		if(($pos=strrpos($id,'/'))!==false)
			$id[$pos+1]=strtoupper($id[$pos+1]);
		else
			$id[0]=strtoupper($id[0]);
		return $module->getControllerPath().'/'.$this->getControllerRelativePath($this->controller).'/'.$id.'Controller.php';
	}
	...
	public function getControllerRelativePath($path){
		foreach(explode("/",$path) as $module){
